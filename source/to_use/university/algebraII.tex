...
Let \(v' \in v + \text{ker}_\varphi\), meaning that \(v' = v + w\)
with \(w \in \text{ker}_\varphi\).
\begin{align*}
...
\begin{enumerate}
    \item \[
            \overline{\varphi}(0_{V_1} + \text{ker}_\varphi) =
            \varphi(0_{V_1}) = 0_{V_2}
        \]
    \item
        \begin{align*}
            \overline{\varphi}((v_1 + \text{ker}_\varphi) + (v_2 +
            \text{ker}_\varphi))
...
Il seguente diagramma commuta e \(\pi\) è suriettiva in quanto
\(v+\text{ker}_\varphi = \pi(v)\).
...
\begin{tikzcd}
    V_1 \arrow[r, "\varphi"] \arrow[d, "\pi"', two heads]
    & V_2 \\
...
\begin{snippettheorem}{First isomorphism theorem}
...
\begin{snippetproof}{First isomorphism theorem}
        ...
        \item we want to show that the kernel is trivial.
            \begin{align*}
                \text{ker}_{\overline{\varphi}} &= \{
                    v + \text{ker}_\varphi \suchthat
                    \overline{\varphi}(v + \text{ker}_\varphi) = 0_{V_2}
                \} \\
                &= \{v + \text{ker}_\varphi \suchthat v \in
                \text{ker}_\varphi\} \\
                &= 0_{V_1} + \text{ker}_\varphi
            \end{align*}
            since \(v + \text{ker}_\varphi = \text{ker}_\varphi\) and
            we can just choose \(0_{V_1}\).
    \end{enumerate}
\end{snippetproof}
...
\begin{snippetexample}{}
    ...
    \]
    Using the diagrams with \(\overline{p_1},\pi_1\) and
    \(\overline{p_2},\pi_2\),
    we have
    \[
...
\begin{snippettheorem}{Second isomorphism theorem}
...
\begin{snippetproof}{Second isomorphism theorem}
    ...
    \begin{align*}
        \text{ker}_\varphi &= \{
            w \in W \suchthat w + U = 0_{V/U} = 0_V + U
        \} \\
        &= \{
...
\begin{snippettheorem}{Third isomoprhism theorem}
    Sia \(V\) uno spazio vettoriale e \(W \leq V\) e \(U \leq W\) dei
    sottospazi.
...
\begin{snippetproof}{Third isomoprhism theorem}
    ...
    dato \(v' \in v + U\) diverso da \(v\), e quindi \(v' = v+u\) con
    \(u\in U\) vale
    \begin{align*}
    ...
    \begin{align*}
        \text{ker}_{\overline{\varphi}}
        &= \{
            v + U \in U/V \suchthat v + W = \overline{\varphi}(v + U)
            = 0_{V/W} = 0_V + W
        \} \\
        &= \{
...
\((Z, +, \cdot)\) è un anello commutativo dove gli elementi
invertibili sono solo \(\pm 1\). \\
\((\mathbb{K}[x], +, \cdot)\) è un anello commutativo dove gli
elementi invertibili sono solo i polinomi
di grado zero.

Algebra gruppale: Sia \(G\) un gruppo e sia \(\mathbb{K}\) un campo.
\[
    G[\mathbb{K}] = \left\{
        \sum_{g\in G} \lambda g \suchthat \lambda \in \mathbb{K}
    \right\}
\]
(Giusto?)
...
\begin{align*}
    ...
    &= \sum_{k\in G} \left(\sum_{g\cdot h = k} (\lambda_g \lambda_h)
    \right) \cdot k
\end{align*}
...
\begin{snippetexample}{I quaternioni sono una algebra reale}
    \[
        \mathbb{H} = \text{span}\{e,i,j,k\}
    \]
    Abbiamo che
    \[
        Z(\mathbb{H}) = \text{span}\{e\} \cong \realnumbers
    \]
...
Sia \(A = \mathbb{Z}\) e \(I = \langle n \rangle = \{n \cdot k
\suchthat k \in \mathbb{Z}\}\).
Abbiamo quindi il quoziente \(\mathbb{Z}/_{\langle n \rangle}\) che è dato da
\[
    a+I = a+\langle n \rangle = {[a]}_n
\]
Chiaramente \(1_{\mathbb{Z}/_{\langle n \rangle}} = {[1]}_n\).
...
\begin{align*}
    {[ak_1]}_p &= ak_1 + \langle p \rangle = \left(1 + (-k_2)p\right)
    + \langle p \rangle
    = 1 + \langle p \rangle = {[1]}_p \\
...
Consideriamo \(\mathbb{Q}[x]/_{\langle f \rangle}\).
Due laterali sono per esempio \((x-2) + \langle f \rangle\)
e \((x-1) + \langle f \rangle\) che non sono lo zero.
Invece il loro prodotto
\[
    \left((x-2) + \langle f \rangle\right) \cdot
    \left((x-1) + \langle f \rangle\right)
    = f + \langle f \rangle = 0 + \langle f \rangle
\]
...
\[
    \left((x-2) + \langle f \rangle\right) \cdot
    \left((x^3 - 1) + \langle f \rangle\right) = f \cdot (x^2 + x +
    1) + \langle f \rangle = 0 + \langle f \rangle
\]
...
\begin{align*}
    (x^3 - 1) + \langle f \rangle &= 7x-6 - 1 + \langle f \rangle \\
    &= 7x-7 + \langle f \rangle \\
    &= 7(x+1) + \langle f \rangle
\end{align*}
...
\begin{snippettheorem}{Teorema cinese del resto}
    Let \(A\) be a commutative ring, \(I_1, \cdots, I_r\)
    coprime ideals. The homomorphism
    \[
        \varphi\colon A \to \bigoplus_{i=1}^r A/I_i
    \]
    dato da \(\varphi = (\pi_1, \cdots, \pi_r)\)
    le proiezioni \(\pi_i \colon A \to A/I_r\).
    The kernel
    \[
        \text{ker}(\varphi) = \bigcap_{i=1}^r I_i = \prod_{i=1}^r I_r
    \]
...
\begin{snippetproof}{}
    ...
    \begin{align*}
        \text{ker}(\varphi) &= \{
            a \in A \suchthat \varphi(a) = 0_{\oplus A/I_i} =
            (0_{A/I_1}, \cdots, 0_{A/I_r})
        \} \\
        &= \{
            a \in A \suchthat \varphi(a) = (0_A + I_1, \cdots, 0_A + I_r)
        \} \\
        &= \{
            a \in A \suchthat a \in I_1, I_2, \cdots, I_2
        \} \\
        &= \bigcap_{i=1}^r I_i = \prod_{i=1}^r I_i
    \end{align*}
    We now show that it is surjective. We need to show that \(\forall
    i\), there exists \(a_i \in A\)
    such that
    \begin{align*}
        \varphi(a_i) = (0_A + I_1, \cdots, 1_A + I_i, \cdots, 0_A + I_r)
    \end{align*}
    Furthermore, \(\pi_i \colon A \to A/I_i\) is surjective
    which means that \(\forall b + I_i \in A/I_i\).
    \begin{align*}
        \pi_i(b) = b+I_i &\implies (0_A + I_1, \cdots, b + I_i,
        \cdots, 0_A + I_r) \\
        &= \varphi(b) \cdot \varphi(a_i) = (x_1, \cdots, b + I_i, \cdots, \ast)
        \cdot (0_{A/I_i}, \cdots, 1 + I_i, \cdots, 0_{A/I_r}) \\
        &= \varphi(b \cdot a_i)
    \end{align*}
    We can thuse create every element by composing \(0\)'s. \\
    Note that \(I_i\) and \(\bigcap_{i\neq j} I_j\) are coprimi,
    meaning \(\exists x_i \in I_i\)
    and \(y_i \in \bigcap_{i\neq j} I_j\) such that \(x_i + y_i = 1_A\).
    Thus
    \begin{align*}
        \varphi(y_i) &= (\pi_1(y_i), \cdots, \pi_i(y_i), \cdots, \pi_r(y_i)) \\
        &= (y_i + I_1, \cdots, y_i + I_i, \cdots, y_i + I_r) \\
        &= (0_A + I_1, \cdots, (1_A - x_i) + I_i, \cdots, 0_A + I_r) \\
        &= (0_{A/I_1}, \cdots, 1_{A/I}, \cdots, 0_{A/I_r})
    \end{align*}
    e quindi \(a_i = y_i\).
    Quindi un generico elemento della somma diretta pu?o essere
    scritto come immagine di questo.
\end{snippetproof}
