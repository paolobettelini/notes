...
\sdefinition{Componente connesse per archi}{
    ...
    \[
        x \sim y \iff \exists \alpha [0,1] \fromto X \suchthat \alpha(0) = x \land \alpha(1) = y
    \]
...
\sdefinition{}{
    ...
    \begin{enumerate}
        \item \(R(x,0) \in Y\) per \(x \in X\);
        \item \(R(x, 1) = x\) per \(x \in X\);
        \item \(R(y, 1) = y\) per \(y \in Y\) e \(t\in I\).
    \end{enumerate}
}
...
\sproof{}{
    \begin{enumerate}
        ...
        L'idea è quella di percorrere \(\alpha\) da \(0\) a \(t/2\),
        \(1_a\) da \(t/2\) a \(1-t/2\), e \(i(\alpha)\)
        da \(1-t/2\) a \(1\). Abbiamo quindi
        \[
            F(t,s) = \begin{cases}
                \alpha(2t) & 0 \leq t \leq \frac{s}{2} \\
                \alpha(s) & \frac{s}{2} \leq t \leq 1-\frac{s}{2} \\
                \alpha(2-2t) & 1 - \frac{s}{2} \leq t \leq 1
            \end{cases}
        \]
        Notiamo che per \(s=0\) e \(s=1\) ottengo i due cammini,
        quindi \(F(t, 1) = (\alpha \ast i(\alpha))(b)\).
        Per verificare che gli estremi fissi basta verificare
        \(F(0, s) = a\) e \(F(1, s) = b\) per ogni \(s\).
    \end{enumerate}
}

\sdefinition{Gruppo fondamentale}{
    Sia \(X\) uno spazio topologico e \(a \in X\).
    L'insieme delle classi di omotopia a estremi fissi
    \[
...
\begin{align*}
    ...
    \gamma_{\#}([\alpha]) \cdot \gamma_{\#}([\beta]) &= [i(\gamma) \ast \alpha \ast \gamma] \\
    &= [i(\gamma) \ast \alpha \ast \gamma \ast i(\alpha) \ast \beta \ast \gamma] \\
    &= [i(\gamma) \ast \alpha \ast \beta \ast \gamma]
\end{align*}
...
\sproof{}{
    ...
    \[
        Y_n = \{y\in Y \suchthat \exists U \in A, f(B_{2^{-n}}(y)) \subseteq U\}
    \]
    ...
    \]
    in quanto per ogni \(y \in Y\), \(f(y) \in f^{-1}(U)\),
    ma quest'ultimo è un aperto di \(Y\) in quanto \(U\) è aperto e \(f\) è continua,
    e quindi \(f^{-1}(U) \supseteq B_{2^{-n}}(y)\)
    cioè un raggio sufficientemente piccolo per un \(n\) sufficientemente grande.
    ...
    Allora
    \begin{align*}
        d(x,y) &= d(x,z) + d(z,y) < 2 \cdot 2^{-(n+1)} = 2^{-n}
    \end{align*}
    ...
    \]
    Essendo \(Y\) compatto abbiamo che \(Y = \text{int}(Y_{n_k})\)
    dove \(k = \max\{n_1, \cdots, n_m\}\).
}
...
\sproof{}{
    ...
    Consideriamo la funzione \(\text{deg} \colon I \fromto S^1\)
    data da
    \[
        g_r(s) \triangleq \frac{
            f(re^{2\pi i s}) / f(r)
    ...
    dato da \((s,t) \fromto g(s,r) = g_r(s)\).
    Allora \[
    ...
    \begin{align*}
        ...
        & \leq \sum |c_i| \cdot |x^{n-i} \leq \cdots.
    \end{align*}
    ... % 10 dic penultima lezione
    Quindi considerando
    \[
...
\sproof{}{
    ...
    della seiretta che collega l'asse \(x\) con \(f(x)\) su \(S^1\) (che esiste perché i due punti non coincidono).
...
\sproof{}{
    Per ipotesi per ogni un intorno \(U\) di \(x\) in \(X\) tale che
