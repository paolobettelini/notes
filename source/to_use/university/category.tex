...
\begin{snippetdefinition}{Category equivalence}
    TODO
...
\begin{snippetproof}{}
    \begin{iffproof}
        \item Trivial check. (Exercise) Non richiede l'assioma della scelta.
        \item Dobbiamo costruire un funtore \(G\) che sia un inverso.
...
\begin{snippetdefinition}{Slice category}
...
\begin{snippetexercise}{}
    ...
    Quindi prendo le fibre \(\{f^{-1}(i) \suchthat i \in I\}\).
...
\begin{snippetdefinition}{Balanced category}
...
% A natural transformation is the same concept as an homotopy but for two categories.
...
\[
    \text{Eq}(f,q) = \{v \in V \suchthat f(v) = g(v)\}
\]
Quindi 
...
\[
    A \times_D B = \{(a,b) \in A\times B \suchthat h(a) = k(b)\}
\]
...
\[
    R' = R \cup \{(x,x) \suchthat x\in X\}
    \cup \{(x,x') \suchthat (x', x) \in R\}
\]
...
\begin{snippetexample}{Nota da mettere nella trasformazioni naturali}
...
Quando costruisco gli esponenziali dei prefasci,
l'isomorfismo fra gli omomorfismi deve valere per tutti gli
\(Z\). In particolare, scelgo \(Z=y(c)\).
Applichiamo il lemma di Yoneda e
otteniamo quindi la definizione di esponenziale
che siamo obbligati ad adottare. Dobbiamo comunque verificare
che questa cosa funzioni (è condizione necessaria ma non è detto che sia sufficiente).
Dobbiamo mostra che forma un prefascio.
Dobbiamo poi verificare che vi è la corrispondenza biunivoca
per ogni \(R\).

Dopo aver trovato la biiezione
fra \(C \to Q^P\) e
\[
    \text{Hom}_{C/c}(C \times P, C \times Q)
\]
notiamo che i due funtori
fra \(C\) e \(C/c\)
e quello forgetful da \(C/c\) a \(C\) sono in aggiunzione.

\section{Heyting algebras}

Nella definizione, il simbolo \(x \implies y\)
è come dire \(y^x\).
Infatti, la condizione data del se e solo se è precisamente l'aggiunzione dell'esponenziale.

Si dice pseudocomplemento perché
soddisfa che \(x \land \lnot x = 0\)
but not necessarily \(x \lor \lnot x = 1\).

\section{Suboject classifiers}

La freccia di appartenenza è una generalizazione di quella classica \(\in \colon A \times \mathcal{P}(A) \to \{0,1\}\).
L'esponenziale \(\Omega^A\) è come l'insieme delle parti, infatti
\(\mathcal{P}(A) = {\{0,1\}}^A\).

\section{Topos}

"Elementary topos" è un po' un misnomer in quanto
è troppo generale.
Non è veramente collegato alla topologia in quanto posso solamente fare operazioni finitarie.
È quindi una nozione più algebrica.
Ogni topos di Grothendieck è un elementary topos.

La freccia true è un monomorfismo perché il dominio è un oggetto terminale.

Come nel gruppo libero faccio tutte le combinazioni, nel topos faccio tutte le possibilità
di colimiti. È come quando prendo un quoziente in un gruppo o anello e mi serve un sottogruppo normale,
o ideale, e qua quando vogliamo forzare la relazione mi serve la topologia di Grothendieck.
