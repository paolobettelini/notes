...
\begin{snippetproof}{}
    ...
    Prendiamo un punto nell'intersezione e prendiamo le due bolle dentro gli insiemi centrate
    nel punto. Siccome hanno lo stesso centro la loro intersezione è sempre una bolla di raggio
    il minore fra i due.
\end{snippetproof}

La metrica discreta può generare una bolla che è un singoletto.


\begin{snippetproposition}{Distanza troncata}
    Sia \((X,d)\) uno spazio metrico. Definiamo
    \[
        \widetilde d(x,y)
        =
        \frac{d(x,y)}{1+d(x,y)}.
    \]
    Allora \(\widetilde d\) è ancora una distanza su \(X\). Inoltre
    \[
        0\leq \widetilde d(x,y)<1
        \qquad
        \forall x,y\in X.
    \]
\end{snippetproposition}
...
\begin{snippetproof}{}
    La positività, la simmetria e il fatto che
    \[
        \widetilde d(x,y)=0
        \iff
        x=y
    ...
    Resta da verificare la disuguaglianza triangolare. Poniamo
    \[
        a=d(x,y),
        \qquad
        b=d(x,z),
        ...
        c=d(z,y).
    \]
    Per la disuguaglianza triangolare di \(d\), sappiamo che
    \[
        a\leq b+c.
    \]
    ...
    \[
        ...
        \frac{c}{1+c}.
    \]
    Portando tutto a denominatore comune, è sufficiente verificare
    \[
        a(1+b)(1+c)
        \leq
        b(1+a)(1+c)+c(1+b)(1+a).
    ...
    \begin{align*}
        a(1+b+c+bc)
        &\leq
        b(1+a+c+ac)+c(1+b+a+ab).
    \end{align*}
    Cancellando i termini comuni, resta da verificare
    \[
        a\leq b+c+2bc+abc.
    \]
    Ma \(b,c\geq 0\), quindi
    \[
        b+c+2bc+abc\geq b+c.
    \]
    Poiché \(a\leq b+c\), segue la disuguaglianza desiderata.
\end{snippetproof}

\begin{snippetexample}{Limite con funzione segno}
    Un limite che compare spesso in questo contesto è
    \[
        \lim_{x\to 0}\operatorname{sgn}(x)\,|x|^{\frac{5}{4}-\alpha}.
    \]
    Si distingue in base al segno dell'esponente:
    \[
        \lim_{x\to 0}\operatorname{sgn}(x)\,|x|^{\frac{5}{4}-\alpha}
        =
        \begin{cases}
            0, & \frac{5}{4}-\alpha>0,\\[4pt]
            \text{non esiste}, & \frac{5}{4}-\alpha=0,\\[4pt]
            \text{non esiste in } \realnumbers, & \frac{5}{4}-\alpha<0.
        \end{cases}
    \]
    Nel secondo caso il limite destro vale \(1\), mentre il limite sinistro vale \(-1\). Nel terzo caso i due limiti laterali divergono con segno opposto.
\end{snippetexample}
...
Se \(x_0\) è punto di accumulazione per E, in ogni bolla centrata in \(x_0\) cadono infiniti punti.
...
\begin{snippetproof}{per l'interno}
    Dimostriamo che \(E^\circ\) è aperto. Sia \(x_0 \in E^\circ\).
    un punto interno a \(E\), quindi esiste una bolla centrata in tale punto che è contenuta in \(E\).
    Prendiamo un altro punto \(y\) in questa bolla. Possiamo costruire una bolla interna centrata in \(y\)
    ...
    Sia \(x_0 \in A\). \(A\) è unione di bolle quindi esiste un raggio tale che la
...
\begin{snippetdefinition}{Compatto}
    Uno spazio metrico \(X\) è compatto se ogni ricoprimento aperto di \(X\)
...
\begin{snippetproof}{}
    \begin{enumerate}
        ...
        Se \(x\in E\) allora \(r(x) > 0\) perché \(y\notin E\).
...
\begin{snippettheorem}{Teorema dell'intersezione finita}
...
\begin{snippetcorollary}{caso particolare}
    Sia \(\{E_n\}_{n\in \naturalnumbers}\) una famiglia di compatti tale che
    \[
        E_{n+1} \subseteq E_n
    \]
    Allora
...
\begin{snippettheorem}{Teorema di Heine-Borel}
    Sia \(E \subseteq \realnumbers^n\) con la metrica euclidea. Allora \(E\) è compatto se e solo se
    \(E\) è chiuso e limitato.
\end{snippettheorem}

\begin{snippetlemma}{}
    Sia \(\{I_k\}_{k\in \naturalnumbers}\) una famiglia di intervalli
...
\begin{snippetproof}{}
    ...
    In particolare \(a_k \leq b_i\). Consideriamo l'insieme \(E = \{a_k \,|\, k\in \naturalnumbers\}\).
...
\begin{snippetdefinition}{}
    Siano \(a,b \in \realnumbers^n\) con \(a_i < b_i\) per ogni \(i=1,\ldots,n\).
...
\begin{snippetlemma}{}
    Sia \(\{R_k\}_{k\in \naturalnumbers}\) una famiglia di rettangoli chiusi
...
\begin{snippetproof}{}
    Siccome
    \[
        R_k = I_{k,1} \times I_{k,2} \times \cdots \times I_{k,n}
    \]
    possiamo applicare il primo lemma e quindi
    \[
        \exists y_i \in \bigcap_{k\in \naturalnumbers} I_{k,i}
    \]
    Il punto \(y = (y_1, \ldots, y_n)\) è in ogni \(R_k\).
\end{snippetproof}

\begin{snippetlemma}{Lemma 3}
    In \(\realnumbers^n\) con la metrica euclidea
    ogni rettangolo è compatto.
\end{snippetlemma}

\begin{snippetproof}{Lemma 3}
    ...
    di \(R_k\) è dato da \(\frac{1}{2^k} ||b-a||\).
    Per il lemma precedente esiste \(x \in \bigcap_{k\in \naturalnumbers} R_k\).
    ...
    Quindi \(R_k \subseteq G_{\alpha_0}\), assurdo perché \(R_k\) non ammette sottoricoprimento finito.
\end{snippetproof}

\begin{snippetproof}{Heine-Borel}
    ...
    Quindi,
    \[
        E \subseteq [-M, M] \times [-M, M] \times \cdots \times [-M, M] = R
    \]
    \(E\) è un chiuso contenuto in un compatto, quindi è compatto.
\end{snippetproof}

\begin{snippettheorem}{Teorema di Bolzano-Weierstrass}
    Ogni sottoinsieme infinito e limitato di \(\realnumbers^n\) ha almeno un punto di accumulazione.
\end{snippettheorem}

\begin{snippetproof}{Teorema di Bolzano-Weierstrass}
    % todo semplice una riga
\end{snippetproof}

\begin{snippetdefinition}{Insiemi separati}
...
I sottoinsiemi connessi di \(\realnumbers\) sono tutti e soli gli intervalli.
...
\(\realnumbers^n\) con la metrica euclidea è connesso. (Dimostrazione per esercizio non proprio banale).
...
\begin{snippetproof}{}
    Dato che \(y\in E'\), \(\forall m \in \naturalnumbers\), esiste \(x_m\) tale che
...
\begin{snippetproposition}{}
    ...
    Una condizione necessaria per la convergenza è che ogni sottosuccessione converga allo stesso limite.
    La condizione sufficiente è che ogni sottosuccessione ammetta una sottosuccessione
    che converge allo stesso limite.
\end{snippetproposition}

\begin{snippetdefinition}{Compattezza sequenziale}
    Un sottoinsieme \(E\) di uno spazio metrico \(X\) è sequenzialmente compatto se ogni successione
    a valori in \(E\) ammette una sottosuccessione convergente ad un punto di \(E\). 
\end{snippetdefinition}

\begin{snippetproposition}{Equivalenza compattezza}
    \(E\) è compatto se e solo se \(E\) è sequenzialmente compatto.
\end{snippetproposition}
...
\begin{snippetproof}{}
    \begin{iffproof}
        \item Sia \(\{x_n\}\) una successione in \(E\).
        Consideriamo \(F = \{x_n \,|\, n\in \naturalnumbers\}\).
        Se \(F\) è finito, esiste un elemento che compare infinite volte
        ...
        Scegliamo \(x_{m_1}\) tale che \(d(x_{m_1}, y) < 1\).
...
\begin{snippetdefinition}{Spazio completo}
    Uno spazio metrico \((X,d)\) è completo se ogni successione di Cauchy in \(X\)
    converge ad un punto di \(X\).
\end{snippetdefinition}

\begin{snippettheorem}{}
    \(\realnumbers^n\) con la metrica euclidea è completo.
\end{snippettheorem}

\begin{snippetproof}{}
    Sia \(\{x_n\}\) una successione di Cauchy in \(\realnumbers^n\).
    ...
    Quindi \[
        E \triangleq \bigcap_{n\in \naturalnumbers} \overline{E_n} \neq \emptyset
    \]
...
\begin{snippettheorem}{}
    \(E\) sequenzialmente compatto implica \(E\) compatto.
\end{snippettheorem}
...
\begin{snippetproof}{}
    ...
    \begin{enumerate}
        ...
        \item \emph{claim 2:} \(E\) è contenuto nell'unione di un numero finito di bolle di raggio
            \(\delta\) centrate in punti di \(E\).
...
\begin{snippetdefinition}{}
    ...
    \[
        ...
        \land 0 < d_1(x, p) < \delta \implies d_2(f(x), l) < \varepsilon
    \]
...
\begin{snippetproposition}{}
    ...
    Allora \(f(x) \to l\) per \(x\to p\) se e solo se \(f(x_n) \to l\)
...
\begin{snippetproposition}{}
    Sia \(f \colon E \subseteq X \to \realnumbers^n\) per \(n > 1\).
    Allora
    \[
...
\begin{snippetproof}{Sketch}
    Consideriamo la norma per tutte le \(i\)
    \begin{align*}
        |f_i(x)-l_i| \leq ||f(x) - l|| \leq \sum_k |f_k(x) - l_k|
    \end{align*}
\end{snippetproof}

\begin{snippetdefinition}{Continuità}
...
Se \(p\) non è isolato, allora è un punto di accumulazione per \(E\).
...
\begin{snippetproposition}{}
    ...
    Allora \(f\) è continua in \(X_1\) se e solo se \(f^{-1}(V)\) è aperto in \(X_1\)
    per tutti i \(V\) aperti in \(X_2\).
\end{snippetproposition}

\begin{snippetproof}{}
    \begin{iffproof}
        \item Sia \(V\) un aperto di \(X_2\). Se \(f^{-1}(V) = \emptyset\)
        in questo caso abbiamo finito. Altrimenti,
        sia \(p \in f^{-1}(V)\), cioè \(f(p) \in V\).
        Essendo \(V\) aperto, riesco a trovare \[ B_\varepsilon(f(p)) \subseteq V \]
        ...
        \item Sia \(p \in X\) e dimostriamo che \(f\) è continua in \(p\).
...
per ogni chiuso \(C \subseteq X_2\). Molto utile.

In generale le funzioni continue non mandano aperti in aperti.
Per esempio \(f \colon \realnumbers \to \realnumbers\)
data da \(x \to x^2\).
Abbiamo che
\[
    f((-1,1)) = [0,1)
...
\begin{snippetdefinition}{Funzione aperta}
    Una funzione viene detta \emph{aperta}
    se \(f(U)\) è aperto in \(X_2\) per tutti gli insiemi \(U\) aperto in \(X_1\).
    Analogamente funzione chiusa.
\end{snippetdefinition}

Sia \(f \colon (X, d) \to \realnumbers^n\) con \(n>1\)
...
\begin{snippettheorem}{Teorema di Weierstrass}
    ...
    \(f \colon X \to \realnumbers\) una funzione continua.
...
\begin{snippetproof}{Teorema di Weierstrass}
    \(f(X)\) è compatto in \(\realnumbers\), quindi è chiuso e limitato.
...
\begin{snippettheorem}{Teorema da compatto ad Hausdorff}
...
\begin{snippetproof}{Teorema da compatto ad Hausdorff}
...
\begin{snippetproof}{}
    Dobbiamo mostrare che \((f^{-1})^{-1}(C)\) è chiuso per ogni \(C\) chiuso di \(X_1\).
    Ma questo coincide con \(f(C)\) che è chiusa per il teorema da compatto ad Hausdorff.
\end{snippetproof}
...
\begin{snippetproof}{}
    ...
    Sicuramente \(F,G \neq \emptyset\) e \(E = F\cup G\). Vogliamo mostrare che \(F\) e \(G\)
    sono separati.
    Siccome \(A \subseteq \overline{A}\) vale anche \(f^{-1}(A) \subseteq f^{-1}(\overline{A})\).
    ...
    e \(\overline{A} \cap B = \emptyset\), e quindi \(\overline{F} \cap G = \emptyset\)
    perché altrimenti vi sarebbe un elemento sia in \(\overline{A}\) che in \(B\).
    Dovrebbe essere \(f(x) \in \overline{A}\) e \(f(x) \in B\) lightning.
...
\begin{snippettheorem}{Teorema di Heine-Cantor}
...
\begin{snippetdefinition}{Funzione di Lipschitz}
    Siano \((X_1, d_1),(X_2, d_2)\) spazi metrici,
    \(f\colon X_1 \to X_2\).
...
\begin{snippetexample}{}
    Consideriamo \(f \colon \realnumbers \to \realnumbers\)
    data da \(f(x) = x^2\). Non è lipschitziana in quanto non è uniformemente continua.
...
\begin{snippetproposition}{}
    Sia \(f\colon I \subseteq \realnumbers \to \realnumbers\) differenziabile.
    Allora, \(f \in \text{Lip}_\alpha(I, \realnumbers)\)
...
\begin{snippetproof}{}
    \begin{iffproof}
        ...
        \item Per il teorema di Lagrange esiste \(\theta \in (\min\{x,y\}, \max\{x,y\})\)
        \begin{align*}
            f(x) - f(y) &= f'(\theta) (x-y) \\
...
\begin{snippetexample}{}
    Sia \(v \in \realnumbers^n\) e consideriamo
    \(f\colon \realnumbers^n \to \realnumbers\) data da
    \(f(x) = v \cdot x\).
    Dobbiamo studiare \(|f(x) - f(y) = |v\cdot x - v \cdot y|\)
    usando la bilinearità del prodotto scalare ottengo
    \(|v\cdot (x-y)|\). Per Cauchy-Schwarz
    \[
...
\begin{snippettheorem}{Teorema di Banach-Cacciopoli o delle contrazioni}
...
\begin{snippetproof}{}
    ...
    \begin{enumerate}
        ...
        \begin{align*}
            d(x_{n+1}, x_n) &= d(f(x_n), f(x_{n-1})) \leq \alpha d(x_n, x_{n-1}) \\
        ...
        \begin{align*}
            d(x_{n+k}, x_n) &\leq \sum_{i=0}^{k-1} d(x_{n+k-i}, x_{n+k-i-1}) \\
            &\leq d(x_1, x_0) \sum_{i=0}^{k-1} \alpha^{n+k-i-1} \\
            &= \alpha^n d(x_1, x_0) \sum_{i=0}^{k-1} \alpha^i \\
            &= \alpha^n \frac{1 - \alpha^k}{1 - \alpha} d(x_1, x_0) \\
...
\begin{snippetcorollary}{Indebolimento del teorema delle contrazioni: teorema delle iterate contrazioni}
    Sia \((X, d)\) uno spazio metrico completo e sia
    \(f \colon X \to X\) un'applicazione tale che \(\exists n \in \naturalnumbers\)
...
\begin{snippetproof}{}
    ...
    Sia \(x\) un punto fisso di \(f\). Allora \(f^n(x) = x\).
    ...
    \begin{align*}
        ...
        f^n(f(x)) &= f(x)
    \end{align*}
    quindi \(f(x)\) è un punto fisso di \(f^n\), ma siccome \(f^n\) è una contrazione
...
\begin{snippettheorem}{di dipendenza del punto fisso del parametro}
    ...
    Allora \(\varphi\) è continua in \(y_0\).
\end{snippettheorem}

\begin{snippetproof}{}
    Vogliamo mostrare che \(d(\varphi(y), \varphi(y_0)) \to 0\) se \(y \to y_0\).
    Vogliamo stimare \(d(\varphi(y), \varphi(y_0))\) con \(\varphi(y) = T(\varphi(y), y)\).
    Sia \(x = \varphi(y)\) e \(\varphi(y_0) = x_0\). Allora
    \[
        d(\varphi(y), \varphi(y_0)) = d(T(x,y), T(x_0, y_0))
    \]
    Usando la disuguaglianza triangolare
    \begin{align*}
        d(\varphi(y), \varphi(y_0))
        &= d(T(\varphi(y), y), T(\varphi(y_0), y_0)) \\
        &\leq d(T(\varphi(y), y), T(\varphi(y_0), y)) +
        d(T(\varphi(y_0), y), T(\varphi(y_0), y_0)) \\
        &\leq \alpha d(\varphi(y), \varphi(y_0))
        + d(T(\varphi(y_0), y), T(\varphi(y_0), y_0)) \\
        (1-\alpha) d(\varphi(y), \varphi(y_0))
        &\leq d(T(\varphi(y_0), y), T(\varphi(y_0), y_0)) \to 0
    \end{align*}
    La costante è positiva e indipendente da \(y\).
    La funzione \(T(\varphi(y_0), -)\) è continua in \(y\).
\end{snippetproof}

\begin{snippetlemma}{Sugli spazi normati}
    \[
        \left|
            ||y|| - ||x||
        \right|
...
\begin{snippetdefinition}{Spazio di Banach}
    Uno \emph{spazio di Banach} è uno spazio normato
    completo rispetto alla norma.
\end{snippetdefinition}

\begin{snippetdefinition}{Equivalenza di norme}
...
\begin{snippettheorem}{Equivalenza di norme reali}
    Tutte le norme in \(\realnumbers^n\) sono equivalenti.
\end{snippettheorem}
...
\begin{snippetproof}{}
    ...
    \[
        \alpha||x||_2 \leq ||x|| \leq \beta||x||_2, \quad \forall x \in \realnumbers^n
    \]
    ...
    Una volta fatto questo, possiamo ottenere che la definizione
    di \(\alpha\) dice che \(||x||_2 = 1 \implies ||x|| \geq \alpha\).
    Voglio dimostrare che \(||x|| \geq \alpha ||x||_2\) per tutte le \(x \in \realnumbers^n\).
    Se \(x=0\) la disuguaglianza è soddisfatta. Altrimenti, normalizziamo \(z = x / ||x||_2\).
...
Lo spazio \(\mathcal{L}(\realnumbers^n, \realnumbers^m) \cong \text{Mat}_{m\times n}(\realnumbers)\).

Studiamo la continuità degli operatori lineari in spazi normati.

\begin{snippetproposition}{}
    Ogni \(A \in \mathcal{L}(\realnumbers^n, \realnumbers^m)\)
    è continua rispetto alla topologia indotta dalla norma (qualsiasi visto che sono equivalenti in \(\realnumbers^n\)).
\end{snippetproposition}

\begin{snippetproof}{}
    \begin{enumerate}
        \item Mostriamo che la continuità dell'operatore in un singolo punto, come l'origine,
        implica la continuità di \(A\) in tutto \(\realnumbers^n\). Questo è un fatto generale.
        ...
        Inoltre, per la continuità uniforme possiamo mostrare che \(\forall \varepsilon > 0, \exists \delta > 0\)
        tale che
        \[
        ...
        Usiamo il fatto che \(\realnumbers^n\) ha dimensione finita.
...
\begin{snippetexample}{Controesempio}
    Consideriamo lo spazio delle funzioni \(f\colon \realnumbers \to \realnumbers\)
    limitate, in \(\mathcal{C}^1\) e con derivata limitata \(\mathcal{BC}^1(\realnumbers)\).
    Come secondo spazio prendiamo delle funzioni continue e limitate \(f\colon \realnumbers \to \realnumbers\),
    \(\mathcal{BC}(\realnumbers)\).
    Consideriamo quindi l'operatore della derivata \(\mathcal{BC}^1(\realnumbers) \to \mathcal{BC}(\realnumbers)\).
    ...
    Per farlo prendiamo una funzione oscillante che si schiaccia sull'asse delle ascisse, e quindi la sua derivata
    ...
    Abbiamo la norma
    \begin{align*}
        ||f||_\infty &= \sup_{\realnumbers} \frac{1}{n}|\sin(nx)|
        = \frac{1}{n}
    \end{align*}
    Mentre la norma della derivata
        \begin{align*}
        ||f'||_\infty &= \sup_{\realnumbers} |\cos(nx)| = 1
    \end{align*}
\end{snippetexample}

Andiamo a definire una norma speciale su questo spazio, la norma operatoriale.

\begin{snippetdefinition}{Norma operatoriale}
    \[
        ||A||_* = \sup_{||x|| \leq 1} ||Ax||
    \]
\end{snippetdefinition}
...
\begin{snippetproposition}{Stima fondamentale}
    Per ogni \(x\in \realnumbers^n\) vale
    \[
        ||Ax||_* \leq ||A|| \cdot ||x||
    \]
\end{snippetproposition}
...
\begin{snippetproof}{}
    ...
    \begin{align*}
        ...
        = \frac{1}{||x||} ||Ax||
    \end{align*}
    moltiplicando entrambi i membri per \(||x||\) si ottiene la tesi.
\end{snippetproof}

In realtà questa costante è la migliore.

\begin{snippetproposition}{}
    Se \(\exists \alpha \in \realnumbers\)
    tale che \(||Ax|| \leq \alpha ||x||\) per tutte le \(x \in \realnumbers^n\),
...
\begin{snippetproof}{La norma operatoriale è una norma}
    \begin{enumerate}
        ...
        \]
        e quindi anche per tutte le altre \(x\) visto che possiamo normalizzare. Quindi, \(Ax = 0\).
        ...
        \begin{align*}
            ...
            \right)
        \end{align*}
        Per la proposizione precedente, \(||A+B||_*\) è la più piccola costante
...
\begin{snippettheorem}{}
    \((\mathcal{L}(\realnumbers^n, \realnumbers^m), ||\cdot||)\) è uno spazio di Banach.
\end{snippettheorem}

\begin{snippetproof}{}
    Siccome questo spazio è finito dimensionale è isomorfo allo spazio \(\realnumbers^{m \times n}\)
    che è completo per esempio rispetto alla norma seconda.
    Tuttavia, dimostramolo con le successioni di Cauchy. Consideriamo quindi una successione di operatori lineari.
    Per ogni \(\varepsilon > 0\) esiste \(N_\varepsilon\) tale che \(\forall m,n > N_\varepsilon\)
    \[
        ||A_m - A_n|| < \varepsilon
    \]
    cioè per tutte le \(x\)
    \[
        ||(A_m - A_n)x|| \leq \varepsilon||x||
    \]
    Questo vuole dire che per \(x\) fissato la successione \(\{A_n x\}\) è una successione di Cauchy
    in \(\realnumbers^m\). Siccome \(\realnumbers^m\) è completo, la successione converge.
    ...
    \]
    cioè abbiamo trovato che \(||A_n - A||_* \leq \varepsilon\).
\end{snippetproof}

Notiamo che abbiamo sfruttato solo la completezza di \(\realnumbers^n\).

\begin{snippetexercise}{}
    Sia \(A \colon \realnumbers^2 \to \realnumbers^3\)
    dato da
    \[
        A = \begin{pmatrix}
            3 & -2 \\ 0 & 1 \\ 1 & 3
...
\begin{snippettheorem}{}
    \(\text{GL}_n(\realnumbers)\) è un aperto 
    in \(\mathcal{L}(\realnumbers^n, \realnumbers^n)\)
...
\begin{snippetproof}{}
    ...
    \[
        \text{GL}_n(\realnumbers) = \text{det}^{-1}(\realnumbers \backslash \{0\})
    \]
    ma visto che è la preimmagine di un aperto tramite il determinante, che è continuo.

    Dimostriamo tuttavia la proposizione con un argomento che può funzionare anche
    nel caso infinito dimensionale.
    \begin{enumerate}
        \item \emph{identità è un punto interno dell'insieme:} mostriamo che esiste \(\delta\)
        tale che \(||A-\mathbf{1}|| < \delta\) implica che \(A \in \text{GL}(\realnumbers^n)\).
        Stiamo usando la norma operatoriale.
        Poniamo \(B = \mathbf{1} - A\) e quindi \(A^{-1} = {(\mathbf{1} - B)}^{-1}\)
        ...
        Usando il criterio di Cauchy, troviamo che la serie converge se e solo se \(\forall \varepsilon > 0, \exists N_\varepsilon \in \naturalnumbers\)
        tale che \(\forall p \in \naturalnumbers\) \begin{align*}
            \left|\left|
                \sum_{k=m}^{m+p} B^k
        ...
        \item Sia \(A_0 \in \text{GL}_n(\realnumbers)\).
        Vogliamo mostrare che \(\exists \delta > 0\) tale che
        \[
            ||A-A_0||_* < \delta \implies A \in \text{GL}_n(\realnumbers)
        \]
        Calcoliamo
        \begin{align*}
            A = A_0 - (A_0 - A) = A_0 (\mathbf{1} - A_0^{-1}(A_0 - A))
            = A_0 (\mathbf{1} - B), \quad B = A_0^{-1}(A_0 - A)
        \end{align*}
        per il punto precedente \(\mathbf{1} - B\) è invertibile purché \(||B||_* < 1\).
        Studiamo quando ciò avviene
        \begin{align*}
            ||B||_* = ||A_0^{-1}(A_0 - A)||_* \leq ||A_0^{-1}||_* \cdot ||A_0 - A||_* < 1
            \text{ se } ||A_0 - A||_* < \frac{1}{||A_0^{-1}||_*}
        \end{align*}
        la divisione la possiamo fare in quanto la funzione nulla, che ha norma nulla, non è invertibile quindi non siamo
        in quel caso.
        Quindi, scegliendo \(\delta \leq 1 / ||A_0^{-1}||_*\)
        si ha che \(A\) è invertibile, quindi è un aperto.
        Cioè in questo caso \(||B||_* < 1\) quindi \(\mathbf{1} - B\) è invertibile
        e visto che abbiamo scritto \(A\) come prodotto di operatori invertibili, è anch'esso invertibile.
    \end{enumerate}
...
\begin{radioactive}
\begin{snippetdefinition}{Limite multivariabile}
    Sia \(f \colon \Omega \subseteq \realnumbers^n \to \realnumbers^m\)
    e sia \(a\) un punto di accumulazione per \(\Omega\).
    Allora
    \[
...
\begin{radioactive}
\begin{snippetdefinition}{Limite a infinito multivariabile}
    Sia \(f \colon \Omega \subseteq \realnumbers^n \to \realnumbers^m\)
...
\begin{snippetdefinition}{Limite infinito multivariabile}
    Sia \(f \colon \Omega \subseteq \realnumbers^n \to \realnumbers^m\)
...
\begin{radioactive}
\begin{snippetdefinition}{Cammino}
    Un \emph{cammino} è un applicazione \(\varphi: (0,1) \to \realnumbers^n\) continua.
\end{snippetdefinition}
...
\begin{radioactive}
\begin{snippetdefinition}{Derivata direzionale}
    TODO 15 ottobre
    \[
...
nella direzione data dal vettore direzionale, e calcolare la derivata ad una singola variabile.
...
\begin{snippetexample}{}
    ...
    \[
        f(y^2, y) = \frac{1}{2}
    \]
    Quindi abbiamo trovato un cammino lungo il quale la funzione non tende a zero.
...
\begin{radioactive}
\begin{snippetdefinition}{}
    Sia \(f \colon \Omega \subseteq \realnumbers^n \to \realnumbers\)
    con \(\Omega\) aperto e \(a \in \Omega\).
    Diremo che \(f\) è differenziabile in \(a\)
    se esiste una forma lineare (funzionale lineare),
    o equivalentemente se esiste \(\lambda \in \realnumbers^n\) tale che
    \[
...
\begin{snippettheorem}{}
    ...
    \begin{enumerate}
        ...
        \item \(\forall v \in \realnumbers^n\), esiste la derivata direzionale
...
\begin{snippetproof}{}
    \begin{enumerate}
        \item Per ipotesi esiste \(\lambda \in \realnumbers^n\)
        tale che
        \[
...
\begin{radioactive}
Per stabilire se una funzione \(f\) è differenziabile in \(a\),
basta controllare se le derivate parziali esistono.

\begin{snippetcorollary}{Formula del gradiente}
...
\begin{radioactive}
\begin{snippettheorem}{Teorema del differenziale totale}
    Sia \(f \colon \Omega \to \realnumbers\), con \(\Omega \subseteq \realnumbers^n\) aperto,
...
\begin{snippetproof}{}
    ...
    \[
        \lim_{(x,y) \to (x_0,y_0)} \frac{
            f(x,y) - f(x_0, y_0) - \gradient f(x_0,y_0) \cdot (x-x_0, y-y_0)
        }{\sqrt{{(x-x_0)}^2 + {(y-y_0)}^2}} = 0
    \]
    Calcoliamo l'incremento \(f(x,y) - f(x_0, y_0)\). Piuttosto che congiungere
    i due punti mediante un segmento, facciamo lo stesso percorso con due segmenti nelle direzioni degli assi,
    ...
    \end{align*}
    I due termini con le radici a denominatore sono minori di 1.
    Per il teorema del confronto, se \((x,y) \to (x_0, y_0)\)
    si ha che \(\xi \to y_0\) e \(\eta \to x_0\).
    Per la continuità delle derivate parziali segue la tesi che il valore tende a zero.
\end{snippetproof}
\end{radioactive}
...
\begin{snippetexercise}{}
    ...
    Il dominio massimale è \(\{(x,y) \,|\, xy > -1\}\).
...
\begin{snippetexercise}{}
    Studiare continuità e differenziabilità nell'origine di \[
        ...
        \end{cases}
    \]
    Non conviene usare il teorema del differenziale totale perché la funzione è a tratti
...
\begin{radioactive}
La forma differenziabile \(\text{d}f(-) \colon \Omega \to \mathcal{L}(\realnumbers^n, \realnumbers)\)
e quindi associa ad ogni punto il funzionale lineare differenziale.
Quindi
\[
    \text{d}f(a) \colon \realnumbers^n \to \realnumbers
\]
...
\]
la cui applicazione può essere scritta con le proiezioni
\[
...
\begin{align*}
    ...
    \right]
\end{align*}
dove \(\theta\) è l'angolo fra \(h\) e \(\gradient f(a)\).
...
\begin{radioactive}
Consideriamo \(f\colon \Omega \subseteq \realnumbers^n \to \realnumbers^m\) con \(\Omega\)
aperto. Abbiamo quindi
\[
    f = \sum_{i=1}^n f_i e_i, \quad f_i \colon \Omega \fromto \realnumbers
\]
Sia \(a\in \Omega\) e sia \(v \in \realnumbers^n\). Definiamo la derivata direzionale
\[
...
\begin{radioactive}
\begin{snippetdefinition}{Differenziabilità}
    Una funzione \(f\colon \Omega \subseteq \realnumbers^n \to \realnumbers^m\) con \(\Omega\)
    aperto è differenziabile in \(a\in \Omega\) se \(\exists L \in \mathcal{L}(\realnumbers^m, \realnumbers^n)\)
    tale che
    \[
...
in \(a\) se esiste una matrice \(\Lambda \in M_{m\times n}(\realnumbers)\)
...
\begin{snippettheorem}{}
    ...
    in \(a\), \(\forall v \in \realnumbers^n\) tale che \(||v|| = 1\),
    \[
...
\begin{radioactive}
\begin{snippettheorem}{Chain rule}
    Siano \(f \colon \realnumbers^n \to \realnumbers^m\)
    e \(g \colon \realnumbers^m \to \realnumbers^p\) funzioni
...
\begin{radioactive}
Sia \(h = g\circ f \colon \realnumbers^m \to \realnumbers^n\).
...
\begin{snippetexample}{}
    Siano \(f\colon \realnumbers^2 \to \realnumbers^3\) e 
    \(g\colon \realnumbers^3 \to \realnumbers^2\). Chiamiamo \(x,y\) le coordinate di \(f\)
...
\begin{radioactive}
\begin{snippetexample}{}
    Siano \(f\colon \Omega \subseteq \realnumbers^2 \to \realnumbers^2\) e 
    \(g\colon \realnumbers^2 \to \realnumbers\). Chiamiamo le variabili di \(f\)
    \((\rho, \theta)\) e di \(g\) diciamo \((x,y)\).
...
\begin{snippetexercise}{}
    ...
    \[
        ...
        \end{cases}
    \]
    al variare di \(\alpha\). Il dominio è tutto \(\realnumbers^2\).
    ...
    \begin{align*}
        ...
        &= \rho^{\frac{1}{4} - 2 \alpha} \ln(1 + \rho^2) \leq \rho^{\frac{1}{4} - 2 \alpha + 2}
    \end{align*}
    ...
    Non conviene usare il teorema del differenziale totale quindi calcoliamo le derivate parziali.
    Chiaramente
    \[
        \frac{\partial f}{\partial y}(0,0) = 0
    \]
    ...
    \end{align*}
    che tende a zero per \(\alpha < \frac58\), altrimenti il limite non esiste.
    ...
    \begin{align*}
        ...
        &= \frac{{|h|}^{1/4} \ln(1 + h^2 + k^2)}{{(h^2 + k^2)}^{\alpha + 1/2}}
    \end{align*}
...
\begin{snippettheorem}{Teorema di Lagrange}
    Sia \(\Omega \subseteq \realnumbers^n\) open and \([a,b]\subseteq \Omega\).
    Let \(f\colon \Omega \to \realnumbers\) differentiable in \(\Omega\).
...
\begin{snippetproof}{}
    ...
    \[
        \varphi(t) = a+t(b-a)
    \]
    che è differenziabile. Consideriamo la funzione \(F = f \circ \varphi \colon [0,1] \to \realnumbers\).
...
\begin{radioactive}
\begin{snippettheorem}{Teorema dell'incremento finito}
    Sia \(\Omega \subseteq \realnumbers^n\) aperto e sia \(f\colon \Omega \to \realnumbers^m\)
...
\begin{radioactive}
\begin{snippetproof}{Teorema dell'incremento finito}
    Consideriamo \(n=1\). Il teorema
    dice che data \(g\colon [a,b] \to \realnumbers^m\) continua su \([a,b]\)
    ...
    che ha forma \([a,b] \to \realnumbers\). \(\varphi\) è continua su \([a,b]\) e differenziabile su
    \((a,b)\). Per il teorema di Lagrange \(\exists \theta \in (a,b)\) tale che
    ...
    che ha forma \([0,1] \to \realnumbers^m\) continua in \([0,1]\) e differenziabile in \((0,1)\).
...
\begin{radioactive}
\begin{snippetcorollary}{}
    Let \(f\colon \Omega \to \realnumbers^m\), \(\Omega \subseteq \realnumbers^n\) aperto connesso,
...
\begin{snippetproof}{}
    \begin{iffproof}
        \item Ovvia
        \item Sia \(x_0\in \Omega\) e poniamo \(E_{x_0} = \{x \in \Omega \,|\, f(x) = f(x_0) \}\).
        Vogliamo mostrare che \(E_{x_0} = \Omega\).
        Basta mostrare che \(E_{x_0} \neq \emptyset\) in quanto \(x_0 \in E_{x_0}\)
        e che \(E_{x_0}\) è aperto e chiuso in \(\realnumbers^m\).
        ...
        \([x,y] \subseteq B_r(v)\). Per il teorema dell'incremento finito 
        \[
...
Sia \(\Omega \subseteq \realnumbers^m\) e \(f \colon \Omega \to \realnumbers^m\)
differenziabile.

\begin{snippetproposition}{}
    Sia \(f \in \text{Lip}_\alpha(\Omega, \realnumbers^m)\).
    Allora \[
        ||df(x)|| \leq \alpha
    \]
...
dobbiamo usare il teorema dell'incremento finito.
Dobbiamo richiedere che tutti i segmenti siano in \(\Omega\),
quindi ci serve \(\Omega\) convesso.

\begin{snippetproposition}{}
    Sia \(\Omega \realnumbers^n\) aperto, \(f \colon \omega \to \realnumbers^m\)
...
\begin{radioactive}
\begin{snippetexample}{}
    Sia \(f(t) = (\cos t, \sin t, \arctan t)\) da \(\realnumbers\) a \(\realnumbers^3\).
    ...
    \end{align*}
    Il massimo è chiaramente \(\sqrt{2}\) che è la miglior costante di Lipschitz.
    ...
    \end{align*}
    Ci serve il supremum su \(t \in \realnumbers\)
...
\begin{radioactive}
Prendiamo \(\Omega \subseteq \realnumbers^n\) aperto
e consideriamo \(f \colon \Omega \to \realnumbers\).
...
Fissiamo un altro versore \(w \in \realnumbers^m\) e costruiamo la derivata direzionale
...
\begin{radioactive}
\begin{snippetdefinition}{Matrice Hessiana}
...
\begin{snippetexample}{}
    ...
    \]
    perché \(f(0,y) = 0\).
    \(f\) è differenziabile nell'origine (ma non possiamo mostrare con il teorema del differenziale totale).
    Dobbiamo calcolare
    \[
        \lim_{(x,y) \to (0,0)} \frac{f(x,y)}{\sqrt{x^2 + y^2}} = 0
    \]
    che si risolve in coordinate polari.
    Non possiamo calcolare \(f_{xy}(0)\) perché dovrei calcolare
    \[
...
\begin{snippetexample}{}
    ...
    \[
        ...
        \end{cases}
    \]
    Abbiamo che \(f\) è continua in \(0,0\)
...
\begin{snippettheorem}{Teorema di Schwarz}
    Sia \(f \colon \Omega \subseteq \realnumbers^n \to \realnumbers\),
    \(\Omega\) aperto, \(a \in \Omega\) e \(v,w\) due versori in \(\realnumbers^m\).
...
\begin{snippetdefinition}{Classe \(\mathcal{C}^2\)}
    Sia \(f \colon \Omega \subseteq \realnumbers^n \to \realnumbers\),
...
\(df(x) \colon \realnumbers^n \to \realnumbers^m\) dato da
...
\begin{snippetdefinition}{}
    ...
    \(f\) è differenziabile in \(\Omega\) e \(df \colon \Omega \to \mathcal{L}(\realnumbers^n, \realnumbers^m)\)
...
\begin{radioactive}
\begin{snippetdefinition}{}
    Se \(f \colon \Omega \to \realnumbers^m\) è due volte differenziabile in \(a\),
    allora \(d^2 f(a)\) indica il differenziale del differenziale di \(f\) nel punto \(a\).
\end{snippetdefinition}

Quindi
\[
    d^2f(a) \in \mathcal{L}(\realnumbers^n, \mathcal{L}(\realnumbers^n, \realnumbers^m))
\]
Questo significa che \(d^2f(a) h \in \mathcal{L}(\realnumbers^n, \realnumbers^m)\).
Cioè è lineare sia in \(h\) che in \(k\), quindi bilineare
\(k \to (d^2f(a)h)k\).
Siccome è bilineare possiamo vederlo come \(d^2f(a) \colon \realnumbers^n \times \realnumbers^n \to \realnumbers^m\)
...
\begin{radioactive}
Se per \(\{v^{(i)}\}\) versori in \(\realnumbers^m\)
esistono in \(B_\delta(a)\) le derivate direzionali
\[
...
\begin{snippetexample}{}
    Con \(k=3\) sia \(f \colon \realnumbers^2 \to \realnumbers\).
...
\subsection{Differenziabili \(k\)-esimi}

\begin{radioactive}
\begin{snippetdefinition}{Differenziabile \(k\)-esima}
    \[
        d^k f(a) \colon \realnumbers^n \times \realnumbers^n \times \cdots \times \realnumbers^n \to \realnumbers^n
    \]
...
\begin{radioactive}
\begin{snippetexample}{}
    Sia \(f \colon \realnumbers^3 \to \realnumbers\)
    e \(k=4\). Allora
    \begin{align*}
        ...
        \right) + \cdots
    \end{align*}
    Il termine nelle parentesi è pari a \(4 f_{xxxy}\).
    E il valore
    \[
...
\begin{radioactive}
\begin{snippettheorem}{Teorema di Taylor}
    Sia \(\Omega \subseteq \realnumbers^n\) aperto,
    siano \(a,x \in \Omega\) tale che \([a,x] \subseteq \Omega\).
    Sia \(f \colon \Omega \to \realnumbers\) tale che \(f \in \mathcal{C}^k(\Omega)\).
    Allora,
    \[
...
\begin{radioactive}
Sia \(\Omega \subseteq \realnumbers^n\) un sottoinsieme qualunque e sia \(f \colon \Omega \to \realnumbers\).
...
\begin{snippettheorem}{Teorema di Fermat}
    Sia \(\Omega \subseteq \realnumbers^n\)
    aperto, \(a \in \Omega\) un punto di estremo per \(f\).
    Se \(f\) è differenziabile in \(a\),
    allora
    \[
...
\begin{snippetproof}{Teorema di Fermat}
    Supponiamo che \(a\) sia un punto di estremo per \(f\).
    Allora \(a\) è di estremo anche per tutte le restrizioni di rette che passano per \(a\).
    In particolare prendiamo \(\gamma_i(t) = a + te_i\)
    paralleli agli assi cartesiani.
    Sia \(\varphi_i\) la restrizione di \(f\) a tali rette.
    Quindi \(\varphi_i = f \circ \gamma_i\).
    Le funzioni \(\varphi_i\) hanno un punto di estremo in \(t=0\).
    Inoltre, \(\varphi_i\)
    sono differenziabili in quanto composizione di funzioni differenziabili.
    Allora, per il teorema di Fermat in una variabile vale \(\varphi_i'(0) = 0 \).
    Quindi abbiamo
    \begin{align*}
...
Quindi \(F\colon \realnumbers^n \backslash \{0\} \to \realnumbers\) è omogenea di grado zero \(F(\lambda h) = F(h)\).
...
Ci restringiamo quindi a \[
    S = \{x \in \realnumbers^n \,|\, ||x|| = r\}
\]
e allora \(F\) rispetto a \(S\) cioè \(S \to \realnumbers\)
ammette massimo e minimo assoluti,
in quanto \(F\) è continua e \(S\) è compatto,
quindi per Weierstrass.
Allora esistono \(m, M \in \realnumbers\)
tale che \(m \leq F(h) \leq M\) per tutte le \(h \in \realnumbers^n \backslash \{0\}\)
e \(m, M\) sono
\[
    m \leq \frac{h^t \hessian f(g)h}{||h||^2} \leq M
\]
per tutte le \(h \in \realnumbers^n \backslash \{0\}\)
e quindi
\[
    m||h||^2 \leq h^t\hessian f(a)h \leq M||h||^2
\]
per tutte le \(h \in \realnumbers^n\).
...
In generale, se \(Q \colon \realnumbers^n \to \realnumbers\)
...
\begin{snippetexample}{}
    ...
    dell'origine contiene punti in cui \(f(x,y)>0\) e punti in cui \(f(x,y)<0\).
...
Adottando una approccio locale, sotto opportune supposizioni,
l'equazione \(F(x,y) = 0\) definisce implicitamente
localmente un'unica funzione di variabile y oppure x.
\end{radioactive}
...
\begin{radioactive}
\begin{snippettheorem}{Teorema della funzione implicita o teorema del Dini}
...
\begin{snippetproof}{}
    ...
    (supponiamo senza perdita di generalità che sia non-negativo)
    ...
    Consideriamo i valori che la funzione assume nella retta verticale \(x=x_0\).
    ...
    e consideriamo la restrizione \(F(x, -)\), ossia le rette verticali.
    \(F(x, y_0 - \sigma) < 0\)
    ...
    Da questa sottosuccessione vogliamo estrarre una sottosuccessione che converge a
    \(\varphi(x)\).
    Notiamo che \(\varphi(x_{n_k}) \in \overline{B}_\sigma(y_0)\)
    che è compatto.
    Siccome è compatto posso estrarre una sottosuccessione convergente.
    \(\exists n_{k_j}\) tale che \(\varphi(x_{n_{k_j}}) \to y\).
...
\begin{snippetexample}{}
    ...
    cresce in \((0, 2]\) e poi decresce di nuovo.
    Valutiamo se c'è un asintoto obliquo
    a destra. Dobbiamo valutare il limite di una funzione che non possediamo
...
\begin{snippetexercise}{}
    ...
    Notiamo che il problema è ben posto in quanto \(F(0, \frac12) = 0\).
...
\begin{snippetexercise}{}
    ...
    \(F_z = x^2e^z + 1 > 0\).
    In particolare, \(F_z(0,0,0) \neq 0\) e quindi possiamo applicare il teorema della funzione implicita.
    ...
    \]
    bisogna quindi calcolare il limite di \(F(x,y, -c)\),
...
\begin{radioactive}
\begin{snippettheorem}{Teorema della funzione implicita}
    Sia \(\Omega \subseteq \realnumbers^n \times \realnumbers^m\) aperto,
    ...
    Sia \(F(x_0, y_0) = 0\), \(F_y(x_0, y_0) \in \text{GL}(\realnumbers^m)\).
...
\begin{radioactive}
è un discorso locale in quanto dobbiamo trovare i raggi sufficientemente piccoli.
...
\begin{snippetproof}{}
    ...
    Vogliamo dimostrare che \(T\) porta uno spazio metrico in sé e che è una contrazione
    ...
    \begin{enumerate}
        ...
        \item Mostriamo che esistono \(r,R >0\)
            ...
            Scegliamo \(R\) come al punto precedente.
    ...
    inoltre il punto fisso dipende con continuità da \(x\) e quindi la funzione implicita è continua.
\end{snippetproof}
\end{radioactive}
...
\begin{snippetproof}{}
    ...
    gli zeri della mappa \(F(x,y)\) relativi alla stessa \(x\)
...
\begin{snippettheorem}{}
    ...
    per tutte le \(x \in U(x)\). Sia \(F\) differenziabile in \((x_0, y_0)\),
...
\begin{snippetproof}{}
    ...
    \]
    per \(h \to 0\).
    Possiamo sostituire \(\varphi(x_0) = y_0\).
    Poniamo
    \begin{align*}
        ...
        \right]
    \end{align*}
    ma la parentesi quadra è pari al differenziabile applicato a qualcosa
    \[
        F_y(x_0, y_0)(\varphi(x_0 + h) - y_0) + F_x(x_0, y_0)h
...
\subsection{Esempio: studio di una curva definita implicitamente}

Consideriamo, al variare di \(c\in\realnumbers\), la funzione
\[
    f(x,y)=(1-x+y)e^x+e^y-c.
\]
Vogliamo studiare la curva
\[
    f(x,y)=0.
\]
...
\[
    ...
    e^x+e^y>0
    \qquad
    \forall (x,y)\in\realnumbers^2.
\]
Quindi, per ogni \(x\), la funzione \(y\mapsto f(x,y)\) è strettamente crescente. Inoltre
\[
    \lim_{y\to-\infty}f(x,y)=-\infty,
    \qquad
    \lim_{y\to+\infty}f(x,y)=+\infty.
\]
Ne segue che, per ogni \(x\in\realnumbers\), esiste un unico \(y=\varphi(x)\) tale che
\[
    f(x,\varphi(x))=0.
\]
Per il teorema della funzione implicita, \(\varphi\) è di classe \(C^1\).
...
\[
    ...
    =
    \frac{e^x(x-\varphi(x))}{e^x+e^{\varphi(x)}}.
\]
Usando l'equazione
\[
    (1-x+\varphi(x))e^x+e^{\varphi(x)}-c=0,
\]
si ricava anche
\[
    (\varphi(x)-x)e^x
...
\[
    \varphi'(x)
    =
    1-\frac{c}{e^x+e^{\varphi(x)}}.
\]

\subsubsection{Comportamento per \(x\to+\infty\)}

Fissato \(k\in\realnumbers\), si ha
\[
    \lim_{x\to+\infty}f(x,k)
    =
    \lim_{x\to+\infty}\left[(1-x+k)e^x+e^k-c\right]
    =
    -\infty.
\]
Poiché \(f(x,\cdot)\) è strettamente crescente e \(f(x,\varphi(x))=0\), segue che, per \(x\) sufficientemente grande,
\[
    \varphi(x)>k.
\]
Essendo \(k\) arbitrario,
\[
...
\[
    ...
    1.
\]
Dunque l'eventuale asintoto obliquo ha coefficiente angolare
\[
    m=1.
\]

Per determinare il termine noto, studiamo
\[
    \varphi(x)-x.
\]
Dividendo l'equazione implicita per \(e^x\), otteniamo
\[
    1-x+\varphi(x)+e^{\varphi(x)-x}-ce^{-x}=0,
\]
cioè
\[
    \varphi(x)-x
...
\[
    ...
    \varphi'(x)-1
    =
    -\frac{c}{e^x+e^{\varphi(x)}}.
\]
Quindi \(\varphi(x)-x\) è monotona definitivamente, con monotonia dipendente dal segno di \(c\), ed è limitata. Esiste dunque
\[
    q=\lim_{x\to+\infty}\bigl(\varphi(x)-x\bigr).
\]
Passando al limite in
\[
    1+\varphi(x)-x+e^{\varphi(x)-x}-ce^{-x}=0,
\]
si ottiene
\[
    1+q+e^q=0.
...
\[
    1+q+e^q=0
\]
ha un'unica soluzione, che indichiamo con \(\widetilde q\). Pertanto
\[
    \varphi(x)=x+\widetilde q+o(1)
    \qquad
    \text{per } x\to+\infty.
\]
...
\begin{center}
    \begin{tikzpicture}[scale=0.9]
        ...
        \draw[dashed] (-2.2,-1.1) -- (3.4,2.7) node[right] {\(y=x+\widetilde q\)};
        \draw[thick] plot[smooth] coordinates {
            (-2.8,-1.5) (-2,-1.15) (-1,-0.65) (0,-0.1)
            (1,0.55) (2,1.3) (3.2,2.25)
        };
        \node at (2.5,1.1) {\(\varphi\)};
    \end{tikzpicture}
\end{center}

Nel caso \(c=0\), la curva è esattamente una retta. Infatti, se
\[
    y=x+\widetilde q,
\]
allora
\[
    f(x,x+\widetilde q)
    =
    (1+\widetilde q)e^x+e^{x+\widetilde q}
    =
    e^x(1+\widetilde q+e^{\widetilde q})
    =
    0.
\]
Quindi, per \(c=0\),
\[
    \varphi(x)=x+\widetilde q.
\]

\subsubsection{Comportamento per \(x\to-\infty\) nel caso \(c>0\)}

Supponiamo ora \(c>0\). Allora, per ogni \(y\) fissato,
\[
...
e quindi
\[
    \lim_{x\to-\infty}f(x,\ln c)=0.
\]
Poiché \(f(x,\cdot)\) è strettamente crescente, si può confrontare \(\varphi(x)\) con livelli orizzontali vicini a \(\ln c\). Per ogni \(\varepsilon>0\), definitivamente si ha
\[
    f(x,\ln c-\varepsilon)<0
    \quad\Rightarrow\quad
    \varphi(x)>\ln c-\varepsilon,
\]
mentre
\[
    f(x,\ln c+\varepsilon)>0
...
Quindi
\[
    \lim_{x\to-\infty}\varphi(x)=\ln c.
\]
Per \(c>0\), la curva ha dunque asintoto orizzontale
\[
    y=\ln c
\]
per \(x\to-\infty\), e asintoto obliquo
\[
    y=x+\widetilde q
\]
per \(x\to+\infty\).
...
\begin{center}
    \begin{tikzpicture}[scale=0.9]
        ...
        \draw[->] (0,-1.6) -- (0,3.2) node[above] {\(y\)};
        \draw[dashed] (-3.8,1) -- (0.5,1) node[right] {\(y=\ln c\)};
        \draw[dashed] (-0.5,-0.8) -- (3.7,2.8) node[right] {\(y=x+\widetilde q\)};
        \draw[thick] plot[smooth] coordinates {
            (-3.6,1.02) (-2.6,1.03) (-1.5,1.1) (-0.5,1.35)
            (0.6,1.85) (1.8,2.35) (3.2,2.75)
        };
    \end{tikzpicture}
\end{center}
...
Per essere invertibile imponiamo che le dimensioni del dominio e del codominio siano le medesime:
...
Per il teorema della funzione implicita è possibile scrivere \(x=x(y)\) localmente
...
che \(f\) stabilisce una biiezione fra
...
\begin{snippettheorem}{Teorema della funzione inversa}
    Sia \(f \colon \Omega \subseteq \realnumbers^n \fromto \realnumbers^n\),
    \(\Omega\) aperto e sia \(x_0 \in \Omega\), e \(y_0 = f(x_0)\).
...
\begin{snippetexample}{Diffeomorfismo locale non implica diffeomorfismo globale}
    ...
    \[
        ...
        \end{pmatrix}
    \]
    Il determinante è sempre positivo, quindi è sempre invertibile.
...
\begin{snippetproof}{}
    Sia \(y_0 \in f(\Omega)\).
    Vogliamo mostrare che \(y_0\) è intorno di \(f(\Omega)\).
    ...
    tale che la funzione rispetta \(f \colon U \fromto V\) è un diffeomorfismo.
...
\begin{snippetproof}{}
    ...
    è un diffeomorfismo.
    In particolare, l'inverso di questa funzione è differenziabile.
...
\begin{snippetdefinition}{Trasformazione di coordinate}
...
\]
la funzione è biunivoca in questo dominio e inoltre
\[
...
\begin{snippetexample}{}
    ...
    possiamo isolare \(m\) incognite in funzione delle altre \(n-m\).
    Sia \(k = n-m\) la dimensione di \(M\) (se tutto va bene)
    come varietà. \\
    ...
    Possiamo applicare il teorema della funzione implicita se e solo se
    \(\jacobian_\Phi(x_0)\) ha rango massimo, cioè \(\text{rank} \jacobian_\Phi(x_0) = n-k\), ovvero se
    \(\text{d}\Phi(x_0)\) è suriettivo. In questo caso, a meno di permutare le variabili posso scrivere
    \(x\in\realnumbers^n\) con \(x=(z,y)\) con \(z\in\realnumbers^k\) e \(y\in\realnumbers^{n-k}\) e
    ...
    Se \(\Phi\) è di classe \(\mathcal{C}^1\), lo è pure \(\varphi\), quindi possiamo sviluppare
...
\begin{snippetdefinition}{}
    Se \(M = \Phi^{-1}(0)\), \(\Phi(x) = 0\) e \(\text{d}\Phi(x_0)\) è suriettiva, allora
...
\begin{snippetexample}{}
    ...
    \((0,1,1)\in T_{(x,y,z)}M\)?
    \(\nabla \Phi(x,y,z) = (yz^2,xz^2, 2xyz)\). Se si annulla almeno una delle tre coordinate 
    ...
    \[
        ...
        \end{pmatrix}
    \]
    meaning that \(v\in T_{x_0}M\) if and only if \(v \cdot \nabla \Phi_i(x_0) = 0\).
    Cioè, \(T_{x_0}M^\perp = \langle \nabla \Phi_1(x_0), \cdots, \nabla \Phi_{n-k}(x_0)\rangle\)
    ovvero \(T_{x_0}M = \langle \nabla \Phi_1(x_0), \cdots, \nabla \Phi_{n-k}(x_0) \rangle^\perp\)
    e \(\nabla \Phi_1(x_0), \cdots, \nabla \Phi_{n-k}(x_0)\) sono l.i. perché i vettori
    delle righe di \(\jacobian_\Phi(x_0)\) e per ipotesi \(\text{rank}\,\jacobian_\Phi(x_0) = n-k\).
\end{snippetexample}
...
Facendo variare il cammino otteniamo tutti e soli i vettori tangenti
ad \(M\) in \(x_0\), cioè \[
    T_{x_0}M = \{
...
funziona implicita, esiste quindi localmente \(\varphi \colon \realnumbers^n \fromto \realnumbers^{n-k}\)
...
La condizione che \((\alpha,\beta) \in \text{ker}\,\text{d}\Phi(x_0)\), cioè che sia vettore tangente, è che
...
\begin{snippetexample}{Sottovarietà}
    La sfera unitaria è una sottovarietà \((n-1)\)-dimensionale di \(\realnumbers^n\)
    \[
...
\begin{snippetexample}{Sottovarietà}
    ...
    e quindi tutti i punti sono regolari.
...
\begin{snippetexample}{Toro}
    ...
    quindi è una superficie algebrica, cioè luogo degli zeri di un polinomio di quarto grado.
    \begin{align*}
...
\begin{snippetdefinition}{Punto critico vincolato}
...
\begin{snippetproof}{}
    ...
    Per ipotesi, \(f\) ha un punto di estremo in \(x_0\) vincolato da \(M\),
    quindi anche \(f \circ \gamma\) ha un punto di estremo in \(x_0\) siccome \(\gamma(t) \in M\)
    per tutte le \(t\).
    Abbiamo quindi una funzione reale di una variabile che ha un punto di estremo in \(t=0\),
...
\begin{snippetexercise}{}
    ...
    I punti critici vincolari sono la soluzione del sistema
    \[
        \begin{cases}
            y = \lambda(2x-y) \\
            x = \lambda(2y-x) \\
...
\begin{snippetexercise}{}
    ...
    Il vincolo è quindi \(\Phi = x^2 y - 1 = 0\).
...
\begin{snippetexercise}{}
    ...
    Abbiamo \(V(x,y,z) = xyz\) e la superficie è \(S = 2xy +2xz + 2yz > 0\).
    ...
    Se una variabile si annulla, tutte le altre due si annullano e ciò non è possibile nel vincolo.
    ...
    e che quindi ammette massimo. Il massimo si può trovare o all'interno
...
\begin{snippetexercise}{}
    ...
    Ma se \(x=y=0\) allora la prima equazione non è soddisfatta, quindi il vincolo è regolare.
    ...
    in quanto preimmagine di zero tramite funzione continua.
...
\begin{snippettheorem}{Criterio del secondo ordine}
    sia \(\Omega \subset \realnumbers^n\) aperto e sia
    \(M\) il vincolo dato dai punti di annullamento di \[
        \begin{cases}
            \Phi_1(x) = 0 \\
            \vdots \\
            \Phi_m(x) = 0
    ...
    punto regolare (cioè il rango del differenziale di \(\Phi(x_0)\) è \(m\)).
    ...
    \begin{enumerate}
        ...
        è punto di minimo locale vincolato.
...
\begin{snippetexercise}{}
    ...
    \(M = \Phi^{-1}(0)\) per \(\Phi(x,y,z) = z^2 - xy - 1\). Ottimizziamo \(f\) sul vincolo \(M\).
    ...
    \[
        ...
        \end{cases}
    \]
    Dalla terza otteniamo che \(z=0\) oppure \(\lambda = 1\).
    ...
    \[
        ...
        \end{cases}
    \]
    come prima facendo lo stesso ragionamento del sistema, ma il determinante non è nullo e quindi l'unica soluzione è
    ...
    su \(D^\circ\).
    Se \(x_0 \in \partial D\) in particolare \(x_0\) è punto di minimo per \(\restr{g}{\partial D}\) cioè
...
\begin{snippetexample}{}
    ...
    Il gradiente \(\gradient f(x,y) = (2x + 2y, 1 + 2x)\) si annulla
    ...
    \]
    In un intorno di \(B\) il vincolo è il luogo degli zeri della funzione
    ...
    Consideriamo la linea spezzata che congiunge \(x\) a \(B\) tramite un segmento verticale fra
...
Se \(n>1\) si parla di equazione derivata parziale, se \(n = 1\) si parla di equazione differenziale ordinaria.
...
Consideriamo un'equazione differenziale ordinaria del primo ordine in forma normale:
\[
    y'=f(x,y),
\]
dove
\[
    f:\Omega \subseteq \realnumbers^2 \to \realnumbers.
\]

\begin{snippetdefinition}{Soluzione}
    Sia \(I\subseteq\realnumbers\) un intervallo. Una funzione \(y:I\to\realnumbers\) si dice soluzione dell'equazione
    \[
        y'=f(x,y)
    \]
    ...
    \begin{enumerate}
        ...
        \item \((x,y(x))\in\Omega\) per ogni \(x\in I\);
        \item \(y'(x)=f(x,y(x))\) per ogni \(x\in I\).
    \end{enumerate}
\end{snippetdefinition}

\begin{snippetdefinition}{Problema di Cauchy}
    Sia \((x_0,y_0)\in\Omega\). Un problema di Cauchy per l'equazione \(y'=f(x,y)\) è il sistema
    \[
        \begin{cases}
            y'=f(x,y),\\
            y(x_0)=y_0.
        \end{cases}
    \]
    La condizione \(y(x_0)=y_0\) impone che il grafico della soluzione passi per il punto \((x_0,y_0)\).
\end{snippetdefinition}

\begin{snippetexample}{Un esempio elementare}
    Consideriamo
    \[
        \begin{cases}
            y'=\dfrac{1}{x},\\
        ...
        \end{cases}
        \qquad
        \Omega=(\realnumbers\setminus\{0\})\times\realnumbers.
    \]
    Le primitive di \(1/x\) sono
    \[
        y(x)=\ln|x|+c.
    \]
    Imponendo la condizione iniziale \(y(1)=0\), otteniamo \(c=0\). La soluzione del problema di Cauchy sull'intervallo massimale contenente \(1\) è quindi
    \[
        y(x)=\ln x,
        \qquad x\in(0,+\infty).
    \]
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.9]
            ...
            \draw[domain=0.15:4,samples=100] plot (\x,{0.6*ln(\x)-0.8});
            \draw[domain=0.15:4,samples=100,very thick] plot (\x,{0.6*ln(\x)});
            \draw[domain=0.15:4,samples=100] plot (\x,{0.6*ln(\x)+0.8});

            \draw[domain=-4:-0.15,samples=100] plot (\x,{0.6*ln(-\x)-0.8});
            \draw[domain=-4:-0.15,samples=100] plot (\x,{0.6*ln(-\x)});
            \draw[domain=-4:-0.15,samples=100] plot (\x,{0.6*ln(-\x)+0.8});
    ...
    Senza richiedere che la soluzione sia definita su un intervallo, si potrebbero costruire funzioni del tipo
    \[
        y(x)=
        \begin{cases}
            \ln x, & x>0,\\
        ...
        \end{cases}
    \]
    con \(c\in\realnumbers\). Questo mostra perché, nella definizione corretta, il dominio della soluzione deve essere un intervallo.
\end{snippetexample}

\begin{snippettheorem}{Esistenza e unicità locale}
    Sia \(\Omega\subseteq\realnumbers^2\) aperto, sia \((x_0,y_0)\in\Omega\), e sia
    \[
        f:\Omega\to\realnumbers.
    \]
    Supponiamo che \(f\) e \(\frac{\partial f}{\partial y}\) siano continue in \(\Omega\). Allora esiste un intorno \(I\) di \(x_0\) in cui è definita un'unica soluzione del problema di Cauchy
    \[
        \begin{cases}
            y'=f(x,y),\\
            y(x_0)=y_0.
...
\begin{snippetcorollary}{Non intersezione dei grafici}
    Supponiamo che \(f\) soddisfi le ipotesi del teorema di esistenza e unicità locale. Allora i grafici di due soluzioni distinte dell'equazione
    \[
        y'=f(x,y)
    \]
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.8]
            ...
            \draw[domain=-2:2,samples=100] plot (\x,{0.45*\x});
            \draw[domain=-2:2,samples=100] plot (\x,{-0.45*\x});
            \fill (0,0) circle (1.5pt);
            \node at (0,-2.1) {intersezione impossibile};
        \end{tikzpicture}
    \end{center}
\end{snippetcorollary}

\begin{snippetproof}{}
    Se due soluzioni distinte si intersecassero in un punto \((x_0,y_0)\), allora entrambe risolverebbero il problema di Cauchy con dato iniziale \(y(x_0)=y_0\). Questo contraddirebbe l'unicità locale.
\end{snippetproof}

\begin{snippetexample}{}
    L'equazione \(y' = f(x)\) dove
    \[
        ...
        \end{cases}
    \]
    non ha soluzione in alcun intervallo contenente \(x=0\).
\end{snippetexample}

\subsection{Equazioni a variabili separabili}

\begin{snippetdefinition}{Equazione a variabili separabili}
    ...
    \[
        ...
        u:J\to\realnumbers,
    \]
    con \(I,J\subseteq\realnumbers\) intervalli.
\end{snippetdefinition}

Siano \(x_0\in I\) e \(y_0\in J\). Consideriamo il problema di Cauchy
\[
    \begin{cases}
        y'=h(x)u(y),\\
        y(x_0)=y_0.
    \end{cases}
\]
Supponiamo \(h\in C^1(I)\) e \(u\in C^1(J)\). Allora
\[
    f(x,y)=h(x)u(y)
\]
è di classe \(C^1\) in \(I\times J\), quindi vale il teorema di esistenza e unicità locale.

\begin{snippetproposition}{Soluzioni costanti}
    Se \(u(y_0)=0\), allora la funzione costante
    \[
        y(x)=y_0
    \]
...
Supponiamo ora che \(u(y_0)\neq 0\). Per continuità, \(u(y)\neq 0\) in un intorno di \(y_0\). Se \(y(x)\) è la soluzione del problema di Cauchy, allora, restringendosi eventualmente a un intorno di \(x_0\), si ha
\[
    u(y(x))\neq 0.
\]
Possiamo quindi dividere per \(u(y(x))\):
\[
    \frac{y'(x)}{u(y(x))}=h(x).
\]
Integrando tra \(x_0\) e \(x\), otteniamo
\[
    \integral[x_0][x][\frac{y'(t)}{u(y(t))}][t]
    =
    \integral[x_0][x][h(t)][t].
\]
Con il cambio di variabile \(s=y(t)\), il membro sinistro diventa
\[
...
Poniamo
\[
    F(y)=\integral[\frac{1}{u(y)}][y],
    \qquad
    G(x)=\integral[h(x)][x].
\]
Allora la soluzione è descritta implicitamente da
\[
    F(y(x))=G(x)+c.
\]
Se \(F\) è invertibile, otteniamo localmente
\[
    y(x)=F^{-1}(G(x)+c).
\]
...
\[
    F'(y_0)=\frac{1}{u(y_0)}\neq 0,
\]
la funzione \(F\) è localmente invertibile in un intorno di \(y_0\).
...
\begin{snippetexample}{}
    ...
    \]
    La funzione \(u\) non si annulla mai, quindi non ci sono soluzioni costanti. Separando le variabili:
    \begin{align*}
        yy'&=4x^3,\\
        \left(\frac{1}{2}y^2\right)'&=(x^4)'.
    \end{align*}
    Quindi
    \[
        \frac{1}{2}y^2=x^4+c.
    \]
    Imponendo \(y(0)=2\), si ottiene \(c=2\). Pertanto
    \[
        y^2=2x^4+4,
    \]
    e, siccome \(y(0)>0\),
    \[
        y(x)=\sqrt{2x^4+4}.
    \]
    L'intervallo di definizione della soluzione è \(I=\realnumbers\).
\end{snippetexample}
...
\begin{snippetexample}{}
    ...
    \]
    L'equazione è a variabili separabili con
    \[
        h(x)=e^x,
        \qquad
        u(y)=e^{-y},
        ...
        \Omega=\realnumbers^2.
    \]
    Poiché \(u(y)\neq 0\) per ogni \(y\), non ci sono soluzioni costanti. Separando le variabili:
    \begin{align*}
        e^y y'&=e^x,\\
        (e^y)'&=(e^x)'.
    \end{align*}
    ...
    \[
        y(x)=\ln(e^x+e^a-1).
    \]
    Il dominio dipende da \(a\). Dobbiamo imporre
    \[
        e^x+e^a-1>0,
    \]
    ovvero
    \[
        e^x>1-e^a.
    \]
    Se \(a\geq 0\), allora \(1-e^a\leq 0\), quindi la disuguaglianza è sempre soddisfatta e
    \[
        I=\realnumbers.
    \]
    Se invece \(a<0\), allora \(1-e^a>0\), dunque
    \[
        x>\ln(1-e^a),
    \]
    e quindi
    \[
        I=\bigl(\ln(1-e^a),+\infty\bigr).
    \]
    In questo caso \(x=\ln(1-e^a)\) è un asintoto verticale.
\end{snippetexample}
...
\begin{snippetexample}{}
    ...
    \[
        ...
        \end{cases}
    \]
    L'equazione è a variabili separabili. Qui
    \[
        h(x)=2x,
        \qquad
        u(y)=\frac{1}{ye^y}.
    \]
    Poiché \(y(1)=3>0\), lavoriamo nell'intervallo \(J=(0,+\infty)\), dove \(u\) non si annulla. Separando le variabili:
    \[
        ye^y y'=2x.
    \]
    Integrando tra \(1\) e \(x\):
    \[
        \integral[1][x][y(t)e^{y(t)}y'(t)][t]
        =
        \integral[1][x][2t][t].
    \]
    Con il cambio di variabile \(u=y(t)\):
    \[
        \integral[3][y(x)][ue^u][u]
        =
        x^2-1.
    \]
    Osserviamo che
    \[
        \integral[ue^u][u]=(u-1)e^u.
    \]
    Quindi
    \begin{align*}
        \bigl[(u-1)e^u\bigr]_{3}^{y(x)}&=x^2-1,\\
        (y(x)-1)e^{y(x)}-2e^3&=x^2-1.
    \end{align*}
    La soluzione è dunque descritta implicitamente da
    \[
        (y(x)-1)e^{y(x)}=x^2-1+2e^3.
    \]
    Non è possibile esplicitare \(y\) con funzioni elementari. Tuttavia, siccome la derivata rispetto a \(y\) di \((y-1)e^y\) è \(ye^y\), che è positiva per \(y>0\), il teorema della funzione implicita permette comunque di studiare localmente l'andamento della soluzione.
\end{snippetexample}

\subsection{Equazioni differenziali lineari del primo ordine}

\begin{snippetdefinition}{Equazione lineare del primo ordine}
    Un'equazione differenziale lineare del primo ordine in forma normale è un'equazione del tipo
    \[
        y'+p(x)y=q(x),
    \]
    dove
    \[
        p,q:I\to\realnumbers.
    \]
    Essa è in forma normale con
    \[
        f(x,y)=q(x)-p(x)y.
    \]
\end{snippetdefinition}

\begin{snippettheorem}{Formula risolutiva per il problema di Cauchy}
    Sia \(I\subseteq\realnumbers\) un intervallo, siano \(x_0\in I\), \(y_0\in\realnumbers\), e siano \(p,q:I\to\realnumbers\) continue in \(I\). Allora il problema di Cauchy
    \[
        \begin{cases}
            y'+p(x)y=q(x),\\
            y(x_0)=y_0,
        \end{cases}
    \]
    ha una e una sola soluzione definita su tutto \(I\), data da
    \[
        y(x)=e^{-\integral[x_0][x][p(u)][u]}
        \left\{
            y_0+
            \integral[x_0][x][
                q(t)e^{\integral[x_0][t][p(u)][u]}
            ][t]
        \right\}.
    \]
\end{snippettheorem}

Inoltre, tutte le soluzioni dell'equazione differenziale lineare
\[
    y'+p(x)y=q(x)
\]
su \(I\) sono della forma
\[
    y(x)=e^{-\integral[x_0][x][p(u)][u]}
    \left\{
        C+
...
\begin{snippetproof}{}
    Fissiamo \(x_0\in I\) e supponiamo che \(y\) sia soluzione, cioè
    \[
        y'+p(x)y=q(x).
    \]
    Moltiplichiamo entrambi i membri per un fattore integrante \(\mu(x)\), da scegliere in modo che il membro sinistro diventi la derivata di un prodotto:
    \[
        \mu y'+p\mu y=q\mu.
    \]
    Vogliamo che
    \[
        (\mu y)'=\mu y'+\mu' y.
    \]
    È quindi sufficiente imporre
    \[
        \mu'=p\mu.
    \]
    Questa è un'equazione a variabili separabili:
    \begin{align*}
        \frac{\mu'}{\mu}&=p(x),\\
        \ln|\mu|&=\integral[p(x)][x].
    \end{align*}
    Possiamo scegliere
    \[
        \mu(x)=e^{\integral[x_0][x][p(u)][u]},
    \]
    così che \(\mu(x_0)=1\). Allora
    \[
        (\mu y)'=q\mu.
    \]
    ...
    \[
        ...
        =
        \integral[x_0][x][q(t)\mu(t)][t].
    \]
    Poiché \(\mu(x_0)=1\), otteniamo
    \[
        y(x)
        =
        \mu(x)^{-1}
        ...
        \right\}.
    \]
    Sostituendo l'espressione di \(\mu\), segue la formula
    \[
        y(x)=e^{-\integral[x_0][x][p(u)][u]}
        \left\{
            y_0+
...
\begin{snippetexample}{}
    ...
    \begin{align*}
        ...
        \left\{
            1+\integral[0][x][
                t^3 e^{\integral[0][t][2s][s]}
            ][t]
        \right\}\\
        &=
        e^{-x^2}
        \left[
            1+\integral[0][x][t^3e^{t^2}][t]
        \right].
    \end{align*}
    ...
    \[
        \integral[t^3e^{t^2}][t]
        =
        \frac{1}{2}(t^2-1)e^{t^2}.
    \]
    Quindi
    \begin{align*}
        \integral[0][x][t^3e^{t^2}][t]
        &=
        \frac{1}{2}(x^2-1)e^{x^2}+\frac{1}{2}.
    \end{align*}
    Ne segue
    \begin{align*}
        y(x)
        ...
        e^{-x^2}
        \left[
            1+\frac{1}{2}(x^2-1)e^{x^2}+\frac{1}{2}
        \right]\\
        &=
        \frac{3}{2}e^{-x^2}+\frac{x^2-1}{2}.
    \end{align*}
\end{snippetexample}
...
\begin{snippetexample}{}
    ...
    \[
        ...
        \end{cases}
    \]
    La funzione \(\tan x\) è continua negli intervalli del tipo
    \[
        \left(-\frac{\pi}{2}+k\pi,\frac{\pi}{2}+k\pi\right).
    \]
    Poiché il dato iniziale è assegnato in \(x=\pi\), dobbiamo scegliere
    \[
        I=\left(\frac{\pi}{2},\frac{3\pi}{2}\right).
    \]
    Qui
    \[
        p(x)=\tan x,
    ...
    Applicando la formula:
    \[
        y(x)=e^{-\integral[\pi][x][\tan t][t]}
        \left\{
            1+
            \integral[\pi][x][
                3\sin t\,
                e^{\integral[\pi][t][\tan s][s]}
            ][t]
        \right\}.
    \]
    Calcoliamo
    \[
        \integral[\pi][x][\tan t][t]
        =
        -\ln|\cos x|.
    \]
    Quindi
    \[
        e^{-\integral[\pi][x][\tan t][t]}
        =
        e^{\ln|\cos x|}
        =
        |\cos x|.
    \]
    Per \(x\in\left(\frac{\pi}{2},\frac{3\pi}{2}\right)\) si ha \(\cos x<0\), dunque
    \[
        |\cos x|=-\cos x.
    \]
    Inoltre
...
\begin{snippetdefinition}{Equazione di Bernoulli}
    Un'equazione di Bernoulli è un'equazione differenziale del tipo
    \[
        y'+a(x)y+b(x)y^\gamma=0,
    \]
    con
    \[
        \gamma\neq 0,
...
In generale, per rendere sempre ben definita la potenza \(y^\gamma\), si lavora nel dominio
\[
    \Omega=\{(x,y):x\in I,\ y>0\}.
\]
Per particolari valori di \(\gamma\), ad esempio se \(\gamma\) è un intero positivo, l'equazione può essere considerata anche per \(y\leq 0\).

Supponiamo \(a,b\in C^1(I)\) e consideriamo il problema di Cauchy
\[
    \begin{cases}
        y'+a(x)y+b(x)y^\gamma=0,\\
        y(x_0)=y_0.
...
\begin{snippetproposition}{Soluzione nulla}
    Se \(\gamma>0\), allora la funzione
    \[
        y(x)=0
    \]
    è soluzione dell'equazione differenziale. In particolare, è soluzione del problema di Cauchy quando \(y_0=0\).
\end{snippetproposition}

Nel caso \(y_0>0\), per il teorema di esistenza e unicità locale la soluzione locale del problema di Cauchy resta positiva in un intorno di \(x_0\). In tale intorno possiamo moltiplicare l'equazione per \(y^{-\gamma}\). Otteniamo
\[
    y'y^{-\gamma}+a(x)y^{1-\gamma}+b(x)=0.
\]
Poniamo
\[
    z(x)=y(x)^{1-\gamma}.
...
Equivalentemente,
\[
    z'(x)+(1-\gamma)a(x)z(x)+(1-\gamma)b(x)=0.
\]
Questa è un'equazione lineare in \(z\). La condizione iniziale diventa
\[
    z(x_0)=y_0^{1-\gamma}.
\]
Una volta risolta l'equazione lineare in \(z\), se \(z(x)>0\) si recupera
\[
    y(x)=z(x)^{\frac{1}{1-\gamma}}.
\]
...
\begin{snippetexample}{}
    ...
    Supponiamo ora \(b\neq 0\). Per unicità, la soluzione non si annulla localmente. Poniamo
    \[
        z=y^{1-\gamma}=y^{-3}.
    \]
    Allora
    ...
    e
    \[
        y'=-\frac{1}{3}z^{-\frac{4}{3}}z'.
    \]
    Sostituendo nell'equazione:
    \[
        -\frac{1}{3}z^{-\frac{4}{3}}z'
        -
        z^{-\frac{1}{3}}
        +
        xz^{-\frac{4}{3}}
    ...
    \[
        z(0)=b^{-3}.
    \]
    Applicando la formula per le equazioni lineari:
    \[
        z(x)=e^{-3x}
        \left\{
            b^{-3}
            +
            \integral[0][x][3t e^{3t}][t]
        \right\}.
    \]
    Poiché
    \[
    ...
    \[
        ...
        \right]^{-\frac{1}{3}},
    \]
    sull'intervallo massimale contenente \(0\) in cui la quantità tra parentesi non si annulla.
\end{snippetexample}

\subsection{Ancora sulle equazioni di Bernoulli}

Riprendiamo l'equazione di Bernoulli
\[
    y'+a(x)y+b(x)y^\gamma=0,
    \qquad \gamma\neq 0,1.
\]
...
\[
    z=y^{1-\gamma}
\]
si ottiene un'equazione lineare in \(z\). È importante ricordare che questo cambio di variabile è lecito soltanto negli intervalli in cui la soluzione non si annulla.

\begin{snippetexample}{}
    Consideriamo
    \[
        \begin{cases}
            y'+\dfrac{2}{x+1}y-\dfrac{1}{x^2-1}\sqrt{y}=0,\\
            y(0)=b,
        \end{cases}
        \qquad b\geq 0.
    \]
    ...
    Studiamo ora il caso \(b>0\). L'equazione è di Bernoulli con
    \[
        \gamma=\frac{1}{2}.
    \]
    Lavoriamo nell'insieme
    \[
        (-1,1)\times(0,+\infty),
    \]
    dove la funzione
    \[
        f(x,y)=-\frac{2}{x+1}y+\frac{1}{x^2-1}\sqrt{y}
    \]
    è di classe \(C^1\). Poniamo
    \[
        z=y^{\frac{1}{2}},
        \qquad
        y=z^2,
    ...
    Sostituendo nell'equazione:
    \[
        2zz'+\frac{2}{x+1}z^2-\frac{1}{x^2-1}z=0.
    \]
    Siccome \(z>0\), possiamo dividere per \(z\):
    \[
        2z'+\frac{2}{x+1}z-\frac{1}{x^2-1}=0.
    \]
    Equivalentemente,
    \[
        z'+\frac{1}{x+1}z=\frac{1}{2(x^2-1)}.
    \]
    Moltiplicando per il fattore integrante \(x+1\), otteniamo
    \[
        \bigl((x+1)z\bigr)'=\frac{1}{2(x-1)}.
    \]
    Integrando tra \(0\) e \(x\):
    \[
        (x+1)z(x)=\sqrt{b}+\frac{1}{2}\ln(1-x).
    \]
    Pertanto
    \[
        z(x)=\frac{\sqrt{b}+\frac{1}{2}\ln(1-x)}{x+1},
    \]
    e quindi
    \[
        y(x)=
        \left(
            \frac{\sqrt{b}+\frac{1}{2}\ln(1-x)}{x+1}
        \right)^2.
    \]
    La formula è ottenuta imponendo \(z>0\). Lo zero di \(z\) è dato da
    \[
        \sqrt{b}+\frac{1}{2}\ln(1-x)=0,
    \]
    ossia
    \[
        x_1=1-e^{-2\sqrt{b}}.
    \]
    Perciò, per \(b>0\), la soluzione positiva che passa per \((0,b)\) è definita su
    \[
        (-1,x_1).
    \]
    ...
    Osserviamo però che nel punto \(x_1\) si ha \(y(x_1)=0\). Se consideriamo il problema di Cauchy con dato iniziale
    \[
        y(x_1)=0,
    \]
    allora esso ammette almeno due soluzioni: la soluzione identicamente nulla e la soluzione appena costruita. Questo accade perché, per \(y=0\), la derivata \(\frac{\partial f}{\partial y}\) esplode, quindi non sono soddisfatte le ipotesi del teorema di esistenza e unicità locale.

    In realtà, nei punti dell'asse \(x\) è possibile raccordare la soluzione nulla con soluzioni positive, ottenendo molte soluzioni diverse.
...
\begin{snippetdefinition}{Equazione di Riccati}
    Un'equazione di Riccati è un'equazione differenziale del tipo
    \[
...
La funzione
\[
    f(x,y)=-a(x)y-b(x)y^2-c(x)
\]
è continua e localmente lipschitziana rispetto a \(y\). Quindi sono soddisfatte le ipotesi del teorema di esistenza e unicità locale.

Supponiamo di conoscere una soluzione particolare \(y_\ast\) dell'equazione, cioè
\[
    y_\ast'+a(x)y_\ast+b(x)y_\ast^2+c(x)=0.
\]
Poniamo
\[
    y(x)=y_\ast(x)+u(x).
...
\begin{align*}
    ...
    y_\ast'+u'
    +a(x)(y_\ast+u)
    +b(x)(y_\ast^2+u^2+2y_\ast u)
    +c(x)\\
    &=
    \underbrace{
        y_\ast'+a(x)y_\ast+b(x)y_\ast^2+c(x)
    }_{=0}
    +
    u'
    +
    \bigl(a(x)+2b(x)y_\ast(x)\bigr)u
    +
    b(x)u^2.
\end{align*}
Dunque \(u\) soddisfa
\[
    ...
    =0.
\]
Questa è un'equazione di Bernoulli con esponente \(\gamma=2\). Ponendo
\[
    z=\frac{1}{u},
\]
si ottiene un'equazione lineare in \(z\). Infatti, se
\[
    A(x)=a(x)+2b(x)y_\ast(x),
\]
allora
\[
    u'+A(x)u+b(x)u^2=0.
\]
Poiché \(u=1/z\), abbiamo
\[
    u'=-\frac{z'}{z^2}.
\]
Quindi
\[
    -\frac{z'}{z^2}+\frac{A(x)}{z}+\frac{b(x)}{z^2}=0.
\]
Moltiplicando per \(z^2\):
\[
    -z'+A(x)z+b(x)=0,
\]
cioè
...
\begin{snippetexample}{}
    ...
    \[
        \begin{cases}
            y'+\left(y^2+y-\dfrac{3}{4}\right)\cos x=0,\\
            y(0)=y_0.
        \end{cases}
    \]
    La funzione
    \[
        y_\ast=\frac{1}{2}
    \]
    è una soluzione particolare, perché
    \[
        y_\ast^2+y_\ast-\frac{3}{4}
        =
        \frac{1}{4}+\frac{1}{2}-\frac{3}{4}
        =
        0.
    \]
    Quindi, per studiare le soluzioni al variare del dato iniziale \(y_0\), si può porre
    \[
        y=\frac{1}{2}+\frac{1}{z}.
    \]
    Il caso \(y_0=\frac{1}{2}\) corrisponde direttamente alla soluzione particolare
    \[
        y(x)=\frac{1}{2}.
    \]
\end{snippetexample}
...
\begin{snippetexample}{}
    ...
    \[
        \begin{cases}
            y'-y^2+2y\ln x-\ln^2 x-\dfrac{1}{x}=0,\\
            y(1)=a,
        \end{cases}
        \qquad x>0.
    \]
    ...
    \begin{align*}
        y_\ast'-y_\ast^2+2y_\ast\ln x-\ln^2 x-\frac{1}{x}
        &=
        \frac{1}{x}
        -\ln^2 x
        +2\ln^2 x
        ...
        &=0.
    \end{align*}
    Inoltre \(y_\ast(1)=0\), quindi per \(a=0\) la soluzione del problema di Cauchy è
    \[
        y(x)=\ln x,
        \qquad x\in(0,+\infty).
    \]
    ...
    \begin{align*}
        ...
        \frac{1}{x}-\frac{z'}{z^2}
        -
        \left(\ln x+\frac{1}{z}\right)^2
        +
        2\left(\ln x+\frac{1}{z}\right)\ln x
        -\ln^2 x
        -\frac{1}{x}\\
        &=
        -\frac{z'}{z^2}-\frac{1}{z^2}.
    \end{align*}
    Quindi
    \[
        -z'-1=0,
    ...
    Per \(a\neq 0\),
    \[
        y(x)=\ln x+\frac{1}{-x+1+\frac{1}{a}}.
    \]
    Il denominatore si annulla per
    \[
        x=1+\frac{1}{a}.
    \]
    L'intervallo massimale contenente \(1\), nel dominio \(x>0\), è quindi:
    \[
        I=
        \begin{cases}
            \left(0,1+\dfrac{1}{a}\right), & a>0,\\[6pt]
            \left(1+\dfrac{1}{a},+\infty\right), & a<-1,\\[6pt]
            (0,+\infty), & -1\leq a<0.
        \end{cases}
    \]
    Per \(a=0\), invece,
...
\subsection{Equazioni omogenee del primo ordine}

\begin{snippetdefinition}{Equazione omogenea del primo ordine}
    Un'equazione differenziale del primo ordine si dice omogenea se può essere scritta nella forma
    \[
        y'=g\left(\frac{y}{x}\right),
    \]
    dove
    \[
        g:\realnumbers\to\realnumbers.
    \]
\end{snippetdefinition}

Per risolvere questo tipo di equazione si usa il cambio di variabile
\[
    \frac{y}{x}=t,
    \qquad
    y=xt(x).
...
Se esiste \(t_0\in\realnumbers\) tale che
\[
    g(t_0)=t_0,
\]
allora \(t=t_0\) è una soluzione costante dell'equazione in \(t\). Essa corrisponde alla soluzione
\[
    y=t_0x,
\]
cioè a una retta passante per l'origine.
...
\begin{snippetexample}{}
    ...
    \[
        ...
        \end{cases}
    \]
    Per la condizione iniziale possiamo lavorare nel dominio
    \[
        x>0,
        \qquad
        y>0.
    ...
    La soluzione è
    \[
        y(x)=x\sqrt{2\ln x+4-2\ln 2}
        =
        x\sqrt{4+2\ln\left(\frac{x}{2}\right)}.
    \]
    L'intervallo di definizione è determinato da
    \[
        4+2\ln\left(\frac{x}{2}\right)>0.
    \]
    Equivalentemente,
    \[
        \ln\left(\frac{x}{2}\right)>-2,
        \qquad
        \frac{x}{2}>e^{-2},
    \]
    dunque
...
\subsection{Equazioni differenziali lineari di ordine \(k\) a coefficienti costanti}

Consideriamo un'equazione differenziale lineare di ordine \(k\) a coefficienti costanti:
\[
    y^{(k)}
    +
    a_{k-1}y^{(k-1)}
...
dove
\[
    a_0,\ldots,a_{k-1}\in\realnumbers,
    \qquad
    b:I\to\realnumbers.
\]
In forma compatta:
\[
    y^{(k)}+\sum_{j=0}^{k-1}a_jy^{(j)}=b(x).
\]
Se \(b=0\), l'equazione si dice omogenea:
\[
    y^{(k)}+\sum_{j=0}^{k-1}a_jy^{(j)}=0.
\]
Se \(b\neq 0\), all'equazione si associa comunque l'equazione omogenea corrispondente:
\[
    y^{(k)}+\sum_{j=0}^{k-1}a_jy^{(j)}=0.
\]

\begin{snippetdefinition}{Problema di Cauchy per un'equazione di ordine \(k\)}
    Un problema di Cauchy per un'equazione differenziale di ordine \(k\) consiste nell'assegnare il valore della funzione e delle sue prime \(k-1\) derivate in un punto \(x_0\):
    \[
        \begin{cases}
            y^{(k)}+\sum_{j=0}^{k-1}a_jy^{(j)}=b(x),\\
            y(x_0)=y_0,\\
...
\begin{snippettheorem}{Esistenza e unicità globale}
    Se \(b\in C(I)\), allora ogni problema di Cauchy per l'equazione lineare di ordine \(k\) a coefficienti costanti ammette una e una sola soluzione definita su tutto \(I\).
\end{snippettheorem}

In questa situazione i grafici di due soluzioni possono intersecarsi, ma se due soluzioni hanno nello stesso punto \(x_0\) lo stesso valore e le stesse prime \(k-1\) derivate, allora coincidono su tutto l'intervallo di definizione. Quindi due soluzioni distinte devono differire almeno in uno dei dati
\[
    y(x_0),
    \quad
    y'(x_0),
...
\subsubsection{Struttura dello spazio delle soluzioni dell'omogenea}
...
\begin{snippetproposition}{Principio di sovrapposizione}
    Se \(w_1\) e \(w_2\) sono soluzioni dell'equazione omogenea, allora anche
    \[
        c_1w_1+c_2w_2
    \]
    è soluzione, per ogni \(c_1,c_2\in\realnumbers\).
\end{snippetproposition}

Quindi l'insieme delle soluzioni dell'equazione omogenea è un sottospazio vettoriale di \(C^k(\realnumbers)\).

\begin{snippettheorem}{Dimensione dello spazio delle soluzioni}
    Lo spazio vettoriale delle soluzioni dell'equazione omogenea
    \[
        y^{(k)}+\sum_{j=0}^{k-1}a_jy^{(j)}=0
    \]
    ha dimensione \(k\).
...
Per trovare tutte le soluzioni dell'equazione omogenea basta dunque trovare \(k\) soluzioni linearmente indipendenti. Se
\[
    w_1,\ldots,w_k
\]
sono \(k\) soluzioni linearmente indipendenti, allora l'integrale generale dell'equazione omogenea è
\[
    y(x)=\sum_{i=1}^{k}c_iw_i(x),
    \qquad c_i\in\realnumbers.
\]

\subsubsection{Polinomio caratteristico}

Per cercare soluzioni dell'equazione omogenea, partiamo dal caso \(k=1\):
\[
    y'+py=0.
\]
In questo caso
\[
    y(x)=Ce^{-px}.
\]
Questo suggerisce, per il caso generale, di cercare soluzioni della forma
\[
    y=e^{\lambda x}.
\]
Allora
\[
    y^{(j)}=\lambda^j e^{\lambda x}.
\]
Sostituendo nell'equazione omogenea:
\begin{align*}
    0
    &=
    y^{(k)}+\sum_{j=0}^{k-1}a_jy^{(j)}\\
    &=
    e^{\lambda x}
    \left[
        \lambda^k+\sum_{j=0}^{k-1}a_j\lambda^j
    \right].
\end{align*}
Poiché \(e^{\lambda x}\neq 0\), dobbiamo imporre
\[
    \lambda^k+\sum_{j=0}^{k-1}a_j\lambda^j=0.
\]
...
\begin{snippetdefinition}{Polinomio caratteristico}
    ...
    \[
        ...
        \lambda^k+\sum_{j=0}^{k-1}a_j\lambda^j
    \]
    si chiama polinomio caratteristico dell'equazione omogenea
    \[
        y^{(k)}+\sum_{j=0}^{k-1}a_jy^{(j)}=0.
    \]
    Le soluzioni esponenziali \(e^{\lambda x}\) si ottengono scegliendo \(\lambda\) tra le radici di \(P\).
\end{snippetdefinition}

\subsection{Equazioni lineari a coefficienti costanti: radici del polinomio caratteristico}

Consideriamo l'equazione differenziale lineare di ordine \(k\) a coefficienti costanti
\[
    y^{(k)}+\sum_{j=0}^{k-1}a_jy^{(j)}=b(x),
    \qquad b\in C^0(I),
    \qquad a_j\in\realnumbers.
\]
All'equazione non omogenea associamo l'equazione omogenea
\[
    y^{(k)}+\sum_{j=0}^{k-1}a_jy^{(j)}=0.
\]

Sappiamo che lo spazio delle soluzioni dell'equazione omogenea è uno spazio vettoriale reale di dimensione \(k\). Per determinarne tutte le soluzioni è quindi sufficiente trovare \(k\) soluzioni linearmente indipendenti.
...
Allora
\[
    \frac{d^j}{dx^j}\left(e^{\lambda x}\right)=\lambda^je^{\lambda x}.
\]
Sostituendo nell'equazione omogenea, otteniamo
\[
    e^{\lambda x}
    \left(
        \lambda^k+\sum_{j=0}^{k-1}a_j\lambda^j
...
dove
\[
    P(\lambda)=\lambda^k+\sum_{j=0}^{k-1}a_j\lambda^j
\]
è il polinomio caratteristico.

\begin{snippettheorem}{Soluzioni esponenziali}
    La funzione
    \[
        w(x)=e^{\lambda x}
    \]
    ...
    \[
        y^{(k)}+\sum_{j=0}^{k-1}a_jy^{(j)}=0
    \]
    se e solo se \(\lambda\) è una radice del polinomio caratteristico
    \[
        P(\lambda)=\lambda^k+\sum_{j=0}^{k-1}a_j\lambda^j.
    \]
\end{snippettheorem}

Poiché \(P\) ha coefficienti reali, se \(\lambda=\alpha+i\beta\) è una radice complessa di \(P\), allora anche
\[
    \overline{\lambda}=\alpha-i\beta
\]
è radice di \(P\), con la stessa molteplicità. Inoltre
\[
    e^{(\alpha+i\beta)x}
    =
    e^{\alpha x}\left(\cos(\beta x)+i\sin(\beta x)\right).
\]
Quindi da una coppia di radici complesse coniugate si ottengono soluzioni reali usando parte reale e parte immaginaria.

\begin{snippetexample}{Radici reali semplici}
    Consideriamo
    \[
        y''-3y'+2y=0.
    \]
    ...
    \[
        ...
        \lambda=1,2.
    \]
    Le due radici sono reali e semplici, quindi l'integrale generale è
    \[
        y(x)=c_1e^x+c_2e^{2x},
        \qquad c_1,c_2\in\realnumbers.
    \]
\end{snippetexample}

\begin{snippetexample}{Radici complesse coniugate}
    Consideriamo
    \[
        y''+4y=0.
    \]
    ...
    \[
        \lambda=\pm 2i.
    \]
    Le funzioni \(e^{2ix}\) ed \(e^{-2ix}\) sono soluzioni complesse. Combinandole linearmente si ottengono le soluzioni reali
    \[
        \cos(2x)=\frac{e^{2ix}+e^{-2ix}}{2},
        \qquad
        \sin(2x)=\frac{e^{2ix}-e^{-2ix}}{2i}.
    \]
    L'integrale generale reale è quindi
    \[
...
\begin{snippetexample}{Radice reale doppia}
    Consideriamo
    \[
        y''-2y'+y=0.
    \]
    ...
    \]
    La radice \(\lambda=1\) ha molteplicità \(2\). Una soluzione è
    \[
        e^x.
    \]
    Per ottenere una seconda soluzione linearmente indipendente si considera
    \[
        xe^x.
    \]
    Infatti questa nasce, formalmente, dal limite
    \[
        \lim_{\lambda_2\to\lambda_1}
        \frac{e^{\lambda_2x}-e^{\lambda_1x}}{\lambda_2-\lambda_1}
        =
        xe^{\lambda_1x}.
    \]
...
\begin{snippettheorem}{Regola generale per le radici}
    Sia \(P\) il polinomio caratteristico dell'equazione omogenea.

    \begin{enumerate}
        \item Se \(\lambda\in\realnumbers\) è radice di \(P\) con molteplicità \(r\), allora le funzioni
            \[
                e^{\lambda x},
                \quad
                xe^{\lambda x},
        ...
        \item Se \(\lambda=\alpha+i\beta\), con \(\beta\neq 0\), è radice di \(P\) con molteplicità \(r\), allora le funzioni reali
            \[
                e^{\alpha x}\cos(\beta x),
                \quad
                e^{\alpha x}\sin(\beta x),
                \quad
                xe^{\alpha x}\cos(\beta x),
                \quad
                xe^{\alpha x}\sin(\beta x),
                \quad
                \ldots
            \]
            fino a
            \[
                x^{r-1}e^{\alpha x}\cos(\beta x),
                \qquad
                x^{r-1}e^{\alpha x}\sin(\beta x)
            \]
            sono soluzioni reali linearmente indipendenti.
    \end{enumerate}
\end{snippettheorem}
...
\begin{snippetexample}{}
    ...
    \begin{align*}
        ...
        &=
        \lambda^2(\lambda-1)^2+(\lambda-1)^2\\
        &=
...
\begin{snippetexample}{}
    ...
    Il polinomio caratteristico è
    \[
        P(\lambda)=\lambda^3-2\lambda^2+5\lambda
        =
        \lambda(\lambda^2-2\lambda+5).
    \]
    Le radici sono
    \[
        \lambda=0,
    ...
    L'integrale generale è
    \[
        y(x)=c_1+c_2e^x\cos(2x)+c_3e^x\sin(2x).
    \]
\end{snippetexample}

\begin{snippetexample}{Oscillatore armonico}
    Consideriamo
    \[
        y''+\omega^2y=0,
        \qquad \omega>0.
    ...
    \]
    Equivalentemente, può essere scritto nella forma
    \[
        y(x)=A\cos(\omega x+\varphi),
    \]
    per opportuni \(A,\varphi\in\realnumbers\).
\end{snippetexample}

\subsection{Equazioni lineari non omogenee}

Indichiamo con \(L\) l'operatore differenziale lineare
\[
    L[y]=y^{(k)}+\sum_{j=0}^{k-1}a_jy^{(j)}.
\]
L'equazione non omogenea è
\[
    L[y]=b(x),
\]
...
\begin{snippetproposition}{Struttura dell'integrale generale}
    Sia \(y_\ast\) una soluzione particolare dell'equazione non omogenea
    \[
        L[y]=b(x).
    \]
    Siano \(w_1,\ldots,w_k\) \(k\) soluzioni linearmente indipendenti dell'equazione omogenea associata
    \[
        L[y]=0.
    \]
    Allora tutte le soluzioni dell'equazione non omogenea sono della forma
    \[
        y(x)=y_\ast(x)+c_1w_1(x)+\cdots+c_kw_k(x),
        \qquad c_1,\ldots,c_k\in\realnumbers.
    \]
\end{snippetproposition}

\begin{snippetproof}{}
    Se \(y_\ast\) è soluzione di \(L[y]=b(x)\), allora, per ogni soluzione \(w\) dell'omogenea,
    \[
        L[y_\ast+w]=L[y_\ast]+L[w]=b(x)+0=b(x).
    \]
    Quindi \(y_\ast+w\) è ancora soluzione della non omogenea.

    Viceversa, se \(y_\ast\) e \(\widetilde{y}\) sono due soluzioni della non omogenea, allora
    \[
        L[y_\ast-\widetilde{y}]
        =
        L[y_\ast]-L[\widetilde{y}]
        =
        b(x)-b(x)=0.
    \]
    Dunque \(y_\ast-\widetilde{y}\) è soluzione dell'omogenea. Ne segue che ogni soluzione della non omogenea si ottiene sommando a una soluzione particolare una soluzione generale dell'omogenea.
\end{snippetproof}

\subsection{Metodo di variazione delle costanti}

Il metodo di variazione delle costanti permette di determinare una soluzione particolare dell'equazione non omogenea a partire da un sistema fondamentale di soluzioni dell'equazione omogenea.

Per semplicità consideriamo prima il caso \(k=2\). Sia
\[
    y''+a_1y'+a_0y=b(x)
\]
e siano \(w_1,w_2\) due soluzioni linearmente indipendenti dell'omogenea associata. Cerchiamo una soluzione particolare nella forma
\[
    y_\ast(x)=c_1(x)w_1(x)+c_2(x)w_2(x),
\]
dove \(c_1,c_2\) sono funzioni da determinare.
...
Nel caso generale di ordine \(k\), il sistema diventa
\[
    \begin{pmatrix}
        w_1 & w_2 & \cdots & w_k\\
        w_1' & w_2' & \cdots & w_k'\\
        \vdots & \vdots & \ddots & \vdots\\
        w_1^{(k-1)} & w_2^{(k-1)} & \cdots & w_k^{(k-1)}
    \end{pmatrix}
    \begin{pmatrix}
        c_1'\\
        c_2'\\
    ...
    \end{pmatrix}.
\]
Poiché \(w_1,\ldots,w_k\) sono linearmente indipendenti, il wronskiano è non nullo e il sistema determina \(c_1',\ldots,c_k'\).

Nel caso \(k=1\), per l'equazione
\[
    y'+p(x)y=q(x),
\]
la soluzione generale già nota è
\[
    y(x)
    =
    e^{-\integral[p(x)][x]}
    \left(
        C+\integral[q(x)e^{\integral[p(x)][x]}][x]
    \right).
\]
Essa è la somma della soluzione generale dell'omogenea
\[
    Ce^{-\integral[p(x)][x]}
\]
e di una soluzione particolare
\[
    e^{-\integral[p(x)][x]}
    \integral[q(x)e^{\integral[p(x)][x]}][x].
\]
...
\begin{snippetexample}{}
    ...
    \[
        \lambda^2+1=0,
    \]
    quindi un sistema fondamentale di soluzioni è
    \[
        w_1(x)=\cos x,
        \qquad
        w_2(x)=\sin x.
    ...
    \[
        y_\ast(x)=c_1(x)\cos x+c_2(x)\sin x.
    \]
    Il sistema di variazione delle costanti è
    \[
        \begin{cases}
    ...
    \[
        c_1'=-c_2'\frac{\sin x}{\cos x}.
    \]
    Sostituendo nella seconda:
    \begin{align*}
        c_2'\frac{\sin^2 x}{\cos x}+c_2'\cos x
        &=
        \frac{1}{\cos x},\\
        c_2'\frac{\sin^2 x+\cos^2 x}{\cos x}
        &=
        \frac{1}{\cos x}.
    \end{align*}
    Quindi
    ...
    \[
        ...
        c_1=\ln(\cos x),
    \]
    dato che su \(I\) si ha \(\cos x>0\). Pertanto una soluzione particolare è
    \[
        y_\ast(x)=\cos x\ln(\cos x)+x\sin x.
    \]
    L'integrale generale è
    \[
        y(x)=k_1\cos x+k_2\sin x+\cos x\ln(\cos x)+x\sin x.
    \]
\end{snippetexample}

\subsection{Metodo di somiglianza}

Il metodo di somiglianza permette di cercare una soluzione particolare osservando la forma del termine noto \(b(x)\).

Sia
\[
    P(\lambda)=\lambda^k+\sum_{j=0}^{k-1}a_j\lambda^j
\]
il polinomio caratteristico dell'equazione omogenea associata.

\begin{snippetproposition}{Termine noto polinomiale}
    Sia \(b(x)\) un polinomio di grado \(m\).
    \begin{enumerate}
        \item Se \(0\) non è radice di \(P\), allora si cerca una soluzione particolare \(y_\ast\) polinomiale di grado \(m\).
        \item Se \(0\) è radice di \(P\) con molteplicità \(r\), allora si cerca una soluzione particolare della forma
            \[
                y_\ast(x)=x^rQ_m(x),
            \]
            dove \(Q_m\) è un polinomio di grado \(m\).
    \end{enumerate}
\end{snippetproposition}

\begin{snippetproposition}{Termine noto esponenziale}
    Sia
    \[
        b(x)=e^{\mu x}.
    \]
    \begin{enumerate}
        \item Se \(P(\mu)\neq 0\), allora si cerca una soluzione particolare della forma
            \[
                y_\ast(x)=Ce^{\mu x}.
            \]
        \item Se \(\mu\) è radice di \(P\) con molteplicità \(r\), allora si cerca una soluzione particolare della forma
            \[
                y_\ast(x)=Cx^re^{\mu x}.
            \]
    \end{enumerate}
...
\begin{snippetproposition}{Termine noto oscillante}
    Sia
    \[
        b(x)=e^{\alpha x}\left(A\sin(\beta x)+B\cos(\beta x)\right).
    \]
    \begin{enumerate}
        \item Se \(\alpha+i\beta\) non è radice di \(P\), allora si cerca una soluzione particolare della forma
            \[
                y_\ast(x)=e^{\alpha x}\left(C\sin(\beta x)+D\cos(\beta x)\right).
            \]
        \item Se \(\alpha+i\beta\) è radice di \(P\) con molteplicità \(r\), allora si cerca una soluzione particolare della forma
            \[
                y_\ast(x)=x^re^{\alpha x}\left(C\sin(\beta x)+D\cos(\beta x)\right).
            \]
    \end{enumerate}
\end{snippetproposition}

Se il termine noto è una somma, ad esempio
\[
    b(x)=x^2+\cos x-e^{3x},
\]
si sfrutta la linearità dell'equazione: si risolvono separatamente tre equazioni, una per ciascun termine noto, e poi si sommano le soluzioni particolari ottenute.

\begin{snippetexample}{Oscillatore forzato senza smorzamento}
    Consideriamo
    \[
        y''+\omega^2y=\sin(\ell x),
        \qquad
        \omega,\ell>0.
    \]
    L'equazione omogenea associata ha integrale generale
    \[
    ...
    \[
        y_\ast(x)=C\cos(\ell x)+D\sin(\ell x).
    \]
    Allora
    \[
        y_\ast''=-\ell^2y_\ast,
    ...
    perciò
    \[
        y_\ast''+\omega^2y_\ast=(\omega^2-\ell^2)y_\ast.
    \]
    Per ottenere \(\sin(\ell x)\), dobbiamo prendere
    \[
        C=0,
        \qquad
        D=\frac{1}{\omega^2-\ell^2}.
    \]
    Quindi
    \[
        y_\ast(x)=\frac{\sin(\ell x)}{\omega^2-\ell^2}.
    \]
    L'integrale generale è
    \[
        y(x)=c_1\cos(\omega x)+c_2\sin(\omega x)+\frac{\sin(\ell x)}{\omega^2-\ell^2}.
    \]

    \textbf{Caso \(\ell=\omega\).}
    Siamo nel caso di risonanza. Poiché il termine noto ha la stessa frequenza delle soluzioni dell'omogenea, moltiplichiamo per \(x\) e cerchiamo
    \[
        y_\ast(x)=x\left(C\cos(\omega x)+D\sin(\omega x)\right).
    \]
    Sostituendo nell'equazione si ottiene
    \[
        y_\ast''+\omega^2y_\ast
        =
        2D\omega\cos(\omega x)-2C\omega\sin(\omega x).
    \]
    Imponendo
    \[
        2D\omega\cos(\omega x)-2C\omega\sin(\omega x)=\sin(\omega x),
    \]
    si ricava
    \[
        D=0,
    ...
    Dunque
    \[
        y_\ast(x)=-\frac{x}{2\omega}\cos(\omega x).
    \]
    L'integrale generale è
    \[
        y(x)=c_1\cos(\omega x)+c_2\sin(\omega x)-\frac{x}{2\omega}\cos(\omega x).
    \]
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.9]
            ...
            \draw[dashed] (0,0) -- (6,-1.5);
            \node at (5.2,1.7) {ampiezza crescente};
        \end{tikzpicture}
    \end{center}
\end{snippetexample}

\begin{snippetexample}{Termine noto esponenziale-oscillante}
    Consideriamo
    \[
        y''+a_1y'+a_2y=e^{\alpha x}\sin(\beta x).
    \]
    Un modo efficiente per trovare una soluzione particolare è considerare l'equazione complessa associata
    \[
        y''+a_1y'+a_2y=e^{(\alpha+i\beta)x}.
    \]
    Se
    \[
        P(\alpha+i\beta)\neq 0,
    \]
    cerchiamo una soluzione particolare complessa del tipo
    \[
        y_{\ast,c}(x)=Ae^{(\alpha+i\beta)x}.
    \]
    Allora
    \[
        y_{\ast,c}'=A(\alpha+i\beta)e^{(\alpha+i\beta)x},
    \]
    e
    \[
        y_{\ast,c}''=A(\alpha+i\beta)^2e^{(\alpha+i\beta)x}.
    \]
    Sostituendo:
    \[
        A\left[
            (\alpha+i\beta)^2+a_1(\alpha+i\beta)+a_2
        \right]e^{(\alpha+i\beta)x}
        =
        e^{(\alpha+i\beta)x}.
    \]
    Quindi
    ...
    \]
    La soluzione particolare reale dell'equazione con termine noto \(e^{\alpha x}\sin(\beta x)\) è
    \[
        \operatorname{Im}\left(y_{\ast,c}\right).
    \]
    Se invece il termine noto fosse \(e^{\alpha x}\cos(\beta x)\), prenderemmo
    \[
        \operatorname{Re}\left(y_{\ast,c}\right).
    \]
\end{snippetexample}
...
\begin{snippetdefinition}{Equivalenza fra curve}
...
\begin{snippetexample}{}
    ...
    ma non è \(\mathcal{C}^1\) perché se facessi l'inverso della seconda otterrei la radice cubica che non è differenziabile in 0.
\end{snippetexample}

Possiamo quindi sempre supporre che una curva sia parametrizzata su \(([0,1])\).

\begin{snippetdefinition}{Equivalenza orientata}
...
\begin{snippetexample}{In generale non è possibile identificare una curva con il suo sostegno}
...
\begin{snippetproof}{}
    ...
    Osserviamo che \(\gamma \colon [a,b] \fromto \Gamma\) è continua e suriettiva.
...
\begin{radioactive}
\begin{snippetdefinition}{Curva regolare}
...
Sia \(t(x)\) la sua inversa. Possiamo usare questa funzione inversa per
...
Può capitare che una curva non sia regolare per una scelta "sbagliata" della parametrizzazione.
...
\begin{snippetexample}{Curva non rettificabile}
...
\begin{snippetproof}{}
    ...
    Siccome \(\gamma\) è continua, per il teorema di Lagrange
    \[
    ...
    Possiamo quindi applicare l'uniforme continuità all'integranda. Se \(\delta < \delta_\varepsilon\)
    allora
    \begin{align*}
    ...
    con la proprietà che \(|\Pi| < \delta_\varepsilon\), cioè che
    \(|t_k - t_{k-1}| < \delta_\varepsilon\).
    Allora abbiamo
    \begin{align*}
        \integral[t_{k-1}][t_k][
            ||\dot\gamma(\tau)||
...
\begin{snippetproof}{}
    ...
    Così facendo esiste un unico \(s \in [0, L]\) tale che \(p = \gamma(s)\).
...
\begin{snippetdefinition}{Integrale curvilineo}
...
\subsection{Forme differenziali di grado \(1\)}

\begin{snippetdefinition}{Forma differenziale di grado \(1\)}
    Sia \(\Omega\subseteq\realnumbers^n\) un aperto. Una forma differenziale di grado \(1\) su \(\Omega\) è un'applicazione continua
    \[
        \omega:\Omega\to\mathcal{L}(\realnumbers^n,\realnumbers).
    \]
    In coordinate si scrive
    \[
        \omega(x)=F_1(x)\,dx_1+\cdots+F_n(x)\,dx_n,
    \]
    dove
    \[
        F_i\in C^0(\Omega),
        \qquad
        F=(F_1,\ldots,F_n)\in C^0(\Omega,\realnumbers^n).
    \]
    Il campo vettoriale \(F\) è il campo associato alla forma \(\omega\).
\end{snippetdefinition}

Se \(v=(v_1,\ldots,v_n)\in\realnumbers^n\), allora
\[
    \omega(x)(v)=F_1(x)v_1+\cdots+F_n(x)v_n
    =
    F(x)\cdot v.
\]

\begin{snippetdefinition}{Integrale curvilineo di una forma}
    Sia
    \[
        \gamma:[a,b]\to\Omega
    \]
    una curva \(C^1\). L'integrale della forma \(\omega\) lungo \(\gamma\) è
    \[
        \int_\gamma \omega
        =
        \integral[a][b][\omega(\gamma(t))(\gamma'(t))][t].
    \]
    In termini del campo vettoriale associato \(F\), si ha
    \[
        \int_\gamma \omega
        =
        \integral[a][b][F(\gamma(t))\cdot\gamma'(t)][t].
    \]
\end{snippetdefinition}

\begin{center}
    \begin{tikzpicture}[scale=1]
        \draw[thick] (0,0) ellipse (1.9 and 1.4);
        \draw[->,thick] plot[smooth] coordinates {(-0.8,-0.7) (-0.2,-0.2) (0.2,0.3) (0.7,0.8)};
        \node at (-0.95,-0.9) {\(\gamma(a)\)};
        \node at (0.95,0.95) {\(\gamma(b)\)};
        \node at (1.55,1.25) {\(\Omega\)};
        \node at (0.1,0.05) {\(\gamma(t)\)};
    \end{tikzpicture}
\end{center}

\subsection{Forme esatte e forme chiuse}

\begin{snippetdefinition}{Forma esatta}
    Una forma differenziale
    \[
        \omega=F_1\,dx_1+\cdots+F_n\,dx_n
    \]
    si dice esatta in \(\Omega\) se esiste una funzione
    \[
        f\in C^1(\Omega)
    \]
    tale che
    ...
    \[
        ...
        \forall i=1,\ldots,n.
    \]
    In questo caso il campo vettoriale \(F\) si dice conservativo, e \(f\) si chiama potenziale di \(\omega\).
\end{snippetdefinition}

Se \(\omega=df\), allora l'integrale curvilineo di \(\omega\) dipende soltanto dagli estremi della curva. Infatti:
\[
    \int_\gamma \omega
    =
    \int_\gamma df
    =
    \integral[a][b][\nabla f(\gamma(t))\cdot\gamma'(t)][t].
\]
...
\[
    \int_\gamma df
    =
    \integral[a][b][\frac{d}{dt}f(\gamma(t))][t]
    =
    f(\gamma(b))-f(\gamma(a)).
\]
In particolare, se \(\gamma\) è chiusa, cioè \(\gamma(a)=\gamma(b)\), allora
\[
    \int_\gamma df=0.
\]

\begin{snippetdefinition}{Forma chiusa}
    Sia
    \[
        \omega=F_1\,dx_1+\cdots+F_n\,dx_n
    \]
    una forma differenziale di classe \(C^1\). Si dice che \(\omega\) è chiusa se
    \[
...
Nel linguaggio dei campi vettoriali, dire che \(\omega\) è chiusa corrisponde a dire che il campo vettoriale associato è irrotazionale.

\begin{snippetproposition}{Ogni forma esatta è chiusa}
    Sia \(\Omega\subseteq\realnumbers^n\) aperto. Se
    \[
        \omega=df
    \]
    con \(f\in C^2(\Omega)\), allora \(\omega\) è chiusa.
\end{snippetproposition}
...
\[
    ...
    \omega \text{ chiusa}.
\]
L'implicazione inversa non è sempre vera: dipende dalla forma del dominio \(\Omega\).

\begin{snippetexample}{Una forma chiusa ed esatta}
    Consideriamo in \(\realnumbers^2\) la forma
    \[
        \omega(x,y)=2xy\,dx+x^2\,dy.
    \]
    Verifichiamo che è chiusa:
    \[
    ...
    Cerchiamo ora un potenziale \(f\), cioè una funzione tale che
    \[
        df=\omega.
    \]
    ...
    \[
        \begin{cases}
            \dfrac{\partial f}{\partial x}=2xy,\\[4pt]
            \dfrac{\partial f}{\partial y}=x^2.
        \end{cases}
    \]
    Integrando la prima equazione rispetto a \(x\), otteniamo
    \[
        f(x,y)=x^2y+\alpha(y),
    \]
    dove \(\alpha\) dipende soltanto da \(y\). Derivando rispetto a \(y\):
    \[
    ...
    \[
        \alpha'(y)=0.
    \]
    Quindi \(\alpha\) è costante e un potenziale è
    \[
        f(x,y)=x^2y.
    \]
    Pertanto
...
\begin{snippetexample}{Una forma chiusa ma non esatta}
    Consideriamo
    \[
        \Omega=\realnumbers^2\setminus\{(0,0)\}
    \]
    e la forma
    \[
        \omega(x,y)=
    ...
    \]
    Verifichiamo che \(\omega\) è chiusa:
    \begin{align*}
        \frac{\partial F_1}{\partial y}
        &=
    ...
    Se proviamo a cercare un potenziale, dobbiamo imporre
    \[
        ...
        \frac{x}{x^2+y^2}.
    \]
    Integrando la seconda equazione rispetto a \(y\), otteniamo, dove \(x\neq 0\),
    \[
        \frac{x}{x^2+y^2}
        =
    ...
    Quindi localmente
    \[
        f(x,y)=\arctan\left(\frac{y}{x}\right)+\alpha(x).
    \]
    Derivando rispetto a \(x\):
    \[
        \frac{\partial f}{\partial x}
        =
        -\frac{y}{x^2+y^2}+\alpha'(x).
    \]
    Pertanto \(\alpha'(x)=0\). Localmente, quindi, il potenziale è
    \[
        f(x,y)=\arctan\left(\frac{y}{x}\right)+c.
    \]

    Questo però non definisce un potenziale globale su tutto \(\Omega\). Infatti il dominio \(\Omega\) è bucato, e la funzione angolo non può essere scelta in modo continuo e globale su tutto il piano bucato.
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1]
            ...
            \node at (1.35,1.1) {\(\gamma\)};
            \node at (-0.4,-0.25) {\((0,0)\)};
        \end{tikzpicture}
    \end{center}

    Per dimostrare che \(\omega\) non è esatta su \(\Omega\), consideriamo la curva chiusa
    \[
        \gamma(t)=(\cos t,\sin t),
        \qquad
        t\in[0,2\pi].
    ...
    \begin{align*}
        ...
        &=
        \integral[0][2\pi][\sin^2t+\cos^2t][t]\\
        &=
        \integral[0][2\pi][1][t]\\
        &=
        2\pi.
    \end{align*}
    Poiché l'integrale lungo una curva chiusa non è nullo, \(\omega\) non può essere esatta in \(\Omega\).
\end{snippetexample}

\subsection{Caratterizzazione delle forme esatte}

\begin{snippetdefinition}{Dominio connesso per archi}
    Un insieme \(\Omega\subseteq\realnumbers^n\) si dice connesso per archi se per ogni \(P,Q\in\Omega\) esiste una curva continua
    \[
        \gamma:[0,1]\to\Omega
    \]
    tale che
...
\begin{snippettheorem}{Caratterizzazione delle forme esatte}
    Sia \(\Omega\subseteq\realnumbers^n\) un aperto connesso per archi e sia
    \[
        \omega\in C^1(\Omega,(\realnumbers^n)^\ast).
    \]
    Le seguenti affermazioni sono equivalenti:
    \begin{enumerate}
        \item \(\omega\) è esatta in \(\Omega\);
        \item per ogni coppia di curve \(\gamma,\varphi\) contenute in \(\Omega\) e aventi gli stessi estremi, vale
            \[
                \int_\gamma \omega=\int_\varphi \omega;
            \]
        \item per ogni curva chiusa \(\gamma\) contenuta in \(\Omega\), vale
            \[
                \int_\gamma \omega=0.
            \]
    \end{enumerate}
    ...
    \begin{itemize}
        \item \(\text{(1)}\implies\text{(2)}\).
            ...
            \[
                ...
                f(\gamma(b))-f(\gamma(a)).
            \]
            Quindi l'integrale dipende soltanto dagli estremi della curva. Se \(\gamma\) e \(\varphi\) hanno gli stessi estremi, allora
            \[
                \int_\gamma \omega=\int_\varphi \omega.
            \]

        \item \(\text{(1)}\implies\text{(3)}\).
            Se \(\gamma\) è chiusa, allora \(\gamma(a)=\gamma(b)\). Perciò
            \[
                \int_\gamma \omega
                =
                f(\gamma(b))-f(\gamma(a))
        ...
        \item \(\text{(2)}\implies\text{(3)}\).
            Sia \(\gamma\) una curva chiusa. Scegliamo due punti \(P,Q\) sul sostegno di \(\gamma\), e spezziamo \(\gamma\) in due curve:
            \[
                \gamma=\varphi+\psi,
            \]
            dove \(\varphi\) va da \(P\) a \(Q\), mentre \(\psi\) va da \(Q\) a \(P\). La curva \(-\psi\) va da \(P\) a \(Q\), dunque, per ipotesi,
            \[
                \int_\varphi \omega=\int_{-\psi}\omega.
            \]
            Ma
            \[
                \int_{-\psi}\omega=-\int_\psi\omega.
            \]
            Quindi
            \[
                \int_\gamma \omega
            ...
            \begin{center}
                \begin{tikzpicture}[scale=1]
                    ...
                    \draw[->,thick] plot[smooth] coordinates {(-1.2,-0.2) (-0.5,0.7) (0.7,0.8) (1.3,0.1)};
                    \draw[->,thick] plot[smooth] coordinates {(1.3,0.1) (0.4,-0.9) (-0.7,-0.8) (-1.2,-0.2)};
                    \node[left] at (-1.2,-0.2) {\(P\)};
                    \node[right] at (1.3,0.1) {\(Q\)};
                    \node at (0,1.1) {\(\varphi\)};
                    \node at (0,-1.05) {\(\psi\)};
                \end{tikzpicture}
            \end{center}

        \item \(\text{(3)}\implies\text{(2)}\).
            Siano \(\gamma\) e \(\varphi\) due curve da \(P\) a \(Q\). Allora
            \[
                \gamma+(-\varphi)
            \]
            è una curva chiusa. Per ipotesi,
            \[
                0
                =
        ...
        \item \(\text{(2)}\implies\text{(1)}\).
            Fissiamo un punto \(P\in\Omega\). Per ogni \(Q\in\Omega\), scegliamo una curva \(\gamma\) che collega \(P\) a \(Q\) e definiamo
            \[
                f(Q)=\int_\gamma\omega.
            \]
            Per l'ipotesi di indipendenza dal cammino, \(f(Q)\) è ben definita.

            Mostriamo che
            \[
                df=\omega.
            \]
            Sia \(x\in\Omega\) e sia \(v\in\realnumbers^n\). Per \(h\) sufficientemente piccolo, il segmento
            \[
                [x,x+hv]
            \]
            è contenuto in \(\Omega\). Sia \(\gamma\) una curva da \(P\) a \(x\). Allora una curva da \(P\) a \(x+hv\) è ottenuta concatenando \(\gamma\) con il segmento \([x,x+hv]\). Quindi
            \begin{align*}
                \frac{f(x+hv)-f(x)}{h}
                &=
                \frac{1}{h}
                \left[
                    \int_{\gamma+[x,x+hv]}\omega-\int_\gamma\omega
                \right]\\
                &=
                \frac{1}{h}\int_{[x,x+hv]}\omega.
            \end{align*}
            Parametrizziamo il segmento con
            \[
                \varphi(t)=x+thv,
                \qquad
                t\in[0,1].
            ...
            \begin{align*}
                ...
                &=
                \frac{1}{h}
                \integral[0][1][\omega(x+thv)(hv)][t]\\
                &=
                \integral[0][1][\omega(x+thv)(v)][t].
            \end{align*}
            Passando al limite per \(h\to 0\), usando la continuità di \(\omega\), otteniamo
            \[
                D_vf(x)=\omega(x)(v).
            \]
            Poiché questo vale per ogni \(v\in\realnumbers^n\), segue che
            \[
                df=\omega.
            \]
    \end{itemize}
...
Il teorema precedente non dà sempre un metodo comodo per stabilire se una forma è esatta. Tuttavia, quando sappiamo già che una forma è esatta, fornisce un metodo per calcolare un potenziale: fissato un punto \(P\), si definisce
\[
    f(Q)=\int_\gamma\omega,
\]
dove \(\gamma\) è una qualunque curva che collega \(P\) a \(Q\).

\subsection{Connessione e connessione per archi}

\begin{snippetproposition}{Connessione per archi e connessione}
    Se \(\Omega\subseteq\realnumbers^n\) è connesso per archi, allora \(\Omega\) è connesso.
\end{snippetproposition}

Il viceversa non è vero in generale. Esistono insiemi connessi che non sono connessi per archi. Un esempio classico è il cosiddetto pettine topologico, che è connesso ma non connesso per archi. Invece, per aperti di \(\realnumbers^n\), le due nozioni coincidono.

\begin{snippettheorem}{Aperti connessi}
    Sia \(\Omega\subseteq\realnumbers^n\) un aperto connesso. Allora \(\Omega\) è connesso per archi.
\end{snippettheorem}

\begin{snippetproof}{}
    Fissiamo \(x_0\in\Omega\) e definiamo
    \[
        \Omega_{x_0}
        =
        \{x\in\Omega:\text{ esiste una curva in }\Omega\text{ che collega }x_0\text{ a }x\}.
    \]
    Chiaramente
    \[
        x_0\in\Omega_{x_0},
    ...
    Mostriamo che \(\Omega_{x_0}\) è aperto. Se \(x\in\Omega_{x_0}\), siccome \(\Omega\) è aperto, esiste \(r>0\) tale che
    \[
        B_r(x)\subseteq\Omega.
    \]
    Se \(y\in B_r(x)\), allora il segmento \([x,y]\) è contenuto in \(B_r(x)\), quindi in \(\Omega\). Concatenando una curva da \(x_0\) a \(x\) con il segmento \([x,y]\), otteniamo una curva da \(x_0\) a \(y\). Dunque
    \[
        y\in\Omega_{x_0}.
    \]
    Quindi \(\Omega_{x_0}\) è aperto.

    Mostriamo ora che \(\Omega\setminus\Omega_{x_0}\) è aperto. Sia
    \[
        y\in\Omega\setminus\Omega_{x_0}.
    \]
    Essendo \(\Omega\) aperto, esiste \(r>0\) tale che
    \[
        B_r(y)\subseteq\Omega.
    \]
    Se esistesse \(z\in B_r(y)\intersection\Omega_{x_0}\), allora potremmo collegare \(x_0\) a \(z\) e poi \(z\) a \(y\) tramite il segmento \([z,y]\). Questo darebbe una curva da \(x_0\) a \(y\), contro il fatto che
    \[
        y\notin\Omega_{x_0}.
    \]
    Quindi
    \[
        B_r(y)\subseteq\Omega\setminus\Omega_{x_0},
    \]
    e anche \(\Omega\setminus\Omega_{x_0}\) è aperto.

    Abbiamo scritto \(\Omega\) come unione disgiunta di due aperti:
    \[
        \Omega=\Omega_{x_0}\union(\Omega\setminus\Omega_{x_0}).
    \]
    Poiché \(\Omega\) è connesso e \(\Omega_{x_0}\neq\emptyset\), deve essere
    \[
        \Omega=\Omega_{x_0}.
    \]
    Dunque ogni punto di \(\Omega\) può essere collegato a \(x_0\), e quindi \(\Omega\) è connesso per archi.
\end{snippetproof}

\subsection{Domini stellati e convessi}

\begin{snippetdefinition}{Dominio stellato}
    Sia \(\Omega\subseteq\realnumbers^n\). L'insieme \(\Omega\) si dice stellato rispetto a un punto \(x_0\in\Omega\) se per ogni \(x\in\Omega\) il segmento
    \[
        [x_0,x]
    \]
    è contenuto in \(\Omega\).
\end{snippetdefinition}

\begin{center}
    \begin{tikzpicture}[scale=1]
        \draw[thick] plot[smooth cycle] coordinates {
            (0,0) (1.4,0.6) (2.2,0.1) (2.8,0.9) (2.1,1.7)
            (1.2,1.5) (0.4,2.1) (-0.4,1.2)
        };
        \fill (1.1,1) circle (1.4pt) node[below] {\(x_0\)};
        \foreach \p in {(2.3,0.6),(2.0,1.5),(0.1,1.5),(0.5,0.3)}{
            \draw[thin] (1.1,1) -- \p;
        }
        \node at (2.8,1.7) {stellato};
    \end{tikzpicture}
\end{center}

\begin{snippetdefinition}{Dominio convesso}
    Sia \(\Omega\subseteq\realnumbers^n\). L'insieme \(\Omega\) si dice convesso se per ogni \(x,y\in\Omega\) il segmento
    \[
        [x,y]
    \]
    è contenuto in \(\Omega\).
...
Ogni insieme convesso è stellato rispetto a ogni suo punto. Il viceversa non è vero: un dominio può essere stellato rispetto a un punto, ma non contenere tutti i segmenti che congiungono due suoi punti qualunque.

\begin{center}
    \begin{tikzpicture}[scale=1]
        \draw[thick] (-0.2,0) rectangle (2,1.2);
        \fill (0.4,0.4) circle (1.3pt) node[below] {\(x\)};
        \fill (1.6,0.9) circle (1.3pt) node[above] {\(y\)};
        \draw[thick] (0.4,0.4) -- (1.6,0.9);
        \node at (0.9,-0.35) {convesso};
    \end{tikzpicture}
    \qquad
    \begin{tikzpicture}[scale=1]
        \draw[thick] plot[smooth cycle] coordinates {
            (0,0) (1.1,0.3) (2.1,-0.1) (1.5,0.8) (2.2,1.7)
            (1.0,1.3) (0,1.7) (0.5,0.8)
        };
        \fill (0.9,0.8) circle (1.3pt) node[below] {\(x_0\)};
        \node at (1,-0.45) {stellato non convesso};
    \end{tikzpicture}
\end{center}

\subsection{Lemma di Poincaré su domini stellati}

\begin{snippettheorem}{Lemma di Poincaré per domini stellati}
    Sia \(\Omega\subseteq\realnumbers^n\) un aperto stellato e sia
    \[
        \omega\in C^1(\Omega,(\realnumbers^n)^\ast)
    \]
    una forma chiusa. Allora \(\omega\) è esatta in \(\Omega\).
\end{snippettheorem}

\begin{snippetcorollary}{Lemma di Poincaré locale}
    Ogni forma chiusa è localmente esatta.
\end{snippetcorollary}

\begin{snippetproof}{Del corollario}
    Sia \(P\in\Omega\). Poiché \(\Omega\) è aperto, esiste una palla \(B_r(P)\subseteq\Omega\). Ogni palla è convessa, quindi stellata. Per il lemma di Poincaré su domini stellati, la restrizione di \(\omega\) a \(B_r(P)\) è esatta.
\end{snippetproof}

\begin{snippetproof}{Del lemma di Poincaré}
    Per semplicità supponiamo che \(\Omega\) sia stellato rispetto all'origine. Il caso generale si ottiene tramite una traslazione.
    ...
    \]
    Per ogni \(x\in\Omega\), il segmento \([0,x]\) è contenuto in \(\Omega\). Definiamo
    \[
        f(x)=\int_{[0,x]}\omega.
    \]
    Parametrizziamo il segmento con
    ...
    \[
        ...
        =
        \integral[0][1][
            F_1(tx)x_1+\cdots+F_n(tx)x_n
        ][t].
    \]

    Mostriamo che \(df=\omega\). Calcoliamo, ad esempio, la derivata parziale rispetto a \(x_1\):
    \begin{align*}
        \frac{\partial f}{\partial x_1}(x)
        &=
        \integral[0][1][
            \frac{\partial F_1}{\partial x_1}(tx)t x_1
            +
            F_1(tx)
            +
            \frac{\partial F_2}{\partial x_1}(tx)t x_2
            +
            \cdots
            +
            \frac{\partial F_n}{\partial x_1}(tx)t x_n
        ][t].
    \end{align*}
    Poiché \(\omega\) è chiusa,
    \[
    ...
    \begin{align*}
        ...
        \integral[0][1][
            t
            \sum_{j=1}^n
            \frac{\partial F_1}{\partial x_j}(tx)x_j
            +
            F_1(tx)
        ][t].
    \end{align*}
    ...
    \begin{align*}
        ...
        &=
        \integral[0][1][
            t\frac{d}{dt}F_1(tx)+F_1(tx)
        ][t]\\
        &=
        \integral[0][1][
            \frac{d}{dt}\left(tF_1(tx)\right)
        ][t]\\
        &=
        \left[tF_1(tx)\right]_{0}^{1}\\
        &=
        F_1(x).
    \end{align*}
    ...
    \[
        df=\omega.
    \]
    Dunque \(\omega\) è esatta.
\end{snippetproof}

\subsection{Forme chiuse, forme esatte e topologia del dominio}

Ricordiamo che, per una forma differenziale di grado \(1\),
\[
    \omega=F_1\,dx_1+\cdots+F_n\,dx_n,
\]
vale sempre l'implicazione
\[
    \omega \text{ esatta}
    \implies
    \omega \text{ chiusa}.
\]
Infatti, se \(\omega=df\), allora \(F_i=\frac{\partial f}{\partial x_i}\), e la chiusura segue dal teorema di Schwarz:
\[
    \frac{\partial F_i}{\partial x_j}
    =
    \frac{\partial^2 f}{\partial x_j\partial x_i}
...
L'implicazione inversa non è sempre vera:
\[
    \omega \text{ chiusa}
    \centernot\implies
    \omega \text{ esatta}.
\]
Il problema non dipende solo dalla forma \(\omega\), ma anche dalla geometria del dominio \(\Omega\).

Se \(\Omega\) è un aperto connesso di \(\realnumbers^n\), il fatto che le forme chiuse in \(\Omega\) siano esatte dipende dalla topologia di \(\Omega\). Per esempio:
\[
    \Omega \text{ stellato}
    \implies
    \text{ogni forma chiusa in } \Omega \text{ è esatta}.
\]
Più in generale, una condizione topologica sufficiente è la semplice connessione.

\begin{snippetdefinition}{Dominio semplicemente connesso}
    Sia \(\Omega\subseteq\realnumbers^n\). Diciamo che \(\Omega\) è semplicemente connesso se:
    \begin{enumerate}
        \item \(\Omega\) è connesso per archi;
        \item ogni curva chiusa contenuta in \(\Omega\) può essere contratta con continuità a un punto restando sempre dentro \(\Omega\).
    \end{enumerate}
\end{snippetdefinition}

Intuitivamente, un dominio è semplicemente connesso quando non ha buchi che impediscono di contrarre una curva chiusa a un punto.
...
\begin{center}
    \begin{tikzpicture}[scale=0.95]
        ...
        \draw[->,thick] (0.8,0) arc[start angle=0,end angle=70,radius=0.8];
        \node at (0,-2.1) {\(\realnumbers^2\setminus\{(0,0)\}\) non è semplicemente connesso};

        \begin{scope}[xshift=5.2cm]
            \draw[thick] plot[smooth cycle] coordinates {
                (-1.5,-0.4) (-0.8,0.8) (0.2,1.1) (1.3,0.6)
                (1.5,-0.5) (0.4,-1.1) (-0.7,-0.9)
            };
            \draw[thick] plot[smooth cycle] coordinates {
                (-0.6,-0.2) (-0.2,0.3) (0.4,0.2) (0.6,-0.3)
                (0,-0.5)
            };
            \draw[->,thick] (0.6,-0.3) -- (0.35,-0.43);
            \node at (0,-2.1) {curva contraibile};
        \end{scope}
    \end{tikzpicture}
\end{center}

\begin{snippetexample}{Esempi}
    Valgono i seguenti fatti:
    \begin{enumerate}
        \item \(\realnumbers^2\) è semplicemente connesso;
        \item \(\realnumbers^2\setminus\{(0,0)\}\) non è semplicemente connesso;
        \item \(\realnumbers^3\setminus\{(0,0,0)\}\) è semplicemente connesso;
        \item \(\realnumbers^3\setminus r\), dove \(r\) è una retta, non è semplicemente connesso;
        \item \(S^2\) è semplicemente connesso;
        \item il toro non è semplicemente connesso.
    \end{enumerate}
\end{snippetexample}

\begin{snippettheorem}{Forme chiuse su domini semplicemente connessi}
    Sia \(\Omega\subseteq\realnumbers^n\) un aperto semplicemente connesso. Allora ogni forma differenziale chiusa di classe \(C^1\) in \(\Omega\) è esatta.
\end{snippettheorem}

\subsection{La forma angolare nel piano bucato}
...
\[
    \Omega=\realnumbers^2\setminus\{(0,0)\}.
\]
Abbiamo già visto che \(\omega\) è chiusa ma non è esatta in \(\Omega\).

Infatti, lungo la circonferenza unitaria
\[
    \gamma(t)=(\cos t,\sin t),
    \qquad t\in[0,2\pi],
\]
...
\[
    \int_\gamma \omega
    =
    \integral[0][2\pi][\sin^2 t+\cos^2 t][t]
    =
    2\pi.
\]
Perciò l'integrale lungo una curva chiusa non è nullo, e dunque \(\omega\) non può essere esatta su tutto il piano bucato.

In realtà, l'integrale
\[
    \frac{1}{2\pi}\int_\gamma\omega
\]
misura l'indice della curva chiusa \(\gamma\) rispetto all'origine, cioè il numero di giri orientati che \(\gamma\) compie attorno al punto \((0,0)\).

\subsection{Un potenziale locale della forma angolare}
...
\[
    \Omega
    =
    \realnumbers^2\setminus\{(0,y):y\leq 0\}.
\]
Questo dominio è il piano privato della semiretta verticale negativa. Esso è stellato rispetto al punto \((0,1)\), quindi è semplicemente connesso. Poiché \(\omega\) è chiusa, segue che \(\omega\) è esatta in \(\Omega\).

Vogliamo costruire esplicitamente un potenziale. Fissiamo il punto
\[
    P=(0,1)
\]
e, dato
\[
    Q=(x_0,y_0)\in\Omega,
\]
colleghiamo \(P\) a \(Q\) con un cammino spezzato:
\[
    P=(0,1)
    \longrightarrow
    (x_0,1)
...
\begin{center}
    \begin{tikzpicture}[scale=1]
        ...
        \draw[very thick] (0,0) -- (0,-1.45);
        \node[right] at (0,-1.25) {\(\text{taglio}\)};

        \fill (0,1) circle (1.5pt) node[left] {\((0,1)\)};
        \fill (1.55,1) circle (1.5pt) node[below] {\((x_0,1)\)};
        \fill (1.55,1.65) circle (1.5pt) node[right] {\((x_0,y_0)\)};

        \draw[->,thick] (0,1) -- (1.55,1);
        \draw[->,thick] (1.55,1) -- (1.55,1.65);
    \end{tikzpicture}
\end{center}

Definiamo
\[
    f(x_0,y_0)=\int_\gamma\omega,
\]
dove \(\gamma\) è il cammino spezzato appena descritto.

Sul primo tratto si ha \(y=1\) e \(dy=0\), dunque
\[
    \int_{\gamma_1}\omega
    =
    \integral[0][x_0][-\frac{1}{x^2+1}][x]
    =
    -\arctan x_0.
\]
Sul secondo tratto si ha \(x=x_0\) e \(dx=0\), dunque
\[
    \int_{\gamma_2}\omega
    =
    \integral[1][y_0][
...
\]
Se \(x_0=0\), allora questo integrale è nullo, quindi
\[
    f(0,y_0)=0
    \qquad
    \forall y_0>0.
...
\[
    ...
    -\arctan x_0
    +
    \arctan\left(\frac{y_0}{x_0}\right)
    -
    \arctan\left(\frac{1}{x_0}\right).
\]
Poniamo
\[
    g(x)=\arctan x+\arctan\left(\frac{1}{x}\right).
\]
Allora
\[
    g'(x)
    ...
    \frac{1}{1+x^2}
    +
    \frac{1}{1+\left(\frac{1}{x}\right)^2}
...
\[
    ...
    \end{cases}
\]
Pertanto un potenziale di \(\omega\) in \(\Omega\) è
\[
    f(x,y)=
    \begin{cases}
        \arctan\left(\dfrac{y}{x}\right)+\dfrac{\pi}{2}, & x<0,\\[8pt]
        0, & x=0,\ y>0,\\[8pt]
        \arctan\left(\dfrac{y}{x}\right)-\dfrac{\pi}{2}, & x>0.
    \end{cases}
\]
La funzione è continua nei punti dell'asse \(y\) con \(y>0\), perché
\[
    \lim_{\substack{x\to 0^-\\ y>0}}
    \left[
        \arctan\left(\frac{y}{x}\right)+\frac{\pi}{2}
    \right]
    =
    0
\]
...
\subsection{Lavoro di un campo conservativo}

\begin{snippetexample}{}
    Calcoliamo il lavoro del campo
    \[
        F(x,y)=xy\,\vec{\imath}+\frac{x^2}{2}\,\vec{\jmath}
    \]
    lungo la curva
    \[
        \gamma(t)=
        \left(
            t,
            \arctan\left(
                \frac{1-\cos(3t)}{2-\sin^2 t}
            \right)
        \right),
        \qquad 0\leq t\leq \pi.
    \]

    Il campo \(F\) è definito su tutto \(\realnumbers^2\), che è semplicemente connesso. Inoltre
    \[
        \frac{\partial}{\partial y}(xy)=x,
        \qquad
        \frac{\partial}{\partial x}\left(\frac{x^2}{2}\right)=x.
    \]
    Quindi la forma associata
    \[
        \omega=xy\,dx+\frac{x^2}{2}\,dy
    \]
    è chiusa, e dunque è esatta.

    Cerchiamo un potenziale \(f\). Deve valere
    \[
        \frac{\partial f}{\partial x}=xy,
        \qquad
        \frac{\partial f}{\partial y}=\frac{x^2}{2}.
    \]
    Integrando la seconda equazione rispetto a \(y\), otteniamo
    \[
        f(x,y)=\frac{x^2}{2}y+\alpha(x).
    \]
    Derivando rispetto a \(x\):
    \[
        \frac{\partial f}{\partial x}
        ...
        xy+\alpha'(x).
    \]
    Imponendo \(\frac{\partial f}{\partial x}=xy\), segue
    \[
        \alpha'(x)=0.
    \]
    ...
    Per il teorema fondamentale per gli integrali curvilinei,
    \[
    ...
    \[
        ...
        \left(
            \pi,
            \arctan\left(\frac{1-\cos(3\pi)}{2-\sin^2\pi}\right)
        \right)
        =
        \left(\pi,\arctan 1\right)
        =
        \left(\pi,\frac{\pi}{4}\right).
    \]
    Quindi
    \[
        \int_\gamma F\cdot d\gamma
        =
        f\left(\pi,\frac{\pi}{4}\right)-f(0,0)
        =
...
Definendo gli integrali doppi
otteniamo come bonus il fatto che possiamo definire l'area di un dominio
piano.
Quando generalizziamo l'idea di funzioni a scala, usando partizioni
di dominio di dimensioni superiori, abbiamo il problema di sforare fuori dal bordo.

\begin{radioactive}
\begin{snippetdefinition}{}
    Sia \(Q = [a,b] \times [c,d]\) un rettangolo
    e sia \(f \colon Q \fromto \realnumbers\) limitata
    in \(Q\).
    Siano \(\Pi_X = \{a = x_0, < x_1 < \cdots < x_n = b\}\)
    e \(\Pi_Y = \{c = y_0, < y_1 < \cdots < y_m = c\}\) due partizioni.
    Sia quindi \(\Pi = \Pi_X \times \Pi_Y\) una partizione di \(Q\).
    Denotiamo \(x_h\) e \(y_k\) le coordinate dei rettangoli delle rispettive variabili, quindi
    abbiamo \(n \times m\) rettangolini
    \[
    ...
    \[
        ...
        M_{hk} \triangleq \sup_{Q_{hk}}
    \]
    e \[
        |Q_{hk}| = (x_h - x_{h-1}) (y_k - y_{k-1})
    \]
    l'area di un rettangolino.
    Definiamo quindi
    \[
        s(\Pi) \triangleq \sum_{h,k} |Q_{hk}| m_{hk}, \quad
        S(\Pi) \triangleq \sum_{h,k} |Q_{hk}| M_{hk}
    \]
    Definiamo l'integrale superiore e inferiore
    \[
        \lriint_Q f(x,y)\,\text{d}x\,\text{dy} \triangleq
        \sup_\Pi s(\Pi), \quad \uriint_Q f(x,y)\,\text{d}x\,\text{dy} \triangleq
        \inf_\Pi S(\Pi)
    \]
    Se i due coincidono allora diciamo che \(f\) è Riemann-integrabile in \(Q\).
\end{snippetdefinition}
\end{radioactive}

\begin{radioactive}
Chiaramente la funzione di Dirichlet generalizzata
\[
    f(x,y) = \begin{cases}
        1 & x,y \in \rationalnumbers \intersection [0,1] \\
        0 & \text{otherwise}
    \end{cases}
\]
...
\begin{snippetproposition}{}
    Se \(\forall \varepsilon > 0\), esiste partizione \(\Pi\)
    tale che \(S(\Pi) s(\Pi) < \varepsilon\), allora \(f\) è integrabile.
\end{snippetproposition}

\begin{snippettheorem}{}
    Se \(f \colon Q \fromto \realnumbers\) è continua, allora è integrabile in \(Q\).
\end{snippettheorem}

\begin{snippetproof}{}
    \(f\) è continua in \(Q\) (compatto) quindi è uniformemente continua,
    cioè \(\forall \varepsilon > 0\), esiste \(\delta > 0\)
    tale che
    \[
        ||(x_1,y_1) - (x_2, y_2)|| < \delta \implies |f(x_1,y_2) - f(x_2, y_2)| < \varepsilon
    \]
    Basta scegliere \(\Pi\) in modo tale che \(\text{diam} Q_{hk} < \delta\)
    per tutti gli \(h,k\). Abbiamo quindi
    \begin{align*}
        S(\Pi)- s(\Pi)
        &= \sum_{h,k} |Q_{h,k}| (M_{hk} - m_{hk})
        \leq \varepsilon \sum_{h,k} |Q_{hk}| = |Q|\varepsilon \to 0
    \end{align*}
\end{snippetproof}

\begin{snippetdefinition}{}
    Sia \(Z \subseteq \realnumbers^2\) limitato.
    Diciamo che \(Z\) è trascurabile (di misura nulla)
    se \(\forall \varepsilon > 0\) esistono \(Q_1, \cdots, Q_n\) rettangoli tali che
    \begin{enumerate}
        \item \(Z \subseteq \bigcup Q_i\);
        \item \(\sum |Q_i| < \varepsilon\)
    \end{enumerate} % su tutti i h,k tali che Qhk intersection Z neq emptyset
\end{snippetdefinition}

Non è quella di Lebesgue. Un insieme finito è trascurabile.

Consideriamo ora un segmento, per esempio parallelo alle ascisse.
Anche questo è trascurabile in quanto è ricopribile da un solo rettangolo
di altezza \(\varepsilon\). Anche se il segmento è orientato
in un'altra direzione, possiamo considerare
un ricoprimento di quadratini di area \(\varepsilon\),
di cui quindi me ne servono \(L / (\sqrt{2}\varepsilon)\)
dove \(L\) è la lunghezza. Quindi ne servono \(N \sim \frac{1}{\varepsilon}\)
e quindi l'area è \(\sim \frac{1}{\varepsilon}\varepsilon^2 \to 0\).

Il grafico di una funzione \(y = f(x)\) non è in generale trascurabile.
Se è Riemann-integrabile su \([a,b]\), lo è di sicuro.
In tal caso, possiamo considerare funzioni a scala arbitrariamente vicine
al grafico, quindi \(S(\Pi)- s(\Pi) < \varepsilon\).
Possiamo costruire dei rettangolini usando le differenza fra
le funzioni a scala superiori e inferiori
\[
    \sum_j (x_j, x_{j-1})(M_j - m_j) < \varepsilon
\]
e i rettangolini contengono il grafico di \(f\), quindi è trascurabile.

Se \(Z\) è un insieme con interno non vuoto, allora non è trascurabile.
In tal caso contiene almeno una bolla.

Nella definizione di insieme trascurabile
richiedo che \(Z \subseteq \bigcup Q_i\).
In generale però non deve essere racchiuso nell'interno.
Non lede la generalità se richiediamo che \(Z\) sia contenuto nell'interno
di \(\bigcup Q_i\), in quanto possiamo sempre aumentare leggermente le grandezze dei rettangoli
(stesso centro ma raddoppiando i lati e \(|2Q_i| = 4|Q_i|\)).
Possiamo quindi assumere ciò d'ora in avanti e ci verrà comodo.
Inoltre non richiediamo che i rettangoli provengano da una partizione prodotto,
e possono appunto sovrapporsi.
Senza ledere la generalità possiamo supporre che derivino da una partizione prodotto.
Infatti, basta aumentare il numero di rettangoli per separare i punto di overlap.
Quindi due rettangoli che si sovrappongono possono diventare 7 nuovi rettangoli.
In questo modo, la somma delle aree diminuisce in quanto non conto molteplici volte gli overlap,
ma l'area totale decresce quindi va bene comunque.

\begin{snippetexample}{}
    Sia \[
        Z = \{(x,y) \in [0,1] \times [0,1] \suchthat x,y \in \rationalnumbers\}
    \]
    Questo insieme non è trascurabile in quanto, se consideriamo una partizione
    prodotto, la partizione deve coprire tutto \([0,1] \times [0,1]\).
    Ma quindi l'area totale è \(1\), quindi non è trascurabile.
    Infatti \(\partial Z = [0,1] \times [0,1]\).
\end{snippetexample}

\begin{snippettheorem}{}
    Sia \(f \colon Q \fromto \realnumbers\) limitata e sia \[D_f = \{(x,y) \in Q \suchthat f \text{ è discontinua nel punto }\}\]
    Se \(\mu(D_f) = 0\) allora \(f\) è integrabile.
\end{snippettheorem}

\begin{snippetproof}{}
    Fissiamo \(\varepsilon > 0\) e costruiamo una partizione prodotto. I punti
    di \(D_f\) sono ricoperti da alcuni rettangoli della partizione, e siccome
    \(\mu(D_f) = 0\) possiamo scegliere \(\Pi\) in modo che la somma delle aree
    di questi rettangoli sia minore di \(\varepsilon\).
    Senza perdita di generalità supponiamo che i punti di discontinuità siano
    contenuti negli interni dei rettangolini che li contengono.
    Nei rettangolini che non contengono punti di discontinuità, la funzione è continua.
    Sui rettangoli che contengono punti di \(D_f\), \(f\) è limitata (come ovunque)
    e quindi
    \begin{align*}
        \sum_{\substack{h,k \\ \bigcup Q_{h,k} \supseteq D_f}} &\leq
        \sum_{\substack{h,k \\ \bigcup Q_{h,k} \supseteq D_f}} 2 \sup_Q(|f|) \\
        &\leq 2 \sup_Q(|f|) \varepsilon
    \end{align*}
    Il contributo della differenza fra la somma superiore e inferiore è quindi trascurabile.
    Consideriamo ora il contributo a \(S(\Pi) - s(\Pi)\) dovuto ai rettangoli
    in cui \(f\) è continua.
    Siccome l'insieme di questi rettangoli è compatto e \(f\) è continua,
    allora \(f\) è uniformemente continua.
    Raffinando eventualmente la partizione, possiamo supporre che \(\text{diam}Q_{h,k} < \delta\).
    Per lo stesso ragionamento utilizzato in precedenza
    \(S(\Pi) - s(\Pi) \leq \mu(Q) \varepsilon\),
    quindi in totale \[S(\Pi) - s(\Pi) \leq 2 \sup_Q(|f|) \varepsilon + \varphi\mu(Q) \to 0\]
\end{snippetproof}

Sia \(Q = [a,b] \times [c,d]\) e sia \(f \colon Q \fromto \realnumbers\)
integrabile. Vogliamo stabilire se esiste una formula del tipo
\[
    \iint_Q f(x,y) \,\text{d}x\,\text{d}y = \integral[c][d][
        \integral[a][b][f(x,y)][x]
    ][y]
\]

\begin{snippetexample}{}
    Sia \(Q = [0,1] \times [0,1]\) e consideriamo
    la funzione
    \[
        f(x,y) = \begin{cases}
            1_{\rationalnumbers}(x) & y = \frac12 \\
            0 & \text{altrimenti}
        \end{cases}
    \]
    Chiaramente \(f\) è Riemann integrabile in \(Q\),
    ma se fisso \(y=\frac12\) la funzione non è integrabile.
    In questo caso può pure andare bene in quanto uno dei due integrali non è valido solo in un punto,
    ma in generale possiamo fare esempi peggiori.
\end{snippetexample}

\begin{snippettheorem}{Teorema di Fubini}
    Sia \(f \colon Q \fromto \realnumbers\) integrabile.
    Se \(\forall y \in [c,d]\) Esiste\[
        F(y) = \integral[a][b][f(x,y)][x]
    \]
    cioè se \(f(\cdot, y)\) è integrabile in \([a,b]\), allora \(F\) è integrabile
    in \([c,d]\) e vale
    \[
        \iint_Q f(x,y)\,\text{d}y\,\text{d}{x} = \integral[c][d][F(y)][y]
    \]
\end{snippettheorem}

\begin{snippetproof}{Teorema di Fubini}
    Fissiamo per \(Q\) una partizione prodotto
    \(\Pi = \Pi_X \times \Pi_Y\).
    Fissiamo un rettangolo \(Q_{h,k}\) e fissiamo \(y \in [y_{k-1}, y_k]\).
    In questo quadratino consideriamo la striscia con \(y\) fissato, maggioriamo
    con il sup sulla striscia e poi con il sup di tutto il quadratino (e rispettivamente sup)
    \begin{align*}
        &\sum_h (x_h - x_{h-1}) m_{h,k} \leq \\
        &\sum_h (x_h - x_{h-1}) \inf_{x \in [x_{h-l}, x_h]} f(x,y)
        \leq F(y) \leq
        \sum_h (x_h - x_{h-1}) \sup_{x \in [x_{h-l}, x_h]} f(x,y) \\
        &\leq \sum_h (x_h - x_{h-1}) M_{h,k}
    \end{align*}
    Per integrare
    scriviamo
    \begin{align*}
        \sum_{h,k} |Q_{h,k}| m_{h,k} \leq
        \sum_k (y_k - y_{k-1}) \inf_{y \in [y_{k-l}, y_k]} F(y)
        \leq
        \sum_k (y_k - y_{k-1}) \sup_{y \in [y_{k-l}, y_k]} F(y)
        \leq \sum_{h,k} |Q_{h,k}| M_{h,k}
    \end{align*}
    \[
        s(f, \Pi) \leq s(F, \Pi_Y) \leq S(F, \Pi_Y) \leq S(f, \Pi)
    \]
    Per ipotesi \(\forall \varepsilon > 0\) esiste \(\Pi\) tale che
    \(S(f, \Pi) - s(f, \Pi) < \varepsilon\). Ma allora
    \(S(F, \Pi_Y) - s(F, \Pi_Y) < \varepsilon\) e quindi \(F\)
    è integrabile su  \([c,d]\) e vale l'uguaglianza tra gli integrali.
\end{snippetproof}

In particolare se \(f \in \mathcal C(Q)\), allora \(f\) è integrabile su \(Q\)
e pure la sua restrizione \(f(\cdot, y)\) è continua su \([a,b]\),
e quindi \(F\) è definita.
...
\begin{snippetexercise}{}
    ...
    e \[
        ...
        \end{cases}
    \]
    La funzione è continua sicuramente tranne sul grafico di \(y=x^2\).
    Tale bordo è una funzione integrabile e quindi trascurabile
    in quanto insieme di misura nulla, quindi \(f\) è integrabile.
    La funzione \(f(x, \cdot)\) è integrabile in quanto continua tranne che al massimo in un singolo punto.
    Quindi vale il teorema di Fubini. Abbiamo quindi
    \begin{align*}
        \iint_Q f(x,y) \,\text{d}x\,\text{d}y &= \integral[0][1][
            \integral[0][3][
                f(x,y)
            ][y]
        ][x] \\
...
\begin{radioactive}
\begin{snippetexercise}{}
    Sia \(f(x,y) = x^{-3}e^{\frac{y}{x}}\)
    e \(Q=[1,2] \times [0,1]\).
    Non abbiamo problemi e la funzione è continua nel dominio, quindi è integrabile.
    È anche limitata in quanto su un compatto. Integriamo prima rispetto alla \(y\)
    \begin{align*}
        \iint_Q f(x,y) \,\text{d}x\,\text{d}y &= \integral[1][2][
            \integral[0][1][
                x^{-3}e^{\frac{y}{x}}
            ][y]
        ][x] \\
...
Sia \(D \subseteq \realnumbers^2\) limitato e \(f \colon D \fromto \realnumbers\). Siccome
è limitato è sicuramente contenuto in un rettangolo \(Q\).
Possiamo pure scegliere un rettangolo tale che \(D\) sia contenuto nel suo interno.
Definiamo ora una funzione ausiliaria
\[
    \tilde f(x,y) = \begin{cases}
        f(x,y) & (x,y) \in D \\
        0 & (x,y) \in Q \difference D
    \end{cases}
\]
Definiamo quindi
\[
    \iint_D f(x,y) \,\text{d}x\,\text{d}y
    \triangleq \iint_Q \tilde f(x,y) \,\text{d}x\,\text{d}y
\]

La definizione è ben posta, cioè indipendente da \(Q\), in quanto
l'integrale sulla intersezione dei rettangoli è nullo.

Anche se \(f\) è continua (in cui caso vogliamo che sia integrabile),
il bordo di \(D\) potrebbe essere non trascurabile, e quindi \(\tilde f\) non sarebbe integrabile.
Dobbiamo quindi imporre che il bordo sia trascurabile.

\begin{snippetexample}{}
    Sia \(D = (\rationalnumbers \intersection [0,1])^2 \subseteq [0,1] \times [0,1]\).
    Prendiamo \(f \colon D \fromto \realnumbers\) data da \(f(x,y) = 1\).
    Tale funzione è continua, ma \(\tilde f\) è la funzione di Dirichlet
    e non è integrabile. Infatti, il bordo è \([0,1] \times [0,1]\) e non è trascurabile.
\end{snippetexample}

\begin{snippetdefinition}{Misura di Peano-Jordan}
    Diciamo che \(D \subseteq \realnumbers^2\) è misurabile su \(D\) se
    \[
        \exists \iint_Q 1\,\text{d}x\,\text{d}y
    \]
    e diciamo
    \[
        \mu(D) \triangleq \iint_Q 1 \,\text{d}x\,\text{d}y
    \]
\end{snippetdefinition}

\begin{snippettheorem}{}
    \(D\) è misurabile secondo Peano-Jordan se e solo se \(\partial D\)
    è trascurabile.
\end{snippettheorem}

Quindi o \(f=1\) è integrabile o ha misura nulla.

\begin{snippetproof}{}
    Sia \(\Pi\) una partizione prodotto di un quadrato \(Q\) contenente \(D\),
    e calcoliamo \(S(\Pi) - s(\Pi)\).
    Se \(Q_{hk} \subseteq D^\circ\), allora il contributo di tale rettangolino è la sua area,
    quindi inf e sup coincidono,
    quindi il contributo in \(S(\Pi)- s(\Pi)\) è nullo.
    Se \(Q_{hk} \subseteq (Q \difference D)^\circ\), vale lo stesso ragionamento, il contributo è nullo.
    Allora i contributi sono dati da quelle che intersecano
    sia \(D\) che il suo complementare, quindi \(Q_{hk} \intersection D \neq \emptyset\)
    e \(Q_{hk} \intersection (Q \difference D) \neq \emptyset\).
    Il contributo di questi è
    \begin{align*}
        S(\Pi) - s(\Pi) &= \sum_{\substack{h,k \\ Q_{hk} \intersection D \neq \emptyset \\ Q_{hk} \intersection (Q \difference D) \neq \emptyset}}
        |Q_{h,k}| \cdot 1
    \end{align*}
    Siccome il quadratino interseca punti di \(D\), il suo sup è \(1\), mentre il suo inf è \(0\)
    in quanto interseca punto del complementare.
    Per avere \(\mu(\partial D) = 0\) vogliamo che
    \[
        \sum_{\substack{h,k \\ Q_{h,k} \intersection \partial D \neq \emptyset}}
        |Q_{h,k}| < \varepsilon
    \]
    Abbiamo due famiglie diverse di indici
    \begin{align*}
        \mathcal F &= \{(h,k) \suchthat Q_{h,k} \intersection D \neq \emptyset \land Q_{h,k} \intersection (Q \difference D) \neq \emptyset\} \\
        \mathcal F' &= \{(h,k) \suchthat Q_{h,k} \intersection \partial D \neq \emptyset\}
    \end{align*}
    Chiaramente \(\mathcal F \subseteq \mathcal F'\).
    L'altra inclusione non è vera in quanto ci può essere un quadratino che tocca solamente il bordo in uno dei suoi lati.
    Tuttavia, possiamo modificare la partizione riducendola, fino a che \(\mathcal F = \mathcal F'\),
    quindi vale la tesi.
    Esercizio:
    dimostrare che se \(Q_{h,k} \intersection D \neq \emptyset\) e \(Q_{h,k} \intersection (Q \difference D)\)
    allora \(Q \intersection \partial D \neq \emptyset\).
\end{snippetproof}

\begin{snippettheorem}{}
    Sia \(D \subseteq \realnumbers^2\) misurabile, \(f \colon D \fromto \realnumbers\)
    limitata, \(f\) continua su \(D^\circ\).
    Allora, \(f\) è Riemann integrabile su \(D\).
\end{snippettheorem}

\begin{snippetproof}{}
    Sia \(Q\) un rettangolo tale che \(Q^\circ \supseteq \overline{D}\).
    \(Q = D^\circ \sqcup \partial D \sqcup (Q \difference D)^\circ\).
    Sia \(\tilde f\) l'estensione nulla di \(f\) a \(Q\).
    Notiamo che in \(D\), \(f \equiv \tilde f\) e quindi
    \(f \equiv \tilde f\) anche su \(D^\circ\).
    Inoltre \(\tilde f \equiv 0\) su \((Q \difference D)^\circ\) e quindi è continua anche su \((Q \difference D)^\circ\).
    Dunque, l'insieme delle discontinuità di \(f\) è contenuto in \(\partial D\), che per ipotesi ha misura nulla,
    quindi è integrabile su \(D\).
\end{snippetproof}

Sia \(D \subseteq \realnumbers^2\) limitato, \(Z \subseteq \realnumbers^2\) trascurabile.
Supponiamo che \(f \colon D \union Z \fromto \realnumbers\) sia limitata.
Allora, \(f\) è Riemann integrabile su \(D\) se e solo se lo è anche su \(D \union Z\)
se e solo se lo è anche su \(D \difference Z\). Anche per unione finita di insiemi trascurabili.

Sia \(f \colon \overline D \fromto \realnumbers\) limitata con \(D\) limitato.
In generale non c'è una correlazione fra l'integrabilità su \(D\) e sulla sua chiusura.
Però, se \(D\) è misurabile, allora
allora \(f\) è integrabile su \(D\) se e solo se lo è su \(\overline{D}\).
Questo è dato dal fatto che \(\overline D = \partial D \union D\).

Può succedere che \(f\) sia integrabile su \(D\) anche se \(D\) non è misurabile.
Per esempio se \(f \equiv 0\).
Anche se ho un dominio di misura nulle, tutte le funzioni sono integrabili e hanno integrale nullo.
In tal caso basta prendere una partizione prodotto \(\Pi\), possiamo ricoprire
il dominio con un numero finito di rettangoli con area \(<\varepsilon\).
Per tutti gli altri rettangoli, il contributo in \(S(\Pi) - s(\Pi)\) è nullo.

\begin{snippetdefinition}{Dominio semplice}
    Siano \(\alpha, \beta \colon [a,b] \fromto \realnumbers\) continua
    such that \(\alpha(x) \leq \beta(x)\) on \([a,b]\).
    Diciamo che \(D = \{(x,y) \suchthat a \leq x \leq b, \alpha(x) \leq y \leq \beta(x)\}\)
    è un \emph{dominio y-semplice} o \emph{normale rispetto all'asse y}.
    Analogamente per l'altro asse.
\end{snippetdefinition}

Un dominio di questo tipo è misurabile in quanto \(\partial D\) è l'unione di quattro
pezzi (2 grafici e 2 segmenti) che sono tutti Riemann integrabili in quanto
le funzioni sono continue, e quindi misurabile.

\begin{snippettheorem}{}
    Sia \(D\) un dominio \emph{y-semplice}
    e \(f\) come sopra.
    Supponiamo che \(f\) sia limitata e continua in \(D^\circ\). Allora
    \[
        \iint_Q f(x,y) \,\text{d}x\,\text{d}y 
        = \integral[a][b][
            \integral[\alpha(x)][\beta(x)][f(x,y)][y]
        ][c]
    \] 
\end{snippettheorem}

\begin{snippetproof}{}
    Possiamo racchiudere il dominio in un rettangolo \(Q\)
    e consideriamo \(\tilde f\) l'estensione nulla di \(f\) a \(Q\).
    Ovviamente \(\tilde f\) è integrabile in quanto \(D\) è misurabile.
    Dobbiamo mostrare che \(\forall x \in [a,b], \tilde f(x, \cdot)\) è integrabile in \([c,d]\)
    dove \([c,d]\) è il lato del rettangolo \(Q\).
    Chiaramente \(\tilde f\) ha al più due discontinuità, è quindi integrabile in quanto continua
    altrove. Quindi vale il teorema di Fubini e vale il teorema
    \begin{align*}
        \iint_Q f(x,y) \,\text{d}x\,\text{d}y 
        &= \iint_Q \tilde f(x,y) \,\text{d}x\,\text{d}y \\
        &= \integral[a][b][
            \integral[\alpha(x)][\beta(x)][f(x,y)][y]
        ][c] \\
        &= \integral[a][b][
            \integral[c][\alpha(x)][0][y]
            + \integral[\alpha(x)][\beta(x)][f(x,y)][y]
            + \integral[\beta(x)][d][0][y]
        ][x] \\
        &= \integral[a][b][
            \integral[\alpha(x)][\beta(x)][f(x,y)][y]
        ][c]
...
\begin{snippettheorem}{}
    Siano \(f,g\) due funzioni integrabili su un dominio
    \(D\) misurabile e siano \(\alpha, \beta \in \realnumbers\). Allora
    \begin{enumerate}
        \item \(\alpha f + \beta g\) è integrabile e vale la somma (linearità);
        \item se \(f \leq g\) allora vale anche con gli integrali (monotona);
        \item se \(|f|\) è integrabile allora il valore assoluto dell'integrale
        è minore dell'integrale di \(|f|\).
        \item Teorema della media: se \(m = \inf_D f\) e \(M = \sup_D f\), allora
        \[
            m \leq \frac{1}{\mu(D)} \iint_d f(x,y) \,\text{d}x\,\text{d}y \leq M
        \]
        \item se \(D\) è compatto e connesso
        e \(f\) è continua, allora esiste \((x_0, y_0) \in D\) tale che
        \[
            \frac{1}{\mu(D)}  \iint_D f(x,y) \,\text{d}x\,\text{d}y 
            = f(x_0, y_0)
        \]
        \item se \(D=D_1 \union D_2\)
        tale che \(\mu(D_1 \intersection D_2) = 0\), allora
        \[
            \iint_D f(x,y) \,\text{d}x\,\text{d}y 
            = \iint_{D_1} f(x,y) \,\text{d}x\,\text{d}y + \iint_{D_2} f(x,y) \,\text{d}x\,\text{d}y 
        \]
    \end{enumerate}
\end{snippettheorem}
...
\begin{radioactive}
\begin{snippetexercise}{}
    Calcolare il volume del cilindroide definito da
    \(z = xy\) nel dominio
    \[
        D = \{
            (x,y) \suchthat 0 \leq y \leq \frac{3}4 \land x^2 + y^2 - 25 \leq 0    
        \}
    \]
    Chiaramente dobbiamo prendere il modulo per assicurarci un valore positivo
    \[
        \iint_D |f(x,y)| \,\text{d}x\,\text{d}y 
    \]
    Il dominio è un cerchio di raggio \(5\) centrato nell'origine,
    tagliando da una retta (e dall'asse delle ascisse).
    Siamo quindi nel primo quadrante.
    Notiamo che il punto \((4, 3)\) si trova sulla circonferenza.
    Sezioniamo tale area orizzontalmente
    \begin{align*}
        \iint_D |f(x,y)| \,\text{d}x\,\text{d}y 
        &= \integral[0][3][
...
\begin{snippetexercise}{}
    ...
    \[
        ...
        \}
    \]
    e \(f(x,y) = \frac{\sin(y^2)}{y}\).
    Notiamo che il denominatore non si annulla mai, ma l'origine è un punto di accumulazione
    quindi ci si può avvicinare. Dobbiamo verificare che \(f\) sia limitata.
    Sicuramente \(f > 0\), inoltre
    \[
        \frac{y^2}{y} \leq \frac{y^2}{y} = y \to 0
    \]
    quindi è limitata e nell'origine si può allungare per continuità.
    \begin{align*}
        \iint_D |f(x,y)| \,\text{d}x\,\text{d}y 
        &= \integral[0][\sqrt\pi][
            \integral[0][y^2][
...
\begin{radioactive}
\begin{snippetexercise}{}
    Calcolare, al variare di \(\alpha > 1\),
    \[
        \iint_D \frac{x^2}{y} \,\text{d}x\,\text{d}y 
    \]
    dove 
        \[
        D = \{
            (x,y) \suchthat  x> 0 \land x^\alpha < y < x
        \}
    \]
    Siccome \(x^\alpha < x\) allora deve essere \(x < 1\).
    Il dominio è quindi l'intersezione nel primo quadrante fra \(y=x\)
    e \(y=x^\alpha\).
    In generale il limite di questa funzione nell'origine non esiste, ma dobbiamo considerare solamente
    le direzioni che stanno nel dominio.
    Possiamo notare che
    \[
        f(x,y) = \frac{x^2}{y} \leq \frac{x^2}{x^\alpha} = x^{2-\alpha}
    \]
    quindi se \(\alpha \leq 2\) va tutto bene.
    Se \(\alpha > 2\), la funzione è illimitata.
    Infatti, se \(\varepsilon > 0\) è tale che
    \(2 - 2 + \varepsilon < \alpha\), allora \(f(t, t^{2 + \varepsilon}) = 1 / t^\varepsilon \to \infty\).
    Quindi la funzione è limitata solo per \(1 < \alpha \leq 2\).
    Abbiamo dunque 
    \begin{align*}
        \iint_D |f(x,y)| \,\text{d}x\,\text{d}y 
        &= \integral[0][1][
...
\begin{radioactive}
\begin{snippetexercise}{}
    Scambiare l'ordine di integrazione in
    \[
        \integral[-1][1][
            \integral[|x|][\sqrt{2-x^2}][f(x,y)][y]
        ][x]
    \]
    Abbiamo che \(|x| = \sqrt{2-x^2}\) se e solo se \(x = \pm 1\).
    Il dominio di integrazione è quindi una fetta di circonferenza verso l'alto.
    Per sezionarlo orizzontalmente dobbiamo spezzarlo in due
    \begin{align*}
        \iint_D |f(x,y)| \,\text{d}x\,\text{d}y 
        &= \integral[0][1][
            \integral[-y][y][
...
\begin{snippetexercise}{}
    Calcolare
    \[
        \iint_{\{x^2 + y^2 < 1\}} (x-y)\cos(x^2 + y^2) \,\text{d}x\,\text{d}y 
    \]
    Notiamo che \(f(x,y) = -f(-x, -y)\) e il dominio è simmetrico, allora l'integrale fa zero.
\end{snippetexercise}
\end{radioactive}

\subsection{Integrali tripli}

La teoria si estende in maniera completamente analoga.
Ora, il grafico di una funzione integrabile \(\realnumbers^2 \fromto \realnumbers\)
è un insieme di misura nulla.

\begin{radioactive}
\begin{snippetexample}{}
    Consideriamo l'insieme \[
        D = \{(x,y,z) \suchthat x\geq 0 \land y \geq 0 \land z \geq 0 \land x + y + z \leq 1\}
    \]
    è un \(3\)-simplesso standard, anche detto tetraedo fondamentale dell'ottante positivo.
    Vogliamo calcolare
    \[
        \iiint_D f(x,y,z) \,\text{d}x\,\text{d}y\,\text{d}z
    \]
    Possiamo sezionarlo per strati \(z \in [0,1]\)
    \begin{align*}
        \iiint_D f(x,y,z) \,\text{d}x\,\text{d}y\,\text{d}z
        &=
        \integral[0][1][
    ...
    \end{align*}
    Alternativamente possiamo sezionarlo per fili. Consideriamo la proiezione del dominio
    sul piano \(xy\). Consideriamo dei fili verticali che arrivano fino all'estremo del dominio \(z = 1 - x - y\).
    Quindi un singolo filo va da \(0\) a \(1-x-y\). Abbiamo quindi
    \begin{align*}
        \iiint_D f(x,y,z) \,\text{d}x\,\text{d}y\,\text{d}z
        &=
        \iint_T \left(\integral[0][1-x-y][f(x,y,z)][z]\right) \,\text{d}x\,\text{d}y
    \end{align*}
    dove \(T\) è la base, cioè \(\{(x,y) \suchthat x \geq 0 \land y \geq 0 \land x+y \leq 1\}\)
\end{snippetexample}
\end{radioactive}

Abbiamo quindi 2 modi di sezionare un integrale doppio e 6 di sezionarne uno triplo.
% (n-1)n

In generale, se
\[
    D = \{(x,y,z) \suchthat (x,y) \in T, \alpha(x,y) \leq z \leq \beta(x,y)\}
\]
abbiamo
\[
    \iiint_D f(x,y,z) \,\text{d}x\,\text{d}y\,\text{d}z
    = \iint_T \integral[\alpha(x,y)][\beta(x,y)][f(x,y,z)][z] \,\text{d}x\,\text{d}y
\]

\begin{radioactive}
\begin{snippetexample}{}
    Calcolare il volume del 3 simplesso standard.
    Dobbiamo integrare la funzione \(1\).
    Cominciamo per strati
    \begin{align*}
        \iiint_D 1 \,\text{d}x\,\text{d}y\,\text{d}z
        &= \integral[0][1][
            \iint_{D_z} 1 \,\text{d}x\,\text{d}y
        ][z] \\
        &= \integral[0][1][
            \integral[0][1-z][
...
\begin{snippetexercise}{}
    L'ipervolume dell'\(n\)-simplesso è \(1 / n!\).
\end{snippetexercise}
...
% Complementi inseriti dalle lezioni del 17/12 e 18/12
\subsection{Complementi su integrali tripli, volumi e cambiamenti di variabili}

\begin{snippetexample}{Integrale triplo su un solido tra paraboloide e piano}
    Calcoliamo
    \[
        \iiint_D (2x-1)\,dx\,dy\,dz,
    \]
    dove
    \[
        D=
        \left\{
            ...
            x^2+y^2\leq z\leq 1+x
        \right\}.
    \]
    Il solido \(D\) è compreso tra il paraboloide
    \[
        z=x^2+y^2
    \]
    e il piano
    \[
        z=1+x.
    \]
    La sua frontiera è quindi formata da due pezzi:
    \[
        \partial D
        =
        \left\{
        ...
        \union
        \left\{
            z=1+x,\ z-x^2-y^2\geq 0
        \right\}.
    \]

    \begin{center}
        \begin{tikzpicture}[scale=0.9]
            \draw[->] (-2,0) -- (2.4,0) node[right] {\(x\)};
            \draw[->] (0,-0.3) -- (0,2.6) node[above] {\(z\)};
            \draw[domain=-1.4:1.4,samples=80,thick] plot (\x,{0.7*\x*\x});
            \draw[domain=-1.4:1.4,samples=80,thick] plot (\x,{1+0.45*\x});
            \node at (-1.35,1.65) {\(z=1+x\)};
            \node at (1.55,1.15) {\(z=x^2+y^2\)};
        \end{tikzpicture}
    \end{center}

    Proiettiamo \(D\) sul piano \(xy\). La proiezione è
    \[
        T=
        \left\{
            (x,y)\in\realnumbers^2:
            x^2+y^2\leq 1+x
        \right\}.
    \]
    Infatti, fissati \(x\) e \(y\), esiste almeno uno \(z\) tale che
    \[
        x^2+y^2\leq z\leq 1+x
    \]
    se e solo se
    ...
    \[
        ...
        \left\{
            (x,y)\in\realnumbers^2:
            \left(x-\frac{1}{2}\right)^2+y^2\leq \frac{5}{4}
        \right\}.
    \]
    È un disco di centro \(\left(\frac{1}{2},0\right)\) e raggio \(\frac{\sqrt{5}}{2}\).
    ...
    \begin{align*}
        ...
        &=
        \iint_T
        \integral[x^2+y^2][1+x][2x-1][z]\,dx\,dy\\
        &=
        \iint_T
        (2x-1)(1+x-x^2-y^2)\,dx\,dy.
    \end{align*}
    Facciamo il cambio
    \[
        u=x-\frac{1}{2}.
    ...
    Il dominio diventa
    \[
        u^2+y^2\leq \frac{5}{4}.
    \]
    Quindi
    \[
        \iiint_D (2x-1)\,dx\,dy\,dz
        =
        \iint_{u^2+y^2\leq \frac{5}{4}}
        2u\left(\frac{5}{4}-u^2-y^2\right)\,du\,dy.
    \]
    L'integranda è dispari rispetto alla variabile \(u\), mentre il dominio è simmetrico rispetto all'asse \(u=0\). Pertanto
    \[
        \iiint_D (2x-1)\,dx\,dy\,dz=0.
    \]
\end{snippetexample}

\subsection{Volumi di solidi di rotazione}

\begin{snippetexample}{Volume ottenuto ruotando una regione piana}
    Calcoliamo il volume del solido \(\Omega\) ottenuto ruotando attorno all'asse \(x\) la regione
    \[
        S=
        \left\{
            (x,y)\in\realnumbers^2:
        ...
        \right\}.
    \]
    Le curve si intersecano quando
    \[
        x^2=\sqrt{x}.
    \]
    Con \(x\geq 0\), otteniamo
    \[
        x^4=x,
    \]
    dunque
    \[
        ...
        x=1.
    \]
    Quindi \(0\leq x\leq 1\).

    \begin{center}
        \begin{tikzpicture}[scale=1.1]
            \draw[->] (-0.2,0) -- (1.4,0) node[right] {\(x\)};
            \draw[->] (0,-0.2) -- (0,1.3) node[above] {\(y\)};
            \draw[domain=0:1,samples=80,thick] plot (\x,{sqrt(\x)});
            \draw[domain=0:1,samples=80,thick] plot (\x,{\x*\x});
            \fill[gray!20] plot[domain=0:1,samples=80] (\x,{sqrt(\x)})
            -- plot[domain=1:0,samples=80] (\x,{\x*\x}) -- cycle;
            \node at (0.85,1.05) {\(y=\sqrt{x}\)};
            \node at (0.8,0.42) {\(y=x^2\)};
        \end{tikzpicture}
    \end{center}

    La sezione del solido ottenuta fissando \(x\) è una corona circolare con raggio esterno \(\sqrt{x}\) e raggio interno \(x^2\). Quindi
    \[
...
\begin{snippetproposition}{Formula dei gusci o delle corone circolari}
    Sia
    \[
        S=
        \left\{
    ...
    \[
        0\leq f(x)\leq g(x).
    \]
    Se \(\Omega\) è il solido ottenuto ruotando \(S\) attorno all'asse \(x\), allora
    \[
        \operatorname{Vol}(\Omega)
        =
        \pi\integral[a][b][g(x)^2-f(x)^2][x].
    \]
\end{snippetproposition}

\begin{snippetexample}{Volume del toro}
    Consideriamo il toro ottenuto ruotando attorno all'asse \(z\) il disco di raggio \(r\) e centro \((R,0)\) nel piano \(xz\), con \(R>r>0\). Nel piano \(xz\) la regione è
    \[
        \left\{
            (x,z):
            R-\sqrt{r^2-z^2}\leq x\leq R+\sqrt{r^2-z^2},
            \ -r\leq z\leq r
        \right\}.
    \]
    ...
    Usando la formula delle corone circolari rispetto alla variabile \(z\), otteniamo
    \begin{align*}
        \operatorname{Vol}(T)
        &=
        \pi\integral[-r][r][
            \left(R+\sqrt{r^2-z^2}\right)^2
            -
            \left(R-\sqrt{r^2-z^2}\right)^2
        ][z]\\
        &=
        ...
        &=
        4\pi R\integral[-r][r][\sqrt{r^2-z^2}][z].
    \end{align*}
    L'integrale
    \[
        \integral[-r][r][\sqrt{r^2-z^2}][z]
    \]
    è l'area di una semicirconferenza di raggio \(r\), quindi vale
    \[
        \frac{\pi r^2}{2}.
    \]
    Ne segue
...
\begin{snippettheorem}{Teorema di Pappo}
    Sia \(D\) una regione piana e sia \(\Omega\) il solido ottenuto ruotando \(D\) attorno a un asse che non interseca \(D\). Se \(G\) è il baricentro di \(D\), allora
    \[
        \operatorname{Vol}(\Omega)
        =
        2\pi d(G,\text{asse})\,|D|.
    \]
    Nel caso dell'asse \(z\), se il baricentro ha ascissa \(x_G\), la formula diventa
    \[
        \operatorname{Vol}(\Omega)=2\pi x_G |D|.
    \]
\end{snippettheorem}

\subsection{Cambiamenti di variabili negli integrali multipli}

In una variabile, se
\[
    \Phi:[c,d]\to[a,b]
\]
è un diffeomorfismo, allora il cambio di variabile \(t=\Phi(u)\) dà
\[
    \integral[a][b][f(t)][t]
    =
    \integral[c][d][f(\Phi(u))\left|\Phi'(u)\right|][u],
\]
quando si intendono gli integrali su intervalli non orientati.

Più in generale, in \(\realnumbers^n\), un cambiamento di variabili è dato da un diffeomorfismo
\[
    \Phi:\Omega\to\Phi(\Omega),
\]
dove
\[
    \Omega\subseteq\realnumbers^n
\]
è aperto.

\begin{snippetdefinition}{Matrice jacobiana}
    Sia
    \[
        \Phi(u_1,\ldots,u_n)
        =
    ...
    \[
        ...
        =
        \left(
            \frac{\partial \Phi_i}{\partial u_j}(u)
        \right)_{i,j=1}^n.
    \]
    Il determinante
    \[
        \det J\Phi(u)
    \]
    misura il fattore locale di dilatazione dei volumi.
\end{snippetdefinition}

\begin{snippettheorem}{Cambio di variabili}
    Sia
    \[
        \Phi:\Omega\to\Phi(\Omega)
    \]
    un diffeomorfismo tra aperti di \(\realnumbers^n\). Se \(D\subseteq\Phi(\Omega)\) è misurabile e \(f\) è integrabile su \(D\), allora
    \[
        \int_D f(x)\,dx
        =
        \int_{\Phi^{-1}(D)}
        f(\Phi(u))
        \left|\det J\Phi(u)\right|\,du.
    \]
    In dimensione \(2\):
    \[
        \iint_D f(x,y)\,dx\,dy
        ...
        \iint_{\Phi^{-1}(D)}
        f(\Phi(u,v))
        \left|\det J\Phi(u,v)\right|\,du\,dv.
    \]
    In dimensione \(3\):
    \[
        ...
        \iiint_{\Phi^{-1}(D)}
        f(\Phi(u,v,w))
        \left|\det J\Phi(u,v,w)\right|\,du\,dv\,dw.
    \]
\end{snippettheorem}

L'idea geometrica è la seguente: vicino a un punto, un diffeomorfismo si comporta come la sua applicazione lineare differenziale. In dimensione \(2\), un piccolo rettangolo viene trasformato approssimativamente in un parallelogramma, e il rapporto tra le aree è
\[
    \left|\det J\Phi\right|.
\]
...
\begin{center}
    \begin{tikzpicture}[scale=0.9]
        ...
        \draw[fill=gray!20] (0.5,0.5) rectangle (1.6,1.5);
        \node at (1.05,0.25) {\(du\,dv\)};

        \draw[->,thick] (3.1,1) -- (4.3,1) node[midway,above] {\(\Phi\)};

        \begin{scope}[xshift=5.1cm]
            \draw[->] (-0.3,0) -- (3,0) node[right] {\(x\)};
            \draw[->] (0,-0.3) -- (0,2.2) node[above] {\(y\)};
            \draw[fill=gray!20] (0.6,0.5) -- (2,0.75) -- (2.35,1.65) -- (0.95,1.4) -- cycle;
            \node at (1.55,0.25) {\(\left|\det J\Phi\right|\,du\,dv\)};
        \end{scope}
    \end{tikzpicture}
\end{center}
...
quindi
\[
    \det J\Phi(\rho,\theta)=\rho.
\]
L'elemento d'area diventa
\[
...
\begin{center}
    \begin{tikzpicture}[scale=1]
        ...
        \draw[thick] (0,0) -- (2.1,1.2);
        \draw[thick] (0,0) -- (1.55,1.85);
        \draw[thick] (1.1,0.62) arc[start angle=29.7,end angle=50,radius=1.25];
        \draw[thick] (1.7,0.97) arc[start angle=29.7,end angle=50,radius=1.95];
        \node at (2.15,1.45) {\(\rho\,d\rho\,d\theta\)};
        \node at (0.75,0.2) {\(\theta\)};
    \end{tikzpicture}
\end{center}

\begin{snippetexample}{Integrale su un settore circolare}
    Calcoliamo
    \[
        \iint_A \frac{1}{1+x^2+y^2}\,dx\,dy,
    \]
    dove
    \[
        A=
        \left\{
            (x,y):
            -x\leq y\leq \sqrt{3}x,
            \ x^2+y^2\leq 4
        \right\}.
    \]
    In coordinate polari, le rette
    \[
        y=-x
        \qquad\text{e}\qquad
        y=\sqrt{3}x
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1]
            ...
            \draw[thick] ({2*cos(-45)},{2*sin(-45)}) arc[start angle=-45,end angle=60,radius=2];
            \fill[gray!20] (0,0) -- ({2*cos(-45)},{2*sin(-45)})
            arc[start angle=-45,end angle=60,radius=2] -- cycle;
            \node at (1.55,1.75) {\(y=\sqrt{3}x\)};
            \node at (1.6,-1.15) {\(y=-x\)};
        \end{tikzpicture}
    \end{center}
    ...
    \begin{align*}
        ...
        \right]_0^2\\
        &=
        \frac{7\pi}{12}\cdot\frac{1}{2}\ln 5\\
        &=
        \frac{7\pi}{24}\ln 5.
    \end{align*}
\end{snippetexample}
...
\begin{snippetexample}{Volume tra una sfera e un paraboloide}
    ...
    Dobbiamo imporre
    \[
        \frac{\rho^2}{3}\leq \sqrt{4-\rho^2}.
    \]
    Quadrando:
    \[
        \frac{\rho^4}{9}\leq 4-\rho^2,
    \]
    ossia
    \[
        \rho^4+9\rho^2-36\leq 0.
    \]
    Ponendo \(s=\rho^2\), otteniamo
    \[
        s^2+9s-36\leq 0.
    \]
    Le radici sono
    \[
        s=\frac{-9\pm 15}{2},
    \]
    perciò la radice positiva è \(s=3\). Dunque
    \[
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.9]
            \draw[->] (-2.3,0) -- (2.4,0) node[right] {\(\rho\)};
            \draw[->] (0,-0.3) -- (0,2.4) node[above] {\(z\)};
            \draw[domain=0:2,samples=100,thick] plot (\x,{sqrt(4-\x*\x)});
            \draw[domain=0:1.75,samples=100,thick] plot (\x,{0.333*\x*\x});
            \fill[gray!20] plot[domain=0:1.732,samples=80] (\x,{sqrt(4-\x*\x)})
            -- plot[domain=1.732:0,samples=80] (\x,{0.333*\x*\x}) -- cycle;
            \node at (1.5,1.55) {\(z=\sqrt{4-\rho^2}\)};
            \node at (1.4,0.45) {\(z=\rho^2/3\)};
        \end{tikzpicture}
    \end{center}
    ...
    \begin{align*}
        ...
        \integral[0][2\pi]
        \integral[0][\sqrt{3}]
        \integral[\frac{\rho^2}{3}][\sqrt{4-\rho^2}]
        [\rho][z]\,d\rho\,d\theta\\
        &=
        2\pi\integral[0][\sqrt{3}]
        [
            \rho\sqrt{4-\rho^2}-\frac{\rho^3}{3}
        ][\rho].
    \end{align*}
    Ora
    \[
        \integral[0][\sqrt{3}][\rho\sqrt{4-\rho^2}][\rho]
        =
        \frac{7}{3},
    \]
    e
    \[
        \integral[0][\sqrt{3}][\frac{\rho^3}{3}][\rho]
        =
        \frac{3}{4}.
    \]
    Pertanto
    \[
        \operatorname{Vol}(V)
        =
        2\pi\left(\frac{7}{3}-\frac{3}{4}\right)
        =
        2\pi\cdot\frac{19}{12}
        =
        \frac{19\pi}{6}.
    \]
\end{snippetexample}

\subsection{Un cambio di variabili non lineare}

\begin{snippetexample}{}
    Calcoliamo
    \[
        \iint_T x^2(y+x^3)e^{y+x^3}\,dx\,dy,
    \]
    dove
    \[
        T=
        ...
        \right\}.
    \]
    La regione è limitata, perché da
    \[
        x^3\leq y\leq 3
    \]
    segue
    \[
        1\leq x\leq \sqrt[3]{3}.
    \]
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1]
            ...
            \draw[domain=0:1.55,samples=100,thick] plot (\x,{\x*\x*\x});
            \draw[thick] (0,3) -- (1.55,3);
            \draw[thick] (1,1) -- (1,3);
            \fill[gray!20] (1,1)
            -- plot[domain=1:1.442,samples=80] (\x,{\x*\x*\x})
            -- (1.442,3) -- (1,3) -- cycle;
            \node at (1.65,3.1) {\(y=3\)};
            \node at (1.55,2.2) {\(y=x^3\)};
            \node at (0.75,0.95) {\(x=1\)};
        \end{tikzpicture}
    \end{center}

    Scegliamo il cambio di variabili
    \[
        u=y-x^3,
        \qquad
        v=y+x^3.
    ...
    \[
        x^2(y+x^3)e^{y+x^3}\,dx\,dy
        =
        x^2ve^v\frac{1}{6x^2}\,du\,dv
        =
        \frac{1}{6}ve^v\,du\,dv.
    \]

    Descriviamo il dominio trasformato. Le condizioni sono:
    \[
        y\geq x^3
        \iff
        u\geq 0,
    ...
    \begin{align*}
        ...
        \right]\\
        &=
        \frac{2}{3}\left(e^6-e^4\right).
    \end{align*}
\end{snippetexample}

\subsection{Cambiamenti di variabili: altri esempi}

\begin{snippetexample}{Un cambio non lineare}
    Calcoliamo
    \[
        \iint_T x^2(y-x^3)e^{y+x^3}\,dx\,dy,
    \]
    ...
    \[
        ...
        \left\{
            (x,y)\in\realnumbers^2:
            x^3\leq y\leq 3,\ x\geq 1
        \right\}.
    \]
    Usiamo il cambio di variabili
    \[
        u=y-x^3,
        \qquad
        v=y+x^3.
    ...
    \[
        ...
        \right).
    \]
    Calcoliamo il determinante jacobiano:
    \[
        \left|\det J\Phi(u,v)\right|
        =
        \frac{1}{6}
        \left(\frac{v-u}{2}\right)^{-\frac{2}{3}}.
    \]
    Inoltre
    \[
        x^2=
    ...
    Descriviamo ora il dominio trasformato. Le condizioni su \(T\) diventano:
    \[
        y\geq x^3 \iff u\geq 0,
    \]
    \[
        y\leq 3 \iff \frac{u+v}{2}\leq 3 \iff u+v\leq 6,
    \]
    e
    \[
        x\geq 1 \iff x^3\geq 1 \iff \frac{v-u}{2}\geq 1 \iff v-u\geq 2.
    \]
    Dunque
    \[
        \Phi^{-1}(T)
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.9]
            \draw[->] (-0.2,0) -- (4.5,0) node[right] {\(u\)};
            \draw[->] (0,-0.2) -- (0,5) node[above] {\(v\)};
            \draw[thick] (0,2) -- (2,4);
            \draw[thick] (0,6) -- (2,4);
            \draw[thick] (0,2) -- (0,6);
            \fill[gray!20] (0,2) -- (0,6) -- (2,4) -- cycle;
            \node[left] at (0,2) {\(2\)};
            \node[left] at (0,6) {\(6\)};
            \node[below] at (2,0) {\(2\)};
            \node at (2.9,4.7) {\(v=6-u\)};
            \node at (3,2.9) {\(v=u+2\)};
        \end{tikzpicture}
    \end{center}
    ...
    Calcolando:
    \[
        \integral[0][2][u e^{6-u}][u]
        =
        e^6-3e^4,
    \]
    e
    \[
        \integral[0][2][u e^{u+2}][u]
        =
        e^4+e^2.
    \]
    Quindi
...
\begin{snippetexample}{Un integrale sulla palla unitaria}
    Calcoliamo
    \[
        \iiint_B \cos(x+y+z)\,dx\,dy\,dz,
    \]
    dove
    \[
        B=
        ...
        \right\}.
    \]
    L'idea è usare una rotazione che mandi il vettore
    \[
        \frac{1}{\sqrt{3}}(1,1,1)
    \]
    nel vettore \((1,0,0)\). In questo modo la quantità \(x+y+z\) diventa \(\sqrt{3}\,u\).

    Poiché la palla unitaria è invariante per rotazioni e il determinante di una rotazione ha modulo \(1\), l'integrale diventa
    \[
        \iiint_{u^2+v^2+w^2\leq 1}
        \cos(\sqrt{3}\,u)\,du\,dv\,dw.
    \]
    Fissato \(u\in[-1,1]\), la sezione nel piano \(vw\) è il disco
    \[
        v^2+w^2\leq 1-u^2,
    \]
    di area \(\pi(1-u^2)\). Quindi
    \begin{align*}
        \iiint_B \cos(x+y+z)\,dx\,dy\,dz
        &=
        \integral[-1][1]
        [
            \pi(1-u^2)\cos(\sqrt{3}\,u)
        ][u]\\
        &=
        \pi\integral[-1][1]
        [
    ...
    Usando
    \[
        \integral[-1][1][(1-u^2)\cos(au)][u]
        =
        \frac{4(\sin a-a\cos a)}{a^3},
    \]
    con \(a=\sqrt{3}\), otteniamo
    \[
        \iiint_B \cos(x+y+z)\,dx\,dy\,dz
        =
        \frac{4\pi}{3\sqrt{3}}
        \left(
            \sin\sqrt{3}-\sqrt{3}\cos\sqrt{3}
        \right).
    \]

    \begin{center}
        \begin{tikzpicture}[scale=0.9]
            \draw[->] (-2,0) -- (2.3,0) node[right] {\(u\)};
            \draw[->] (0,-1.8) -- (0,1.9) node[above] {\(v,w\)};
            \draw[thick] (0,0) circle (1.4);
            \fill[gray!20] (0.6,0) ellipse (0.25 and 1.25);
            \draw[dashed] (0.6,-1.25) -- (0.6,1.25);
            \node at (1.35,1.35) {\(B\)};
            \node at (1.4,-1.3) {\(v^2+w^2\leq 1-u^2\)};
        \end{tikzpicture}
    \end{center}
\end{snippetexample}
...
\begin{snippetdefinition}{Integrale di superficie}
    Sia
    \[
        \Sigma\subseteq\realnumbers^3
    \]
    una superficie e sia
    \[
        f:\Sigma\to\realnumbers.
    \]
    L'integrale di superficie di \(f\) su \(\Sigma\) si indica con
    \[
        \iint_\Sigma f\,dS,
    \]
    dove \(dS\) rappresenta l'elemento d'area della superficie.
\end{snippetdefinition}

L'idea è analoga a quella degli integrali curvilinei. Se una curva \(\Gamma\) è parametrizzata da
\[
    \gamma:[a,b]\to\realnumbers^n,
\]
allora l'elemento di lunghezza è
\[
    ds=\|\gamma'(t)\|\,dt,
\]
e quindi
\[
    \int_\Gamma f\,ds
    =
    \integral[a][b][f(\gamma(t))\|\gamma'(t)\|][t].
\]
Per le superfici, l'elemento d'area sarà costruito a partire da una parametrizzazione della superficie.

\subsection{Superfici parametrizzate}

\begin{snippetdefinition}{Superficie parametrizzata}
    Sia \(A\subseteq\realnumbers^2\) un aperto connesso. Una superficie parametrizzata in \(\realnumbers^3\) è un'applicazione
    \[
        \sigma:A\to\realnumbers^3,
        \qquad
        \sigma(u,v)=
        \left(
            x(u,v),
        ...
        \right).
    \]
    La superficie parametrizzata è detta regolare se \(\sigma\in C^1(A,\realnumbers^3)\) e
    \[
        \operatorname{rank}J\sigma(u,v)=2
        \qquad
        \forall (u,v)\in A.
    \]
\end{snippetdefinition}
...
\[
    ...
    \end{pmatrix}.
\]
La condizione di regolarità significa che le due colonne
\[
    \sigma_u(u,v),
    \qquad
    \sigma_v(u,v)
\]
sono linearmente indipendenti. In \(\realnumbers^3\), questo equivale a
\[
    \sigma_u(u,v)\times\sigma_v(u,v)\neq 0.
\]

\begin{snippetdefinition}{Superficie parametrizzabile}
    Un sottoinsieme
    \[
        \Sigma\subseteq\realnumbers^3
    \]
    si dice superficie parametrizzabile se esiste una parametrizzazione regolare
    \[
        \sigma:A\to\realnumbers^3
    \]
    tale che
    \[
        \Sigma=\sigma(A),
    \]
    con eventuali identificazioni sul bordo del dominio dei parametri.
\end{snippetdefinition}

Quando un minore \(2\times 2\) di \(J\sigma\) è non nullo, localmente la superficie può essere descritta come grafico di una funzione di due variabili. Per esempio, se
\[
    \det
    \begin{pmatrix}
        x_u & x_v\\
    ...
    \neq 0,
\]
allora, per il teorema della funzione inversa, si possono usare \(x\) e \(y\) come coordinate locali sulla superficie, e quindi scrivere localmente
\[
    z=z(x,y).
\]
...
\begin{snippetexample}{Grafico di una funzione}
    Sia
    \[
        f:A\subseteq\realnumbers^2\to\realnumbers
    \]
    una funzione di classe \(C^1\). Il grafico di \(f\) è parametrizzato da
    \[
        \sigma(x,y)=
        \left(
            x,y,f(x,y)
    ...
    \[
        ...
        \end{pmatrix}.
    \]
    Il rango è sempre \(2\), quindi il grafico di \(f\) è una superficie regolare.
\end{snippetexample}

\begin{snippetdefinition}{Piano tangente}
    Sia \(\Sigma=\sigma(A)\) una superficie regolare, e sia
    \[
        P=\sigma(u_0,v_0)\in\Sigma.
    \]
    Il piano tangente a \(\Sigma\) in \(P\) è il piano affine generato dai vettori
    \[
        \sigma_u(u_0,v_0),
        \qquad
        \sigma_v(u_0,v_0).
...
\]
è ortogonale al piano tangente, quindi è un vettore normale alla superficie.

\begin{center}
    \begin{tikzpicture}[scale=0.95]
        \draw[->] (-0.3,0) -- (3,0) node[right] {\(u\)};
        \draw[->] (0,-0.3) -- (0,2.3) node[above] {\(v\)};
        \draw[thick] (1.3,1) circle (0.8);
        \fill (1.3,1) circle (1.4pt) node[below] {\((u_0,v_0)\)};

        \draw[->,thick] (3.5,1) -- (4.8,1) node[midway,above] {\(\sigma\)};

        \begin{scope}[xshift=5.7cm]
            \draw[->] (-0.2,0) -- (3,0) node[right] {\(x\)};
            \draw[->] (0,-0.2) -- (0,2.4) node[above] {\(y\)};
            \draw[thick] plot[smooth cycle] coordinates {
                (0.4,0.6) (1.1,1.2) (2.2,1.1) (2.5,0.4) (1.4,0.1)
            };
            \fill (1.4,0.75) circle (1.4pt) node[below] {\(P\)};
            \draw[->,thick] (1.4,0.75) -- (2.25,1.1) node[right] {\(\sigma_u\)};
            \draw[->,thick] (1.4,0.75) -- (1.0,1.45) node[above] {\(\sigma_v\)};
        \end{scope}
    \end{tikzpicture}
\end{center}

\begin{snippetdefinition}{Elemento d'area}
    Sia
    \[
        \sigma:A\to\Sigma
    \]
    una parametrizzazione regolare. L'elemento d'area della superficie è
    \[
        dS=\|\sigma_u(u,v)\times\sigma_v(u,v)\|\,du\,dv.
    \]
    Quindi, per una funzione \(f:\Sigma\to\realnumbers\),
    \[
        \iint_\Sigma f\,dS
        =
        \iint_A
...
\subsection{Parametrizzazioni della sfera}
...
\begin{snippetexample}{Calotta superiore come grafico}
    La calotta superiore può essere parametrizzata come grafico della funzione
    \[
        z=\sqrt{1-x^2-y^2}.
    \]
    Precisamente,
    ...
    \[
        ...
        \right\}.
    \]
    Questa parametrizzazione descrive solo la calotta superiore aperta. Se si considera anche il bordo
    \[
        x^2+y^2=1,
    \]
    si ottiene l'equatore, cioè la curva che delimita la calotta.

    \begin{center}
        \begin{tikzpicture}[scale=0.95]
            \draw[->] (-1.7,0) -- (1.9,0) node[right] {\(x\)};
            \draw[->] (0,-0.3) -- (0,2.2) node[above] {\(z\)};
            \draw[thick] (-1.2,0) arc[start angle=180,end angle=0,radius=1.2];
            \draw[thick] (-1.2,0) arc[start angle=180,end angle=360,x radius=1.2,y radius=0.25];
            \draw[dashed] (-1.2,0) arc[start angle=180,end angle=0,x radius=1.2,y radius=0.25];
            \node at (0,1.55) {\(\Sigma\)};
            \node at (1.35,-0.35) {equatore};
        \end{tikzpicture}
    \end{center}
\end{snippetexample}

\begin{snippetexample}{Coordinate sferiche sulla sfera}
    Una parametrizzazione naturale della sfera unitaria è
    \[
        \sigma(\varphi,\theta)
        =
        \left(
    ...
    \[
        ...
        \theta\in[0,2\pi].
    \]
    Qui \(\varphi\) è l'angolo polare, mentre \(\theta\) è l'angolo azimutale.

    Osserviamo che:
    \begin{itemize}
        \item \(\varphi=0\) viene mandato nel polo nord;
        \item \(\varphi=\pi\) viene mandato nel polo sud;
        \item i lati \(\theta=0\) e \(\theta=2\pi\) vengono identificati.
    \end{itemize}
    Quindi questa parametrizzazione descrive tutta la sfera, ma non è iniettiva sul bordo del rettangolo dei parametri.

    Calcoliamo i vettori tangenti:
    \[
        \sigma_\varphi
        =
        \left(
    ...
    \[
        ...
        \sin\varphi.
    \]
    La parametrizzazione è regolare per
    \[
        0<\varphi<\pi,
    \]
    mentre non è regolare nei poli, dove \(\sin\varphi=0\).
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1]
            ...
            \draw (0,0) circle (1);
            \draw[dashed] (-1,0) arc[start angle=180,end angle=360,x radius=1,y radius=0.25];
            \draw (1,0) arc[start angle=0,end angle=180,x radius=1,y radius=0.25];
            \fill (0,1) circle (1.3pt) node[above] {polo nord};
            \fill (0,-1) circle (1.3pt) node[below] {polo sud};
            \draw[->,thick] (0,0) -- ({sin(55)},{cos(55)});
            \node at (0.65,0.65) {\(\varphi\)};
        \end{tikzpicture}
    \end{center}
    ...
    \[
        \sigma_\varphi\times\sigma_\theta
    \]
    è un vettore normale alla sfera. Per capire l'orientazione, calcoliamo nel punto
    \[
        (\varphi,\theta)=\left(\frac{\pi}{2},0\right).
    \]
    Si ha
    \[
        \sigma\left(\frac{\pi}{2},0\right)=(1,0,0),
    \]
    e
    \[
        \left(\sigma_\varphi\times\sigma_\theta\right)
        \left(\frac{\pi}{2},0\right)
        =
        (1,0,0).
    \]
    Quindi questa scelta di prodotto vettoriale dà la normale uscente.
\end{snippetexample}

Una superficie chiusa, come la sfera, non ha bordo. In tal caso, quando la superficie delimita un solido, ci sono due orientazioni naturali: la normale esterna e la normale interna.


\begin{snippetexample}{Cilindro circolare retto}
    Abbiamo \(x^2 + y^2 = R^2\) con \(R>0\)
    si può parametrizzare usando le coordinate cilindriche.
    Abbiamo \(\sigma(\theta,z) = (R\cos\theta, R \sin \theta, z)\)
    con \(\theta \in [0,2\pi]\) e \(z \in \realnumbers\).
    Se consideriamo un cilindro limitato, per esempio \(z \in [-1, 1]\),
    otteniamo una superficie con bordo, altrimenti il cilindro illimitato non ha bordo.
    Possiamo costruire tale oggetto prendendo una striscia di piano e identificando i due lati.
    Calcoliamo la normale al cilindro:
    \[
    ...
    \]
    la cui norma è \(R\). Quindi \(\text{d}S = R\,\text{d}\theta\,\text{d}z\).
    L'area è quindi
    \[
        \iint_{[0,2\pi] \times [-1, 1]} R\,\text{d}\theta\,\text{d}z = R2\pi-2
    \]
\end{snippetexample}

Notiamo che partendo da un foglio di carta, possiamo identificare punti
per formare un cilindro o una sfera, ma per la sfera serve che il foglio sia elastico,
a differenza del cilindro.
Questo è legato alla lunghezza d'arco, che non esiste per le superfici,
proprio perché serve qualcosa di elastico e che quindi non rispetta l'area
e le lunghezze.
Oltre al cilindro c'è il cono e la rigata delle tangenti come superficie sviluppabile.
Per essere sviluppabile serve che la curvatura di Gauss sia nulla.

\begin{snippetexample}{Toro}
    Il toro con \(r<R\)
    è dato dalla rotazione di una circonferenza rispetto ad un asse che non lo interseca.
    La sua parametrizzazione è
    \[
        \sigma(\theta, \varphi) = (
            (R+r\cos\varphi)\cos\theta, (R+r\cos\varphi)\sin\theta, r \sin\varphi
        )
    \]
    con \(\theta, \varphi \in [0,2\pi]\).
    Questo si può formare identificando i lati di un foglio a forma di tubo.
    Calcoliamo la sua area. Abbiamo
    \[
        \sigma_\theta \wedge \sigma_\varphi =
        \left(
            (R+r\cos\cos\varphi)r\cos\theta\cos\varphi,
            (R+r\cos\varphi)r\sin\theta\cos\varphi,
            (R+r\cos\varphi)r\sin\varphi
        \right)
    \]
    e la sua norma è \((R + r\cos\varphi)r\), quindi l'area è data da
    \[
        \iint_{[0,2\pi]}^2
        (R + r\cos\varphi)r\,\text{d}\theta\,\text{d}\varphi
        = 2\pi r \integral[0][2\pi][R+r\cos\varphi][\varphi] = (2\pi r)(2\pi R)
    \]
\end{snippetexample}

\begin{snippetexample}{Möbius strip}
    Il bordo del nastro è una curva che è connessa, è una sola componente.
    Per parametrizzarlo prendiamo una circonferenza sul piano \(xy\)
    e un segmento centrato su un punto della circonferenza, e facciamo ruotare il segmento
    e man mano che ruota lo facciamo anche girare con la metà della velocità
    \[
        \sigma(u, v) =(
    ...
    \]
    con \(u \in [0,2\pi]\) e \(v \in [-1, 1]\).
\end{snippetexample}

Se invece invertiamo l'ordine degli altri due lati invertendo l'orientamento
otteniamo la bottiglia di Klein, che non si può immergere in \(\realnumbers^3\).

% Spivak libro copertine

\begin{snippetexample}{Integrale di superficie}
    Calcolare
    \[
        \iint_\Sigma \frac{z+y^2}{\sqrt{1 + 4(x^2 + y^2)}}\,\text{d}S
    \]
    dove \[
        \Sigma = \{
            (x,y,z) \suchthat z=x^2 - y^2 , x^2 + y^2 \geq 1, x^2 + 4y^2 \leq 4
        \}
    \]
    Abbiamo il grafico di un paraboloide iperbolico come superficie,
    quindi \(\sigma(x,y) = (x,y, x^2 - y^2)\).
    Allora \[
        \text{d}S = \sqrt{1 + z_x^2 + z_y^2}\,\text{d}x\,\text{d}y = \sqrt{1 + 4(x^2 + y^2)}\,\text{d}x\,\text{d}y
    \]
    Il dominio di integrazione è % disegno 19/12/25
    l'intersezione fra il complementare di un cerchio e l'ellisse data.
    Questa ellisse ha semiasse \(2\) rispetto alle \(x\) e \(1\) rispetto a \(y\).
    Il dominio sono quindi quattro pezzi simmetrici, di cui ne chiamiamo uno \(\overline A\)
    e poi moltiplichiamo per \(4\).
    \begin{align*}
        \iint_\Sigma \frac{z+y^2}{\sqrt{1 + 4(x^2 + y^2)}}\,\text{d}S &=
        \iint_{\overline A} \frac{x^2 - y^2 + y^2}{\sqrt{1 + 4(x^2 + y^2)}}
        \sqrt{1 + 4(x^2 + y^2)}\,\text{d}x\,\text{d}y \\
        &= \iint_{\overline A} x^2 \,\text{d}x\,\text{d}y
    \end{align*}
    Per calcolare questo integrale calcoliamo l'integrale sull'ellisse e sottraiamo quello sul cerchio \(I_1 - I_2\).
    Calcoliamo quello sul cerchio usando le coordinate polari
    \begin{align*}
        I_2 &= \iint_{\{x^2 + y^2 \leq 1\}} x^2\,\text{d}x\,\text{d}y \\
        &= \integral[0][2\pi][
            \integral[0][1][
                \rho^3 \cos^2\theta
            ][\rho]
    ...
    \end{align*}
    Per fare \(I_2\) schiacciamo l'ellisse in un cerchio, quindi componiamo un'omotetia
    e poi le coordinate polari, quindi la trasformazione è
    \[
        \begin{cases}
            x = 2\rho \cos\theta \\
            y = \rho \sin \theta
    ...
    il cui Jacobiano è
    \[
        \jacobian \Phi = \begin{pmatrix}
            2\cos\theta & -2\rho \sin\theta \\
            \sin\theta & \rho\cos\theta
        \end{pmatrix}
    \]
    il cui determinante è \(2\rho\).
    Abbiamo quindi
    \begin{align*}
        I_2 &= \integral[][2\pi][
            \integral[0][1][
                \left(2\rho\cos\theta\right)^2 2\rho    
            ][\rho]
        ][\theta] = 2\pi
    \end{align*}
    Quindi l'integrale totale è \(2\pi - \frac{\pi}{2} = \frac{7}{4}\pi\).
\end{snippetexample}

\subsection{Flussi}

Sia \(\Sigma\) una superficie orientabile, \(F\) un campo vettoriale
definito su \(\Sigma\).
Per la definizione vera serve una \(2\)-forma.
Vogliamo misurare la quantità di "fluido" che scorre attraverso la superficie.
Mi interessano le componenti normali alla superficie, quindi definiamo
Dobbiamo poter scegliere il vettore normale in maniera consistente, quindi serve
che sia orientabile (a meno che non facciamo un rivestimento doppio).
\begin{align*}
    \iint_\Sigma F \cdot m \,\text{d}S
    &\triangleq \iint_A F(\sigma(u, v)) \cdot \frac{
        \sigma_u(u,v) \wedge \sigma_v(z,v)
    }{
        ||\sigma_u \wedge \sigma_v||
    }
    ||\sigma_u \wedge \sigma_v|| \,\text{d}u\,\text{d}v \\
    &= \iint_A F(\sigma(u, v)) \cdot \sigma_u(u,v) \wedge \sigma_v(u,v) \,\text{d}u\,\text{d}v
\end{align*}

Cambiando l'orientazione si inverte il segno del flusso.

\begin{snippetexample}{}
    Calcoliamo il flusso del campo \(F(x,y,z) = (0,0,z)\)
    uscente dalla sfera di raggio \(R\).
    Abbiamo
    \begin{align*}
        \iint_{S_R} F\cdot m_z\,\text d S &= \iint_{\substack{0 \leq \theta \leq 2\pi \\ 0 \leq \varphi \leq \pi}}
        (0,0, R\cos\varphi)\cdot \left(R\sin\varphi(R \sin\varphi\cos\theta, R \sin\varphi\sin\theta, R \cos\varphi)\right)
        \,\text d \theta \,\text d \varphi \\
        &= R^3 \integral[0][2\pi][
            \integral[0][\pi][\sin\varphi \cos^2\varphi][\varphi]
        ][\theta] = \frac43 \pi R^3
    \end{align*}
    che è anche il volume della sfera.
\end{snippetexample}

\begin{snippetexample}{}
    Calcoliamo il flusso uscente dalla superficie laterale del cilindro
    \[
        ...
        \end{cases}
    \]
    del campo \(F(x,y,z) = (\sin z, y, x^2)\).
    La parametrizzazione è \(\sigma(\theta, z) = (R \cos\theta, R \sin \theta, z)\)
    con \(0 \leq \theta \leq 2\pi\) e \(0 \leq z \leq 1\).
    Abbiamo \(\sigma_\theta \wedge \sigma_z = (R \cos\theta, R \sin\theta, 0)\)
    e l'integrale è
    \begin{align*}
        \iint_{S_R} F\cdot m_z\,\text d S &=
        \iint_{\substack{0 \leq \theta \leq 2\pi \\ 0 \leq z \leq 1}}
        F(\sigma(theta, z)) \cdot \sigma_\theta \wedge \sigma_z \,\text d \theta \,\text d \varphi \\
        &= \iint_{\substack{0 \leq \theta \leq 2\pi \\ 0 \leq z \leq 1}}
        \left(R \sin z \cos\theta + R^2 \sin^2 \theta\right) \,\text d \theta \,\text d \varphi \\
        &= R \pi^2
    \end{align*}
    Calcoliamo ora il flusso uscente dai due "tappi".
    Dobbiamo parametrizzare i tappi. Quello superiore con
    \(\sigma(x,y) = (x,y,1)\) e \(\sigma(x,y) = (x,y,-1)\).
    Il normale esterno è \((0,0,1)\) e \((0,0,-1)\).
    Quindi il flusso attraverso il tappo superiore è
    \begin{align*}
        \iint_{S_R} F(x,y,1)\cdot k\,\text d S &= 
        \iint_{S_R} F(x,y,1)\cdot k\,\text d x \,\text d y \\
        &= \iint_{x^2 + y^2 \leq R^2} x^2 \,\text d x \,\text d y
    \end{align*}
    e quello inferiore è
    \[
        -\iint_{x^2 + y^2 \leq R^2} x^2 \,\text d x \,\text d y
    \]
    quindi si annullano.
...
(Come si generalizza la normale di una ipersuperficie, e in pratica come si calcola? Con il determinante).
Prima avevamo dei campi la cui divergenza era \(1\), quindi usciva il volume.

Il teorema fondamentale del calcolo è un caso particolare del teorema della divergenza
(nella dimostrazione viene usato il teorema fondamentale del calcolo).

L'ipotesi che il dominio debba essere semplice rispetto a tutti gli assi può essere rilassata.
Dato un dominio che non è semplice rispetto a tutti gli assi, possiamo spezzarlo
e scomporre l'integrale su pezzi di dominio che sono semplici rispetto a tutti gli assi.
Per ciò dobbiamo notare che il flusso sul "taglio" non è nullo, ma siccome hanno
direzione opposta si elidono. Quindi possiamo rilassare la condizione all'unione finita di domini
semplici rispetto a tutti gli assi.
