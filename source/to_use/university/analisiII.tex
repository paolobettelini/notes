...
\stheorem{}{
    ...
    \begin{enumerate}
        \item se \(\text{det} Hf(a) < 0\) allora \(a\) è punto di sella;
        \item se \(\text{det} Hf(a) > 0\) e \(f_{xx}>0\) allora \(a\) è punto di minimo;
        \item se \(\text{det} Hf(a) > 0\) e \(f_{xx}<0\) allora \(a\) è punto di massimo;
    \end{enumerate}
...
Per \(n=2\) il segno di \(Hf(a)\) può essere determinato mediante il criterio dei minori
...
\sexample{}{
    ...
    \[
        Hf(\pm\sqrt2, \mp \sqrt2) = \begin{pmatrix}
            20 & 4 \\ 4 & 20
        \end{pmatrix}
    \]
