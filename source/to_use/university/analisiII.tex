...
\begin{snippettheorem}{Theorema di Heine-Cantor}
...
\begin{snippettheorem}{di dipendenza del punto tfisso del parametro}
...
\begin{snippetproof}{}
    Siccome questo spazio è finito dimensionale è isomorfismo allo spazio \(\realnumbers^{m \times n}\)
    ...
    in \(\realnumbers^n\). Siccome \(\realnumbers^n\) è completo, la successione converge.
...
\begin{snippettheorem}{Teorema del differenziale totale}
    Sia \(f \colon \Omega \to \realnumbers\), con \(\Omega \in \realnumbers^n\) aperto,
...
\begin{snippetexample}{}
    ...
    \(g\colon \realnumbers^3 \to \realnumbers^2\). Chiamiano \(x,y\) le coordinate di \(f\)
...
\begin{radioactive}
\begin{snippettheorem}{Teorema dell'incremento finto}
...
\(k \to (d^2f(a)h)k\).
Siccome è bilinare possiamo vederlo come \(d^2f(a) \colon \realnumbers^n \times \realnumbers^n \to \realnumbers^m\)
...
\begin{radioactive}
\begin{snippetdefinition}{Differenzaibile \(k\)-esimi}
    \[
...
\begin{radioactive}
Sia \(\Omega \subseteq \realnumbers^n\) un sottoinsieme qualcunque e sia \(f \colon \Omega \to \realnumbers\).
...
\begin{snippetexample}{}
    ...
    \[
        ...
        \end{cases}
    \]
    has no solution in any interval containing \(x=0\).
\end{snippetexample}

In generale, le soluzioni di un'equazione differenziali sono tante, di solito \(\infty\).
L'integrale generale di un'equazione differenziale è la faimglia di tutte le soluzioni
di un'equazione. Di solito si considera un problema di Cauchy.

\begin{snippettheorem}{Teorema di esistenza e unicità locale}
    Sia \(\Omega \subseteq \realnumbers^2\) aperto, \((x, y_0)\in\Omega \), \(f \colon \Omega \fromto \realnumbers\).
    Se \(f\) e \(\partial f / \partial y\) sono continue in\(\Omega\) (in particolare se \(f \in \mathcal{C}(\Omega)\)),
    llora esiste un intorno \(I\) di \(x_0\) in cui è definita una soluzione del problema di Cauchy
    \[
        \begin{cases}
            y' = f(x,y) \\
            y(x_0) = y_0
...
Supponiamo che \(f\) soddisfi le ipotesi del teorema di esistenza e unicità locale.
Allora, i grafici di due soluzioni distinte dell'equazione \(y' = f(x,y)\) non possono intersecarsi.
Se ciò succedesse, considerando il problema di Cauchy con condizione iinziale nel punto di intersezione,
avremmo due soluzioni.

\subsection{Equazioni a variabili separabili}

Sia \(y' = h(x)k(y)\) dove \(h\colon I \fromto \realnumbers\), \(k \colon J \fromto \realnumbers\) con \(I,J\) intervalli
tali che \(x_0 \in I\) e \(y_0 \in J\).
Supponiamo che \(h\in \mathcal C'(I)\), \(k \in \mathcal C'(J)\), così che se \(f(x,y) = h(x)k(y)\),
\(f \in \mathcal C'(I \times J)\).
In questo caso c'è esistenza e unicità locale della soluzione del problema di Cauchy.

Nel caso particolare \(k(y_0) = 0\), allora la funzione \(y = y_0\) è soluzione del problema di Cauchy.
Se \(k(y_0) \neq 0\), allora \(k(y) \neq 0\) per \(y \in V(y_0)\).
In particolare, se \(y(x)\) è la soluzione del problema di Cauchy, si ha che \(k(y(x)) \neq 0\)
per \(x \in U(x_0)\). Quindi in \(U(x_0)\) possiamo dividere per \(k(y(x))\).
Possiamo quindi integrare
\begin{align*}
    \frac{y'(x)}{k(y(x))} &= h(x) \\
    \integral[x_0][x][\frac{y'(t)}{k(y(t))}][t] &= \integral[x_0][x][h(t)][t] \\
    \integral[y_0][y(x)][\frac{1}{k(s)}][s] &= \integral[x_0][x][h(t)][t]
\end{align*}
ponendo \(y(t) = s\) nell'integrale a sinistra.
...
\begin{snippetexample}{}
    ...
    \[
        \begin{cases}
            y' &= e^{x-y} \\
            y(0) &= a \in \realnumbers
        \end{cases}
    \]
    Abbiamo che \(h(x) = e^x\) e \(k(y) = e^{-y}\) con \(\Omega = \realnumbers^2\).
    Siccome \(k(y) \neq 0\) allora non ci sono oluzioni costanti.
    Separando otteniamo \(e^y = e^x - C\) e imponendo il punto iniziale \(C = e^a - 1\).
    Otteniamo quindi \(y = \ln(e^x + e^a - 1)\), che è definita per \(e^x > 1-e^a\).
    Se \(1-e^a \leq 0\) la disuguaglianza è sempre soddisfatta.
    Se \(a \geq 0\), allora \(I = \realnumbers\). Se \(a < 0\), allora \(1-e^a > 0\), \(x > \ln(1 - e^a)\),
    e \(I = (\ln(1-e^e), +\infty)\).
    Per \(a < 0\) c'è un asintoto verticale.
\end{snippetexample}

\subsection{Equazioni differenziali del primo ordine}

\begin{snippettheorem}{}
    Sia \(I \subseteq \realnumbers\) intervallo, \(x_0\in I\), \(y_0 \in \realnumbers\), \(p,q \colon I \fromto \realnumbers\) continua
    in \(I\). Allora, il problema
    \[
        \begin{cases}
            y' + p(x)y &= q(x) \\
            y(x_0) &= y_0
        \end{cases}
    \]
    ha una e una sola soluzione, definita in \(I\) data da
    \[
        y(x) = e^{-\integral[x_0][x][p(t)][t]} \left[y_0 + \integral[x_0][x][q(t)e^{\integral[x_0][x][p(t)][t]}][t]\right]
    \]
\end{snippettheorem}
...
Considera \(y' + a(x) + b(x)y''\delta = 0\) con \(\delta \neq 0, 1\).
Allora \(y' = a(x)y - b(x)y^\delta = f(x,y)\) con \(f \colon \Omega \subseteq \realnumbers^2 \fromto \realnumbers^2\).
TODO
...
\begin{radioactive}
\begin{snippetdefinition}{Forme differenziali (1-forme)}
...
\begin{radioactive}
\begin{snippetdefinition}{Integrale su curva}
...
\begin{radioactive}
\begin{snippetexample}{Forma non esatta}
...
\begin{snippettheorem}{Condizione necessaria}
...
\begin{snippettheorem}{Forma differenziale irrotazionale}
    Sia \(F \in \mathcal{C}^1(\Omega; \realnumbers^n)\) irrotazionale
    se \[
...
Non è quella di Lebesgue. Un insieme finito è trscurabile.
...
di \(\bigcup Q_i\), in quanto possiamo sempre aumentare leggermente le grandezze dei rettangolo
...
\begin{snippetproof}{}
    ...
    di questi rettangoli siano minori di \(\varepsilon\).
    ...
    in cui \(f\) è continua.
    Siccome l'insieme di questi rettangoli è comaptto e \(f\) è continua,
    allora \(f\) è uniformemente continua.
    Raffiniamo eventualmente la partizione possiamo supporre che \(\text{diam}Q_{h,k} < \delta\).
...
Sia \(D \subseteq \realnumbers^2\) liimtato, \(Z \subseteq \realnumbers^2\) trascurabile.
...
\begin{snippetproof}{}
    ...
    Chiaramente \(\tilde f\) ha al più due discontinuità, e quindi integrabile in quanto continua
...
\begin{snippettheorem}{}
    ...
    \begin{enumerate}
        ...
        \item se \(|f|\) è integrabile allora il valore assoliuto dell'integrale
...
Sia \(\sigma \colon \overline A \subseteq \realnumbers^2 \fromto \realnumbers^3\)
con \(A\) aperto connesso (limitato per quando vogliamo calcolare gli integrali).
Imponiamo l'iniettività tranne che sui punti del bordo, cioè
\((u_1, v_1) \neq (u_2, v_2)\) e \(\sigma(u_1, v_1) = \sigma(u_2, v_2)\) allora
\((u_1, v_1),(u_2, v_2)\in \partial A\).
Abbiamo che \(\sigma_u, \sigma_v\) costituiscono una base di \(T_{\sigma(u,v)}\Sigma\).
Definiamo \(\sigma_u \wedge \sigma_v\) come il vettore normale alle superfici,
che è per forza non nullo, cioè stiamo completando una base.
In particolare, \(||\sigma_u \wedge \sigma_v||\)
rappresenta l'area del parallelogrande, quindi abbiamo
\[
    \text{d}S = ||\sigma_u \wedge \sigma_v||\,\text{d}u\,\text{d}v
\]
Quindi se vogliamo integrare
\[
    \text{Area}(S) \triangleq
    \iint_\Sigma \text{d}S = \iint_A ||\sigma_u \wedge \sigma_v||\,\text{d}u\,\text{d}v
\]
Se \(f \colon \Sigma \fromto \realnumbers\) il suo integrale di superficie
\[
    \iint_\Sigma f \text{d}S \triangleq \iint_A f(\sigma(u,v))||\sigma_u \wedge \sigma_v||\,\text{d}u\,\text{d}v
\]
dove \(\sigma \colon \overline A \fromto \realnumbers^3\)
è una parametrizzazione semplice e regolare di \(\Sigma\).

\begin{snippetexample}{}
    Sia \(f \colon A \subseteq \realnumbers^2 \fromto \realnumbers\)
    possiamo definire una parametrizzazione cartesiana per tale grafico, cioè
    \(\sigma(x,y) = (x,y, f(x,y))\).
    Abbiamo che \(\sigma_x = (1, 0, f_x)\) e \(\sigma_y = (0, 1, f_y)\)
    e quindi
    \[
        \sigma_x \wedge \sigma_y = (-f_x, -f_y, 1)
    \]
    e quindi la norma è data da \(\sqrt{1 + ||\gradient f||^2}\).
\end{snippetexample}
...
\begin{snippetexample}{Möbius strip}
    ...
    e un segmento centrato su un punto della circonferenza, e facciamo rutoare il segmento
...
\begin{snippetexample}{Integrale di superficie}
    ...
    \end{align*}
    Per fare \(I_2\) schiacciamo l'ellisse in un cerchio, quindi componimo un'omotetia
...
(Come si generalizza la normale di una ipersuperificie, e in pratica come si calcola? Con il determinante).
Prima avevamo dei campi la cui divergenza era \(1\), quindi usciva il volume.

Il teorema fondamenale del calcolo è un caso particolare del teorema della divergenza
(nella dimostrazioe viene usato il teorema fondamentale del calcolo).
