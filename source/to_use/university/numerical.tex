...
\subsection{Gauss-Seidel è il doppio più veloce di Jacobi in certi casi}
...
\subsection{Matrici semi-elementari (generalizzazione)}
...
la matrice originale arrebbe autovalore \(1 - 1 = 0\).
...
Il costo della raprpesentazione dell'inversa
è \[
    k^2(2n-1) + \mathcal O(k^3)
\]

\subsection{Sherman-Morrison-Woodbury}
...
\(UV^*\) è invertibile, cioè se e solo se \(V^*A^{-1}U\).
...
\subsection{Norme matriciali indotte dalla norma \(1\) e \(2\)}

Abbiamo visto la norma matriciale indotta dalla nora infinito. Studiamo ora i casi
con norma \(1\) e \(2\).
...
\begin{DispWithArrows*}
    ...
    &= \max_j ||\text{col}_j(A)||_1
\end{DispWithArrows*}
Pr completare e dimostrare l'uguaglianza
...
Per la norma indotta abbiamo \begin{align*}
...
\begin{snippetproof}{Decomposizione ai valori singolari}
    ...
    \begin{itemize}
        ...
        Ma allora, \begin{DispWithArrows*}
...
Nel caso normale, il modulo degli autovalori sono i valori singolari
dopo averli ordinati.

\pagebreak

\section{Teoria dell'approssimazione}
...
\begin{snippetproof}{}
    ...
    Scriviamo ora \begin{DispWithArrows*}
    ...
    Quindi possiamo aggiornare la maggiorazione a \begin{align*}
        |f(y) - f(x)| &\leq \tilde \varepsilon + 2||f||_{\infty, K}
        \frac{(y-x)^2}{\tilde \delta^2}
    \end{align*}
    Sostituendo questa troviamo che per monotonia \begin{DispWithArrows*}
    ...
    \]
    Otteniamo quindi la successione numerica che va a zero \begin{align*}
...
Vediamo il test di Korovkin sugli operatori di Bernstein \(B_n(\cdot)\) su \([0,1]\). Abbiamo \begin{enumerate}
    \item Vediamo se \(B_n(1)(x)\) converge a \(1\). Abbiamo
    \[
        \sum_{k=0}^n \binom{n}{k}x^k(1-x)^{n-k} = ((1 - x) + x)^n = 1
    \]
    e quindi \(\varepsilon_n^{(0)} = 0\).
    \item Vediamo se \(B_n(y)(x)\) converge a \(x\). Abbiamo \begin{align*}
    ...
    \item Vediamo se \(B_n(y^2)(x)\) converge a \(x^2\). Abbiamo \begin{align*}
        B_n(y^2)(x) &= \sum_{k=0}^n \binom{n}{k} \frac{k^2}{n^2} x^k(1-x)^{n-k} \\
        &= \frac{1}{n^2}\sum_{k=0}^n k^2 \binom{n}{k} x^k(1-x)^{n-k} \\
        &= \frac{1}{n^2}\sum_{k=0}^n k(k-1) \binom{n}{k} x^k(1-x)^{n-k}
        + \frac{1}{n^2}\sum_{k=0}^n k \binom{n}{k} x^k(1-x)^{n-k} \\
        &= \frac{n(n-1)}{n^2}x^2 + \frac{n}{n^2}x \\
        &= x^2 + \frac{x(1-x)}{n}
    \end{align*}
    quindi
    \[
    ...
    e
    \[
        ||\varepsilon_n^{(2)}||_{\infty, [0,1]} \leq \frac{1}{4n} \to 0.
    \]
\end{enumerate}
...
\begin{snippetexercise}{}
    ...
    \[
        \left(A\right)_{j,k} = \begin{cases}
            a + c & j=k \\
...
\begin{snippetsolution}{}
    ...
    \begin{align*}
        ...
        b   & a   & b   & a   & b   & \cdots & a+c
        \end{pmatrix} \\
        &= cI_n + b \underbrace{\begin{pmatrix}
        0 & 1   & 0   & 1   & 0   & \cdots & 1 \\
        1   & 0 & 1   & 0   & 1   & \cdots & 0 \\
        0   & 1   & 0 & 1   & 0   & \cdots & 1 \\
        1   & 0   & 1   & 0 & 1   & \cdots & 0 \\
    ...
    della matrice risultante, che i sono i due autovalori non banali rimanenti. Abbiamo quindi
    \begin{DispWithArrows*}
        \left[
        \begin{array}{c}
            bg^T+af^T \\ \hline
...
\begin{snippetexercise}{3/04/2025}
    Sia \begin{align*}
        A = \begin{pmatrix}
            B_1 & C \\
            0 & B_2
        \end{pmatrix}
    \end{align*}
    matrice a blocchi e siano \(m_1, m_2 \in \naturalnumbers^+\)
    tali che \(n < m_1 + n_2\) così da definire
    \(B_1 \in \matrices_{m_1}(\realnumbers)\)
    e \(B_2 \in \matrices_{m_2}(\realnumbers)\)
    e \[
        (C)_{i,j} = 1 + i
    \]
    ...
    \[
        ...
        \end{pmatrix}
    \]
    dove la diagonal di \(B_2\) è tutta costante tranen l'ultimo elemento, e gli elementi sono definiti come
    \[
        \alpha_j = 1 + \frac{1}{9 + j}, \quad
        \frac{1}{10} \leq \delta_j \leq \frac15
    \]
    \begin{enumerate}
        ...
        \item Mostra la convergenza. Qual'è il \(k\) minimo?.
    \end{enumerate}
\end{snippetexercise}
...
\begin{snippetsolution}{}
    \begin{enumerate}
        \item Dalla forma della matrice sappiamo che \[
            ...
            \]
            Lo si vede usando la formula generale sopra data.
            Studiamo quindi separamente questi spettri. I dischi di Gershgorin di \(B_1\) abbiamo
            \begin{align*}
                D_{1,m_1} &= \{ z \in \complexnumbers \suchthat |z-3| \leq 1\} = [2,4] \\
                D_i &= \{ z \in \complexnumbers \suchthat |z-3| \leq 2\} = [1,5], \quad 1 < i < m_1 \\
            \end{align*}
            siccome la matrice è Hermitiana e quindi gli autovalori sono positivi.
            ...
            dovrebbe stare sul bordo di tutti i dischi, cioè nell'intersezione, che non contiene \(1\) e \(5\).
            Quindi \(\sigma(B_1) \subseteq (1,5)\). Studiamo ora lo spettro di \(B_2\).
            Studiamo lo spettro di \(\tilde B = D^{-1} B_2 D\) con \(D\) diagonale che tanto non cambia.
            ...
            \[
                \tilde B = \begin{pmatrix}
                    ...
                    \delta_i \frac{d_2}{d_1} & \ddots & \ddots  \\
                    & \ddots & \ddots & \\
                    & & & 12
                \end{pmatrix}
                = \begin{pmatrix}
                    2 & \alpha_1 \sqrt{\frac{\delta_1}{\alpha_1}} & \\
                    \delta_i \sqrt{\frac{\alpha_1}{\delta_1}} & \ddots & \ddots  \\
                    & \ddots & \ddots & \\
                    & & & 12
                \end{pmatrix}
                = \begin{pmatrix}
                    2 & \alpha_1 \sqrt{\alpha_1\delta_1} & \\
                    \delta_i \sqrt{\alpha_1\delta_1} & \ddots & \ddots  \\
                    & \ddots & \ddots & \\
                    & & & 12
                \end{pmatrix}
            \]
            Abbiamo precisamente scelto i valori \(d_j\) per forzare una matrice Hermitiana.
            Sia \(\gamma_i =  \sqrt{\alpha_i \gamma_i}\). Calcoliamo ora i dischi di Gershgorin di questa matrice
            \begin{align*}
                D_1 &= \left[2 - \sqrt{\alpha_1 \delta_1}, 2 + \sqrt{\alpha_2 \delta_2}\right] \\
                D_i &= \left[2 - (\sqrt{\alpha_{i-1}\delta_{i-1}} + \sqrt{\alpha_i \delta_i}), 2 + (\sqrt{\alpha_{i-1} \delta_{i-1}} + \sqrt{\alpha_i \delta_i})\right], \quad 1 < i < m_2 \\
                D_{m_2} &= \left[12 - \sqrt{\alpha_{m_2 - 1} \delta_{m_2 - 1}}, 12 + \sqrt{\alpha_{m_2 - 1} \delta_{m_2 - 1}}\right]
            \end{align*}
            Chiaramente \(D_1 \subseteq D_2\) e quindi lo scartiamo.
            Diamo un bound \[
            ...
            Vogliamo anche stimare la somma \[
                ...
                = \sqrt{
                    \frac{10 + j}{9 + j}\frac15
                } + \sqrt{
                    \frac{11 + j}{10 + j}\frac15
                } \leq \sqrt{\frac{11}{50}} + \sqrt{\frac{12}{55}}
            \]
            per \(i=2\). Quindi, \[
                \tilde D = \left[
                    2 - \sqrt{\frac{11}{50}} + \sqrt{\frac{12}{55}}, 2 + \sqrt{\frac{11}{50}} + \sqrt{\frac{12}{55}}
                \right]
                \union \left[
                    12 - \sqrt{\frac{\alpha_{n_2-1}}{5}}, 12 + \sqrt{\frac{\alpha_{n_2-1}}{5}}
                \right]
            \]
            Il raggio più grande del secondo termine avviene per \(m_2 = 2\), così otteniamo una stima globale
            per qualsiasi grandezza, cioè raggio \(\sqrt{\sfrac{11}{55}}\).
            Questo può essere usato per controllare che l'ultimo cerchio sia isolato rispetto agli altri, cosa che è.
            Inoltre, per il terzo teorema di Gershgorin, siccome i due intervalli sono disgiunti, il secondo intervallo lo possiamo considerare
            aperto.
        \item Per il secondo teorema di Gershgorin, \(D\) è unico cerchio e quindi contiene un unico autovalore
        \(\lambda\) e siccome \(D \supset D_1\) è precisamente il raggio spettrale.
        ...
        \[
            ...
            \end{pmatrix}
        \]
        dove \(\begin{pmatrix}Z \\ Y\end{pmatrix}\) è un vettore a blocchi, quindi
        \(B_1 Z + C_y = b_1\) e \(B_2 X = b_2\). Dobbiamo capire il costo di \(B_2 Y = b_2\).
        Possiamo adoperare l'algoritmo di Gauss e poi risolvere con sostituzione all'indietro.
        % Magari qua mettere tutti i calcoli per tutti i passaggi per le prossime cose.
        La parte di eliminazione di Gauss costa di \(m_2\).
        Per l'eliminazione forward abbiamo 3 prodotti e de somme.
        Ricorsivamente per \(i\) abbiamo il costo dell'eliminazione di \(5(n-1)\).
        Per la back substitution troviamo \(1+3(n-1)\), quindi in totale \(8n - 7\) operazioni.
        Per \(CY\) troviamo un costo \(n\) (assengnazione dei valori).
        Quindi \(B_1Z = b_1 - CY\) quindi operazione tra vettori \(m_2 \times 1\).
        Per trovare il costo risolvo con \(B_1 = 8m_1 - 7\). Calcolo \(CY = m_2\), in totale \((8m_1 - 7)+m_2\).
        Il costo totale è quindi \[
            (8m_1 - 7)+m_2 + (8m_2 - 7) = 9m_1 + 9m_2 - 14
        \]
        \item Sia \(A=M-N\) con \[
            M = \begin{pmatrix}
                B_1 & 0 \\ 0 & B_2
            \end{pmatrix}, \quad D = M^{-1}N
        \]
        con \[
            \lim_{n \to +\infty} (M^{-1} N)^k = 0
        \]
        Abbiamo \[
            M^{-1} = \begin{pmatrix}
                B_1^{-1} & \\
