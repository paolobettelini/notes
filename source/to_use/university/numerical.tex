...
\begin{align*}
    L &= E_1^{-1} \cdots E_{n-1}^{-1} = (I-m_1e_1^T) \cdots (I-m_ne_{n-1}^T) = I - \sum_{j=1}^{n-1}m_je_j^T \\
\end{align*}
e quindi
\[
    L = \left(
    \begin{tikzpicture}[
        ...
        scale=0.5,             % Riduce le distanze tra gli elementi
        transform shape,       % Ridimensiona anche il testo e i simboli
        every node/.style={inner sep=1pt} % Riduce lo spazio attorno al testo
    ]
        \node at (0, 0) {\Large $1$};
        \node at (8, -8) {\Large $1$};
