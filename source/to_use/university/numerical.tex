...
\begin{snippetdefinition}{Matrice elementare di Hauseholder}
    Sia \(v \in \complexnumbers^n\) con \(v \neq 0\).
    La \emph{matrice elementare di Hauseholder} è definita come
    \[
...
\begin{snippetproof}{Fattorizzazione \(QR\)}
    ...
    \]
    Il primo blocco è quindi la matrice unitarie e il secodno una matrice di Hauseholder.
...
\begin{snippetproof}{}
    ...
    per una matrice di Hauseholder, cioè il costo di \((I + xy^*) c\)
...
\begin{snippettheorem}{}
    Sia \(P \in \matrices_{n\times n}(\complexnumbers)\). Allora,
    \[
        \lim_n P^k = 0
    \]
...
\begin{snippetproof}{}
    Consideriamo inizialmente \(P\) diagonalizzabile.
    Abbiamo quindi \(P = TDT^{-1}\)
    dove \(D\) è la matrice diagonale degli autovalori \(\lambda_i\).
    Abbiamo quindi \[
        P^k = \prod_k TDT^{-1} = T D^k T^{-1}
    \]
    Quindi, se esiste, per continuità abbiamo
    \[
        \lim_n P^k = T \left(\lim_n D^k\right) T^{-1}
    \]
    Ma dalla forma di \(D\), il limite tende a \(0\)
    se e solo se \(|\lambda_j| < 1\).
    Ci serve quindi che il raggio spettrale sia minore di \(1\).
    Sia \(P\) non diagonalizzabile.
    ...
    Nella forma di Schur, possiamo permutare gli elementi della diagonale come voglio,
    quindi scegliamo l'ordine come
    \[
        |\gamma_j| = |\gamma_{j+1}| = \cdots = |\gamma_q| \leq |\gamma_{q+1}|
    \]
    Dove \(j\) il primo indice tale che ciò è vero.
    Da questa catena troviamo che
    \begin{align*}
        |\gamma_j| \leq |\gamma_j| + \frac{\varepsilon}{N}
        < |\gamma_j| + \frac{2\varepsilon}{N},
        \quad \varepsilon = |\gamma_{q+1}| - |\gamma_{j}|
    \end{align*}
    per \(N < 1 + 1 - j\).
    L'idea è la seguente: se la matrice è diagonalizzabile, allora
    almeno due termini sulla diagonale coincidono.
    Noi li vogliamo a due a due distinti, quindi ogni qualvolta che ne trovo due
    che coincidono, aggiungo un valore ad uno tale da non fargli superare il successivo nella catena.
    Imponiamo che \(\gamma_n < 1\) in maniera tale da avere tutti numeri non negativi, a due a due distinti,
    e tutti minori di 1.\\
    Notiamo che \(|T| \leq |T^k|\) per espansione del prodotto e applicando la disuglianza triangolare.
    Passando all'esponente abbiamo
    \[
        0_{\matrices_{n \times n}(\complexnumbers)}
        \leq |T| \leq |T^k| = |T| \cdot |T^{k-1}|
        \leq |T|^k \leq |\hat T|
    \]
    Ma siccome \(\hat T\) è diagonalizzabile, per il caso canale il suo limite tende a zero,
    e quindi il limite della catena collassa a 0.
\end{snippetproof}
