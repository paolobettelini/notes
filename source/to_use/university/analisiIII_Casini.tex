...
\begin{radioactive}
\begin{snippetdefinition}{Convergenza uniformemente di Cauchy}
    Sia \(\{f_n\}_{n\in\naturalnumbers}\) una successione di funzioni.
...
\begin{snippettheorem}{Convergenza uniforme e convergenza uniformemente di Cauchy}
    Sia \(\{f_n\}_{n\in\naturalnumbers}\) una successione di funzioni.
...
\begin{snippettheorem}{}
    Sia \(f_n\colon [a,b] \to \realnumbers\) una successione di funzioni R-integrabili
...
\begin{snippettheorem}{}
    Sia \(f_n\colon [a,b] \to \realnumbers\) una successione di funzioni derivabili.
    Supponiamo che:
    \begin{enumerate}
...
\begin{radioactive}
\begin{snippettheorem}{Monotone convergence theorem for non-negative measurable functions}
...
\begin{snippetdefinition}{Integrabilità di una funzione positiva}
...
Per esempio \(\{1/n^2\} \in L^1(\naturalnumbers)\) ma \(\{1/n\} \notin L^1(\naturalnumbers)\)

\begin{snippetdefinition}{Integrabilità di una funzione}
    Sia \(f\colon X \to \realnumbers\) misurabile.
...
\begin{snippettheorem}{Teorema della convergenza dominante}
    Sia \(f_n \colon X \to \realnumbers\) misurabile e sia \(f = \lim_n f_n\).
...
\begin{snippetproof}{}
    \(f_n\) sono integrabili in quanto \(|f_n| \leq g\) che è integrabile, quindi sarà finito anche
...
Se \(A \subseteq X\) con \(A\) integrabile e \(f\colon X \to \realnumbers\)
...
\(\realnumbers\). Sia allora \(I=(a,b)\) (non cambia se incluso o meno)
e denotiamo \(l(I) = b-a\). Sia anche \(E \subset \realnumbers\). Diamo la \emph{misura esterna}
\[
    \mu^*(E) = \inf \left\{
        \sum_{n=1}^\infty l(I_n) \suchthat E \subset \bigcup_n I_n
    \right\}
\]
...
\begin{snippetdefinition}{Misurabile secondo Lebesgue}
...
Abbiamo quindi l'algebra \((\realnumbers, \mathcal{M}, \mu)\). Abbiamo pronta la teoria dell'integrazione
...
\(\realnumbers\) con la misura di Lebesgue è uno spazio di misura completo.

\begin{snippetproposition}{}
    Sia \(A \subseteq \realnumbers\) aperto. Allora
...
\begin{snippetproof}{}
    ...
    \[
        I_x = \left\{\bigcup I \suchthat x\in I\right\} \subseteq A
    \]
...
\begin{snippetaxiom}{Assioma della scelta}
...
In \(\realnumbers\) con la misura di Lebesgue, sia \(X = [0, 1)\)
e definiamo \[
...
\[
    \int_{\realnumbers} 1_{\mathbb{Q}}\,d\mu = \mu(\mathbb{Q}) = 0
\]

\begin{snippettheorem}{}
    Sia \(f\colon [a,b] \to \realnumbers\) Riemann-integrabile. Allora
    \begin{enumerate}
...
\begin{snippettheorem}{}
    Sia \(f\colon \realnumbers \to [0, +\infty)\) misurabile tale che
...
\begin{snippetproof}{}
    ...
    Quindi
    \begin{align*}
        \lim_n \int_{\realnumbers} f_n\,d\mu
        &= \lim_n \int_{\realnumbers} f1_{[a, c_n]}\,d\mu \\ 
        &= \lim \int_{[a, c_n]} f\,d\mu \\
        &= \lim \integral[a][c_n][f][\mu] \\
        &= \lim \integral[a][c_n][f(x)][x] \\
        &= \lim \int_{\realnumbers} f1_{[a, +\infty)}\,d\mu \\
        &= \int_{[a, +\infty)} \,d\mu
    \end{align*}
\end{snippetproof}
...
Lo spazio di probabilità è una misura \((\realnumbers, \mathcal{B}, \mathbb{P})\)
...
La misura ha la proprietà che \(\mathbb{P}(\realnumbers) = 1\).
Se prendiamo \(x\in \realnumbers\), \(\delta_x(\realnumbers) = 1\) (misura di Dirac).

Costruiamo una funzione \(F \colon \realnumbers \to [0,1]\) tale che
\[
...
\begin{enumerate}
    ...
    \[
        \bigcup I_n = \realnumbers
    \]
    quindi la probabilità (misura) dell'unione è data dal limite
    \[
        \mathbb{P} \left(
            \bigcup I_n
        \right) = \lim_n \mathbb{P}(I_n) = \mathbb{P}(\realnumbers)
        = \lim_n F(x_n) = 1
    \]
...
Una funzione \(F \colon \realnumbers \to [0,]\) che soddisfa \((1), (2), (3)\) è detta
una funzione di distribuzione (anche funzione di ripartizione della probabilità \(\mathbb{P}\)).
Sia dunque \(F\) una tale funzione. Allora esiste una probabilità \(\mathbb{P}\) su \(\realnumbers\)
...
\[
    ...
    \right\}
\]
Un sottoinsieme \(E \subseteq \realnumbers\) è \(F\)-misurabile
se
\[
    \forall A \subseteq \realnumbers, \mu_F^*(A) = \mu_F^*(A \cap E) + \mu^*_F(A \cap E^c)
\]
...
\section{Misura di Lebesgue su \(\realnumbers^n\)}

Consideriamo ora un insieme della forma (che chiamiamo per semplicità rettangolo)
\[
    J = (a_1, b_1) \times (a_2, b_2) \times \cdots \times (a_n, b_n) \subseteq {\realnumbers}^n
\]
Definiamo l'area come
\[
    \text{area}_n(J) = \prod_{i=1}^n (b_i-a_i)
\]
Consideriamo quindi \(E \subseteq \realnumbers^n\) e definiamo la misura n-dimensionale
\[
    \mu_n^*(E) = \inf\left\{
...
\begin{enumerate}
    ...
    \[
        \mu_n(E) = \mu_n^*(E)
    \]
    che è la misura di Lebesgue in \({\realnumbers}^n\)
\end{enumerate}
è la stessa cosa ma siamo partiti dall'area piuttosto che dalla lunghezza.
Quindi, \(({\realnumbers}^n, {\mathcal{M}}_n, \mu_n)\) è l'oggetto con cui abbiamo a che fare,
e abbiamo quindi la teoria dell'integrazione.

Una funzione \(f\in L^1({\realnumbers}^n)\),
cioè una funzione integrabile in \({\realnumbers}^n\).
...
\begin{snippettheorem}{Teorema di Fubini}
    Sia \(f \colon {\realnumbers}^n = {\realnumbers}^k \times {\realnumbers}^{n-k} \to {\realnumbers}\)
    misurabile e integrabile.
    \begin{enumerate}
        \item \(f_x(y) \colon {\realnumbers}^{n-k} \to {\realnumbers}^n\)
        è integrabile per quasi ogni \(x\in {\realnumbers}^k\).
        \item
        \[
            G(x) = \int_{{\realnumbers}^{n-k}} f_x(y)\,d\mu_{n-k}(y)
        \]
        è definita quasi ovunque, è integrabile in \({\realnumbers}^{k}\).
        \item \begin{align*}
            \int_{{\realnumbers}^{k}} G(x)\,d\mu_n(x) &= \int_{{\realnumbers}^{n}} f\,d\mu_n \\
            &= \int_{{\realnumbers}^{k}} \left(
...
\begin{snippettheorem}{Teorema di Tonelli}
    Sia \(f \colon {\realnumbers}^n = {\realnumbers}^k \times {\realnumbers}^{n-k} \to [0, +\infty)\) misurabile.
    Allora
    \[
        G(x) = \int_{{\realnumbers}^{n-k}} f_x(y)\,d\mu_{n-k}(y)
    \]
    (che potrebbe assumere valore infinito)
    è misurabile in \({\realnumbers}^k\) e
    \begin{align*}
        \int_{{\realnumbers}^{k}} G(x)\d\mu_k(x) = \int_{{\realnumbers}^{n}} f\,d\mu_n
    \end{align*}
\end{snippettheorem}
...
\begin{snippetexercise}{}
    ...
    \]
    \begin{enumerate}
        \item Determinare per quali \(\alpha\) la serie converge in \(\realnumbers\):
        Abbiamo che
        \[
            \frac{n^\alpha x}{x^4 + n^2} \sim \frac{C}{n^{2-\alpha}}
        \]
        quindi converge per \(\alpha < 1\).
        \item Determinare per quali \(\alpha\) la somma è integrabile in \(\realnumbers\):
...
\begin{snippetexercise}{}
    ...
    \]
    Mostrare che converge uniformrmente in \(\realnumbers\) (non fatto qui).
    Mostriamo che \(S \in L^1(\realnumbers)\). Notiamo che il termine è una funzione dispari,
...
\begin{snippetexercise}{}
    ...
    \begin{align*}
        \int_E f\,d\mu_2 &= \int_{{\realnumbers}^2} f\chi_E \,d\mu_2 \\
        &= \int_{{\realnumbers}} \left(
            \int_{{\realnumbers}} f(x,y)\chi_E(x,y)\,d\mu(y)
        \right)\,d\mu(x) \\
        &= \int_{{\realnumbers}} \left(
            \int_{{\realnumbers}} f(x,y)\chi_{E_x}(y)\,d\mu(y)
        \right)\,d\mu(x) \\
        &= \int_{{\realnumbers}} \left(
        ...
        \right)\,d\mu(x)
    \end{align*}
    Chiamiamo la \(x\)-sezione \(E_x = \{ y\in \realnumbers \suchthat (x,y) \in E\}\).
    Siccome \(E_x\) è spesso vuoto (fuori dall'area), definiamo
    \[
        A = \{x\in \realnumbers \suchthat E_x \neq \varnothing\}
    \]
    Quindi
    \begin{align*}
...
\begin{snippetexercise}{}
    ...
    è integrabile in
    \[
        E = \left\{(x,y) \suchthat x>0, 0<y< \min\left(x^3, \frac{1}{x}\right)\right\}
    \]
...
Se \(X = \realnumbers^n\) con la norma euclidea, allora \(K\) chiuso e limitato
...
\begin{snippetproof}{Dimostrazione della seconda}
...
\begin{snippetdefinition}{Totalmente limitato}
...
\begin{snippetproof}{Dimostrazione di un verso}
...
Sia
\[
    \mathcal{C}(X) = \{f\colon X \to \realnumbers\}
\]
...
\[
    B = \{f \in \mathcal{C}(X) \suchthat ||f||_\infty \leq 1\}
\]
...
\begin{snippetexercise}{}
    ...
    è integrabile in
    \[
        E = \{(x,y) \suchthat x>0, y>0, x+2y + z^2 < 1\}
    \]
    ...
    \begin{align*}
        E_{(x,y)} &= \{z \in \realnumbers \suchthat (x,y,z) \in E^+\} \\
        &= \{z\in\realnumbers \suchthat 0 < z < \sqrt{1 - x - 2y}\}
    \end{align*}
    e
    \begin{align*}
        A =& \{(x,y) \suchthat E_{(x,y)} \neq \varnothing\} \\
        &= \{(x,y) \suchthat x>0 \land y>0 \land x + 2y < 1\}
    \end{align*}
    Avremo quindi
    \begin{align*}
...
\begin{snippetdefinition}{Sottoinsieme equicontinuo}
    Un sottoinsieme \(\mathcal{F}(K) = \{f\colon K \to \realnumbers\} \subseteq C(K)\)
...
\begin{snippetdefinition}{Sottoinsieme equilipschitziano}
    Un sottoinsieme \(\mathcal{F}(K) = \{f\colon K \to \realnumbers\} \subseteq C(K)\)
    del sottospazio normato delle funzioni continue su spazio metrico \(K\)
...
\begin{snippetexample}{}
    Le funzioni \(f_n \colon [0, 2\pi] \to \realnumbers\).
    \[
        f_n(x) = \frac{\sin(nx)}{n}
    \]
...
\begin{snippettheorem}{Teorema di Ascoli - Arzelà}
...
\begin{snippetproof}{Teorema di Ascoli - Arzelà}
    \begin{iffproof}
        \item \(\mathcal{F}\) è compatto, quindi è totalmente limitato.
        ...
        \begin{align*}
            ...
            \leq 3\varepsilon
        \end{align*}
        \item \(\mathcal{F}\) è equicontinuo. Vogliamo mostrare che
...
%\begin{snippetdefinition}{Equazione differenziale di primo ordine in forma normale}
%    Una equazione differenziale del primo ordine in forma normale
%    è
%    \[
%        y' = f(x,y)
%    \]
%    con \(f\colon A \subseteq \realnumbers^2 \to \realnumbers\)
%    dove \(A\) è aperto, \(f\) continua.
%
%\end{snippetdefinition}

%Una soluzione di tale equazione è una coppia \((\varphi, I)\)
%dove \(I\) è un intervallo e \(\varphi: I \to \realnumbers\)
%tale che
%\begin{enumerate}
...
\begin{snippettheorem}{Teorema fondamentale}
...
\begin{snippettheorem}{Teorema di Peano}
...
\begin{snippetdefinition}{Compattezza relativa}
    Uno spazio \(D \subseteq\) metrico è relativamente compatto se la sua chiusura
    è compatta.
\end{snippetdefinition}

\begin{snippettheorem}{Teorema di Schauder}
...
\begin{snippetproof}{Teorema di Peano}
    ...
    e
    \begin{align*}
        C &= \{\varphi \in \mathcal{C}(I_\delta) \suchthat ||\varphi - y_0||_\infty \leq \beta\} \\
        &= B_\beta(y_0)
    \end{align*}
...
Consideriamo sempre \(f\colon A \subseteq \realnumbers^2 \to \realnumbers\) da un aperto, continua.
...
\[
    ...
    \suchthat |f(x,y_1) - f(x, y_2)| \leq L|y_1 - y_2|,
...
\begin{snippetdefinition}{Prolungamento}
...
\begin{snippetdefinition}{Soluzione massimale}
...
\begin{snippetproposition}{Proprietà della soluzione massimale}
    \begin{enumerate}
        \item esiste;
        ...
        dove \(K\) è compatto,
        \[
            \exists \delta >0 \suchthat \forall x \in (a, a+\delta) \text{ e } (b-\delta, b)
        \]
...
\begin{snippettheorem}{}
    Sia \(f \colon A \subseteq \realnumbers^2 \to \realnumbers\) continua e localmente
    lipschitziana in \(A\) aperto.
    Sia \((a,b) \times \realnumbers \subseteq A\). Se
    \[
        \forall [\alpha, \beta] \subseteq (a,b), \exists A,B>0 \suchthat
        |f(x,y)| \leq A + B|y|, \forall x\in [\alpha, \beta], \forall y \in \realnumbers
    \]
...
\begin{snippetexample}{}
    ...
    tale che \([\alpha, \beta] \times \realnumbers \subseteq A\).
...
\begin{snippetexample}{}
    Prendiamo \(y' = x + y^3 / (1 + y^2)\) allora \(A = \realnumbers^2\).
...
\begin{snippetexample}{}
    Sia \(y' = a(x)y + b(x)\) con \(a,b \colon I \to \realnumbers\) continue.
    La funzione \(f(x,y)\) è definita in \(I \times \realnumbers \to \realnumbers\).
...
\begin{snippetlemma}{Lemma di Gronwall}
    Sia \(\varphi \colon I \to \realnumbers\) continua, \(\varphi(x) \geq 0\)
...
\begin{snippetproof}{Teorema sublinearità}
    Abbiamo \((a, b) \times \realnumbers \subseteq A\) e abbiamo una soluzione massimale
    ...
    \[
        \exists \delta \suchthat \forall x\in (\gamma - \delta, \gamma),
        (x, \varphi(x)) \notin K
    \]
...
\begin{snippetdefinition}{Equazioni di Clairaut}
...
\begin{snippetdefinition}{Equazioni di Riccati}
...
\begin{snippetdefinition}{Senza nome}
...
\begin{snippetproposition}{Equazione di Bernoulli}
    Equazione della forma
    \[
...
\begin{snippetproposition}{Equazioni lineari del primo ordine}
    \[
        y'(x) + a(x) y(x) = b(x)
    \]
    ha soluzione
...
\begin{snippetexercise}{}
    ...
    Notiamo che \(\realnumbers \times [a,b] \subset \realnumbers^2\).
...
\begin{snippetexercise}{}
    ...
    assicurare che la soluzione massimale sia definita su tutto \(\realnumbers\).
    ...
    mentre in \(-2\)
    \[
        \integral[\lambda][-2][\frac{1}{t\log(2 + t)}][t] \in \realnumbers
    \]
...
\begin{radioactive}
\begin{snippetdefinition}{Curva}
    Una \emph{curva} è una funzione
    \(\varphi \colon [a,b] \subseteq \realnumbers \to \realnumbers^n\) che sia continua.
    L'immagine \(\varphi([a,b])\) è detto il \emph{sostegno} della curva.
\end{snippetdefinition}
\end{radioactive}

\begin{radioactive}
\begin{snippetdefinition}{Curva chiusa}
...
Una retta in \(\realnumbers^n\)
\[ r(t) = x_0 + v(t-t_0), \quad x_0, v \in \realnumbers^n \land t\in [a,b]\]
è anche una curva.

\pagebreak

\begin{radioactive}
\begin{snippetdefinition}{Retta tangente ad una curva}
...
\begin{radioactive}
\begin{snippetexample}{}
    Sia \(\varphi(t) = (t^2, t^3)\) da \([0,1] \to \realnumbers^2\) con \(t_0 = \frac{1}{2}\).
...
\begin{radioactive}
\begin{snippetdefinition}{Curva regolare a tratti}
...
\end{snippetdefinition}
Quindi la derivata può avere un numero finito di salti
in quanto una partizione è finita.
\end{radioactive}

\begin{radioactive}
\begin{snippetdefinition}{Equivalenza fra curve}
    Supponiamo di avere due curve \(\varphi_1 \colon I_1 \to \realnumbers^n\)
    e \(\varphi_2 \colon I_2 \to \realnumbers^n\). Diciamo che \(\varphi_1\)
    è equivalente a \(\varphi_2\) se esiste
    \(\sigma \colon I_1 \to I_2\) tale che:
    \begin{enumerate}
    ...
    \end{enumerate}
    Si può mettere anche l'immagine del diagramma commutativo.
\end{snippetdefinition}
\end{radioactive}
...
\begin{snippetproposition}{}
    Supponiamo di avere due curve \(\varphi_1 \colon I_1 \to \realnumbers^n\)
    e \(\varphi_2 \colon I_2 \to \realnumbers^n\). Se \(\varphi_1 \sim \varphi_2\) allora
...
\begin{snippetexample}{}
    ...
    Sia ora \(\varphi \colon [a,b] \to \realnumbers^n\)
    e
    \[
...
\begin{radioactive}
\begin{snippetdefinition}{Verso di percorrenza}
...
Sia \(\varphi \colon [a,b] \to \realnumbers^n\) e sia \(P\) una partizione di \([a,b]\)
e indichiamo
\[
...
\begin{radioactive}
\begin{snippetdefinition}{Lunghezza della curva}
...
\begin{radioactive}
\begin{snippetproof}{Del teorema}
    \begin{enumerate}
        \item     \begin{align*}
            L(\varphi, P) &= \sum_{i=1}^n ||\varphi(t_i) - \varphi(t_{i-1})|| \\
...
\begin{snippetexample}{Curva non rettificabile}
    % TODO immagine
    La curva \(\varphi\colon [0,1] \to \realnumbers^2\)
...
\begin{snippetproposition}{Curva parametrica in forma polare}
...
\begin{radioactive}
\begin{snippetdefinition}{Spazio duale di \({\realnumbers^n}\)}
    Lo spazio \(\left(\realnumbers^n\right)'\)
    è lo \emph{spazio duale} di \(\realnumbers^n\),
    ossia l'insieme dei funzionali lineare da \(\realnumbers^n\)
    a \(\realnumbers\).
\end{snippetdefinition}
...
\begin{align*}
    f(x) &= (x,y), \quad y\in \realnumbers^n \\
    &= \sum x_i y_i
\end{align*}
che è il prodotto interno fra due vettori di \(\realnumbers^n\).
In particolare, in tale spazio vi è
\[
    \text{d}x_i \in \left(\realnumbers^n\right)'
\]
definita da
\[
...
\begin{snippetdefinition}{Forma differenziale}
    Sia \(\Omega \subseteq \realnumbers^n\).
    Allora una funzione \(\omega \colon \Omega \to (\realnumbers)'\)
...
\]
dove \(\alpha_i \colon \Omega \to \realnumbers\) che devono essere continue.
\end{radioactive}

\begin{radioactive}
\begin{snippetexample}{Forma differenziale}
    La funzione
    \[
        \omega(x,y,z) = x^2\text{d}x - e^z \text{d}y + (xyz)\text{d}z
    \]
    è una forma differenziale da \(\realnumbers^3\) a \((\realnumbers^3)'\).
...
\begin{radioactive}
\begin{snippetexample}{Forme differenziali indotte da funzioni differenziabili}
    Possiamo ottenere una forma differenziale prendendo una funzione
    \(F \colon \Omega \to \realnumbers\) che sia differenziabile.
...
\begin{radioactive}
\begin{snippetdefinition}{Differenziale esatto}
    Una forma differenziale \(\omega \colon \Omega \to (\realnumbers^n)'\)
    è detta \emph{esatta} se esiste una funzione
    \(F \colon \Omega \to \realnumbers\) differenziabile
...
\[
    ...
    \end{cases}
\]
gli insiemi connessi in \(\realnumbers\) sono solo gli intervalli.
In \(\realnumbers^n\) al posto degli intervalli usiamo gli insiemi connessi.

\begin{snippetdefinition}{Insieme connesso}
...
\begin{snippetproposition}{}
    Sia \(F\colon \Omega \to \realnumbers\) differenziabile
    e \(\Omega\) connesso.
...
\begin{snippetdefinition}{Integrale su curva}
    Consideriamo \(\omega \colon \Omega \to (\realnumbers^n)'\)
...
\begin{snippetexample}{Integrale con forma differenziale esatta}
    \begin{align*}
        \int_\varphi \text{d}F
        &= \integral[a][b][
            \sum_{j=1}^m
...
\begin{tikzcd}
{[a,b]} \arrow[r, "\varphi", bend left] \arrow[rr, "H"', bend right] & \Omega \arrow[r, "F", bend left] & \realnumbers
\end{tikzcd}
\end{center}
...
\begin{snippetproposition}{Proprietà}
    \begin{enumerate}
        \item \emph{linearità}
        \[
        ...
        allora
        \[
            \int_{\tilde{\varphi}} \omega = \pm \int_{\varphi} \omega
        \]
        ...
        \item siano \(\varphi_i \colon [a,b] \to \realnumbers^n\)
        e \(\varphi_2 \colon [c,d] \to \realnumbers^n\)
        tale che \(\varphi_1(b) = \varphi_2(c)\).
        Allora definiamo \(\varphi_1 \cup \varphi_2 \colon I \to \realnumbers\)
...
\begin{snippetdefinition}{Insieme connesso per archi}
    Un sottoinsieme \(\Omega \subseteq \realnumbers^n\) aperto è detto
...
\begin{snippettheorem}{}
    Un insieme \(\Omega \subseteq \realnumbers^n\)
...
\begin{snippetproof}{}
    Sia \[S_{p,q} = \left\{\varphi\colon I \to \Omega \suchthat \varphi(a) = p \land \varphi(b) = q\right\}\]
...
quindi tutte le componenti \(\alpha_j \colon \Omega \to \realnumbers\)
...
\begin{snippetdefinition}{Forma differenziale chiusa}
...
\begin{snippetexample}{}
    ...
    \]
    con \(\Omega = \realnumbers^2 \backslash \{(0,0)\}\).
    Allora
    \[
...
\begin{snippetdefinition}{Insiemi stellati}
    Sono sottoinsiemi \(\Omega \subseteq \realnumbers^n\) dove
    \[
        \exists \overline{x} \in \Omega \suchthat
...
\begin{snippettheorem}{}
    Sia \(\omega\colon \Omega \to (\realnumbers)'\) in \(\mathcal{C}^1(\Omega)\)
    e \(\Omega\) sia stellato. Se \(\omega\) è chiusa, allora è esatta.
\end{snippettheorem}

\begin{snippetproposition}{}
    Sia \(f\colon [a,b] \times A\) con \(A \subseteq \realnumbers^n\) aperto
...
\begin{snippetexercise}{Successioni 1}
...
\begin{snippetexercise}{Successioni 1}
    ...
    \]
    studiare al variare di \(\alpha\in\realnumbers\)
    \begin{enumerate}
        \item \textbf{convergenza puntuale:} studiamo la convergenza puntuale in \((0, +\infty)\).
        Fissato \(x>0\) abbiamo
        \begin{align*}
            \lim_n f_n(x) &= \lim_n n^\alpha \arctan(x)e^{n^2 x} = 0, \forall \alpha \in \realnumbers
        \end{align*}
...
% 2 sett 2020
\begin{snippetexercise}{Successioni 3}
    Data la successione
    \[
    ...
    \begin{enumerate}
        ...
        \end{align*}
        in quanto la parentesi è asintotica all'esponente. Allora l'insieme di convergenza puntuale è \(E = \realnumbers\).
...
\begin{snippetexercise}{Serie 1}
    Consideriamo la serie
    \[
        \sum_{n=1}^\infty \frac{xe^{-\frac{x^2}{n}}}{n^2 + x^2}
    \]
    verifica la convergenza uniforme in \(\realnumbers\). Cominciamo studiando la convergenza totale
    che è più forte, quindi la convergenza di
    \begin{align*}
        \sum_{n=1}^\infty ||f_n||_{\infty, \realnumbers}
        &= \sum_{n=1}^\infty \sup_{x\in\realnumbers}
        \left|
    ...
    \begin{align*}
         ||f_n||_{\infty, \realnumbers}
        &\leq M \sup_{x\in\realnumbers}
        \frac{\sqrt{n}}{n^2 + x^2}
        = \frac{M}{n^{3/2}}
    \end{align*}
    ...
    uniformemente su tutto \(\realnumbers\).
\end{snippetexercise}

\begin{snippetexercise}{Serie 2}
    Conderiamo la serie
    \[
        \sum_{n=1}^\infty \arctan\left(\frac{n^\alpha}{x^2 + n^2}\right)
    \]
    \begin{enumerate}
        \item Stabilire per quali \(\alpha \in \realnumbers\) abbiamo convergenza puntuale:
        ...
        \item Stabilire per quali \(\alpha \in \realnumbers\) abbiamo convergenza uniforme:
        è necessario \(\alpha - 1\). Cominciamo studiando la convergenza totale, che è più forte.
        Abbiamo
        \begin{align*}
            \sum_{n=1}^\infty ||f_n||_{\infty, \realnumbers}
            &=\sum_{n=1}^\infty \sup_{x\in\realnumbers} \left|
        ...
        \[
            ||f_n||_{\infty, \realnumbers} = f_n(0) = \arctan(n^{\alpha - 2})
        \]
...
\begin{snippetexercise}{Serie 3}
    Consideriamo la serie
    \[
        \sum_{n=1}^\infty \frac{\arctan\left(\frac{x}{n^\alpha + 1}\right)}{\sqrt{n+1} - \sqrt{n}}
    \]
    \begin{enumerate}
        \item Valutare per quali \(\alpha\in\realnumbers\) vi è convergenza puntuale in \(\realnumbers\).
        ...
        \item Stabilire per quali \(\alpha\in\realnumbers\) la somma della serie è continua in \(\realnumbers\).
        ...
        Poiché \(a\) è arbitrario, possiamo amplificato e la serie è continua su tutto \(\realnumbers\).
    \end{enumerate}
\end{snippetexercise}
...
\begin{snippetexercise}{}
    ...
    nell'insieme
    \[
        E = \{(x,y,z) \in \realnumbers^3 \suchthat 0<z<x^2 + y^2 <1\}
    \]
    al variare di \(\alpha \in \realnumbers\).
...
\begin{snippetexercise}{}
    ...
    determinare per quali \(\alpha\in\realnumbers\) la funzione
    \[
        f(x,y) = \frac{x}{{(1 + x^4 + y^2)}^\alpha}
    \]
    è integrabile in \(\realnumbers^2\) e calcolare il valore di tale integrale.
...
\begin{snippetexercise}{}
    ...
    dove
    \[
        E_n = \left\{(x,y) \in \realnumbers^2 \,\middle|\, \frac{1}{n} < xy < 2 \land 1 < \frac{y}{x} < 2\right\}
    \]
    ...
    dove
    \[
        E = \left\{(x,y) \in \realnumbers^2 \,\middle|\, 0 < xy < 2 \land 1 < \frac{y}{x} < 2\right\}
    \]
    Applichiamo quindi il teorema di convergenza monotona
    \begin{align*}
        \lim_{n} \int_{E_n} \log^2 \left(\frac{y}{x}\right)\,d\mu_2
        &= \lim_n \int_{\realnumbers^2}  \log^2 \left(\frac{y}{x}\right) \chi_{E_n} \,d\mu_2 \\
        &= \int_{\realnumbers^2} \lim_n \log^2 \left(\frac{y}{x}\right) \chi_{E_n} \,d\mu_2 \\
        &= \int_{\realnumbers^2} \log^2 \left(\frac{y}{x}\right) \chi_{E} \,d\mu_2 \\
...
\begin{snippetexercise}{}
    ...
    \]
    Determinare per quali \(\alpha \in \realnumbers\) la funzione è integrabile in \((0, +\infty)\).
...
\begin{snippetexercise}{}
    Valutare per quali \(\alpha \in \realnumbers\)
    \[
    ...
    \[
        E = \left\{
            (x,y) \in \realnumbers^2 \,\middle|\, 0 < x < 1 \land y > 0
        \right\}
    \]
    Abbiamo quindi
    \begin{align*}
...
\begin{snippetexercise}{}
    ...
    su
    \[
        \Omega = \{(x,y,z) \in \realnumbers^3 \suchthat x^2 + y^2 \leq 1\}
    \]
    ...
    \[
        \Omega(\rho,\theta, z) = \{
            0\leq \rho \leq 1 \land 0 \leq \theta \leq 2\pi \land z\in \realnumbers    
        \}
    \]
...
\begin{snippetexercise}{}
    Studiare per \(\alpha \in \realnumbers\) l'integrabilità di
    \[
    ...
    \[
        \Omega' = \{(x,y) \in \realnumbers^2 \suchthat 0 < {(x-1)}^2 + {(y-1)}^2 < 2\}
        \subset \Omega \cap \{(x,y) \in \realnumbers^2 \suchthat x>0 \land y>0\}
    \]
...
\begin{snippetexercise}{}
    ...
    sull'insieme
    \[
        E = \{(x,y) \in \realnumbers^2 \suchthat (x^2 + y^2)z^2 \leq 1 \land y>0 \land z>0 \land y>-x\}
    \]
    Usiamo le coordinate cilindriche
    \[
...
\begin{snippetexercise}{Equazioni di Eulero}
    Risolviamo
    \[
        \begin{cases}
            x^2y'' -xy' + 10y &= 5x^2 -3x \\
...
\begin{snippetexercise}{}
    ...
    \[
        ...
        \right|\right|_\infty \leq L \in \realnumbers
    \]
...
\begin{snippetexample}{}
    ...
    \]
    E si dimostri che esiste \(g \in \mathcal{C}^1(\realnumbers)\)
    tale che \(\omega\) è esatta in \(\realnumbers\).
