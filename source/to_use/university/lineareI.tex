...
\begin{snippetproof}{}
    \begin{iffproof}
        \item Se non è singolare, è invertibile per l'algoritmo di Gauß-Jordan.
        \item \(A\) invertibile, allora non esiste \(A^{-1}\) tale che \(A^{-1}A = I_n\).
...
\begin{snippetdefinition}{Isomorfismi fra spazi vettoriali}
...
\begin{snippetproof}{}
    \begin{iffproof}
        \item Sia \(f\colon V \to W\) un isomorfismo. Allora \(f\) è invertibile quindi iniettiva e suriettiva.
        \[
            \text{dim} V = \text{dim ker} f + \text{dim Im} f = \text{dim} W
        \]
        \item Let \(n = \text{dim} V = \text{dim W}\). Sia \(\mathcal{B} = \{v_1, \cdots, v_n\}\)
...
\begin{snippetproof}{Rouché-Capelli}
    \begin{iffproof}
        \item Supponiamo che \(Ax = b\) abbia soluzione e mostriamo che il rango
        ...
        \[
            ...
            = \text{rank} A
        \]
        \item Supponiamo che i due ranghi siano uguali. Ma allora il sottospazio generato dalle colonne di \(A\)
