...
\[
    D\phi^{-1} = \{  a \in A \suchthat a\phi \in D \}
\]

\begin{snippetexample}{Funzione}
    Sia \(\phi\colon \realnumbers \to \realnumbers\) definita ponendo \(\phi x \triangleq x^2\).
\end{snippetexample}
...
\begin{snippetproof}{}
    ...
    Ricordiamo che \(D\phi^{-1} = \{ y \in A \suchthat y\phi \in D \}\). Dunque
    \(Cy\phi = \{ y \in A \suchthat y\phi \in C\phi \}\). Ma ora \(x\phi \in C\phi\),
...
\begin{snippetproof}{}
    ...
    per qualche \(z\in D\phi^{-1}\). Ma \(D\phi^{-1} = \{y\suchthat y\phi \in D\}\).
...
\begin{snippetdefinition}{Funzione identità}
...
\begin{snippetproof}{Invertibilità è equivalente a biettività}
    \begin{iffproof}
        \item Sia \(f\) invertibile. Allora sappiamo che \(f(f^{-1})\) è la funzione identità di \(A\) e \(f^{-1}(f)\)
...
\begin{snippetproposition}{Distributività destra}
...
\begin{snippetproposition}{Distributività sinistra}
    Con \(A, B \in M_{m,n}(R)\) e \(C \in M_{n,p}(R)\)
    \[
        A(B+C) = AB + AC
    \]
...
\begin{snippetexample}{Matrice non invertibile}
    \[
        \begin{bmatrix}
            1 & 2 \\
            2 & 4
...
\begin{snippetdefinition}{Assiomi di Peano}
    I numeri naturali sono un insieme \(\naturalnumbers\) dotati di una funzione successore
    \(S \colon \naturalnumbers \to \naturalnumbers\)
    e di un elemento fissato \(0\) tali che:
    \begin{itemize}
        \item la funzione \(S\) è iniettiva;
        \item \(0 \not\in \text{Im}_S\);
        \item se \(A\subseteq \naturalnumbers\) tale che \(0\in A\) e \(As\subseteq A\), allora \(A = \naturalnumbers\);
    \end{itemize}
\end{snippetdefinition}
...
Quindi, dati due modelli \((\naturalnumbers, S, 0)\) e \((\naturalnumbers', S', 0')\)
bisogna trovare una funzione biettiva \(f\colon \naturalnumbers \to \naturalnumbers'\)
...
\begin{snippetproposition}{}
    Un altro modo per dire l'assioma 3 è che ogni sottoinsieme non vuoto di \(\naturalnumbers\)
    ammette un minimo.
\end{snippetproposition}

\begin{snippetproof}{}
    Per dimostrarlo sia \(A\subseteq B\) l'insieme di tutti i minoranti.
    L'insieme \(A\) contiene sicuramente \(0\). Infatti, \(0 \leq n\) per ogni \(n\in\naturalnumbers\).
    L'insieme \(A\) è diverso da \(\naturalnumbers\). Infatti, preso un \(n \in B\),
    sappiamo che \(B \neq \emptyset\), abbiamo che \(n+1\) non è minore o uguale di \(n\),
    quindi non è un minorante di \(B\). Pertanto \(n+1 \notin A\).
    Sappiamo per gli assiomi di Peano che un sottoinsieme di \(\naturalnumbers\) che contiene \(0\)
    e contiene il successore di ogni elemento, coincide con \(\naturalnumbers\).
    Poiché \(0\in A\) e \(A\neq \naturalnumbers\), possiamo concludere che esiste \(k\in A\)
...
%\begin{snippetdefinition}{Isometria}
...
\begin{snippetdefinition}{Normalizzante}
    Il \emph{normalizzante} in \(G\)
    di \(H\) è il sottoinsieme di \(G\) così definito
    \[
        N_G(H) \triangleq \{g\in G \suchthat H^g = H \}
    \]
\end{snippetdefinition}
...
Ora,
\[
    HK = \{hk \suchthat h\in H, k\in K\} = \bigcup_{k\in K} Hk
\]
e
\[
    KH = \{kh \suchthat h\in H, k\in K\} = \bigcup_{k\in K} kH
\]
...
\begin{snippetproof}{}
    ...
    \]
    Ora \(C_G(x) \cap H = \{x\in H \suchthat xg = gx\} = C_H(x)\).
...
Sappiamo che \(K\varphi = \{k\varphi \suchthat k\in K\}\).
...
Ricordiamo che
\[
    L\varphi^{-1} = \{x \in G \suchthat x\varphi\in L \}
\]
...
\begin{snippetproof}{}
    \begin{iffproof}
        \item Data ua congruenza, allora la la classe dell'identità è un sottogruppo normale
        ...
        \end{enumerate}
        \item Sia \(H \unlhd G\) e sia \(\sim\) la relazione di equivalenza
...
\begin{snippetproposition}{Alcune proprietà}
    \begin{enumerate}
        ...
        per \(n\in\naturalnumbers\) e quindi \(Hx = {(Hg)}^n\)).
...
\begin{snippetproof}{}
    ...
    Abbiamo anche
    \[
        \text{ker}_\varphi = \{x \in G \suchthat x\varphi = 1_{G/H}\}
        = \{x\in G \suchthat Hx = H\} = H
    \]
\end{snippetproof}
...
\begin{snippetproof}{}
    ...
    \[
        Z(G)x = {(Z(G) g)}^n
    \]
    per qualche \(n\in\naturalnumbers\), cioè \(x \in {(Z(G) g)}^n = Z(g)g^n\),
    cioè \(x = zg^n\) per qualche \(z\in Z(G)\) e \(n\in\naturalnumbers\).
    Se ora \(y\) è un altro elemento di \(G\), abbiamo che \(y=z'g^m\)
    per qualche \(z'\in Z(G)\) e \(m\in\naturalnumbers\).
...
\begin{snippetdefinition}{Prodotto semidiretto esterno}
...
\begin{snippettheorem}{Il prodotto semidiretto esterno è un gruppo}
...
\begin{snippetproof}{Il prodotto semidiretto esterno è un gruppo}
