...
Tale isomorfismo ci dice che si può scrivere in maniera unica
(decomposizione unica) \(h_1h_2\cdots h_n\) poiché vi è una biiezione.
In particolare, se \(n=2\), le condizioni affinché \(G\)
sia prodotto direttodi due suoi sottogruppi \(H\) e \(K\) si riducono a
\begin{enumerate}
    \item \(H \unlhd G\) \(K \unlhd G\);
    \item \(H \cap K = 1\);
    \item \(G = HK\).
\end{enumerate}
...
\sdefinition{Prodotto semidiretto interno}{
    Sia \(G\) un gruppo e siano \(H \leq G\)
    e \(N \unlhd G\) tali che:
    \begin{enumerate}
        \item \(N \cap N = 1\);
        \item \(G = HN\).
    \end{enumerate}
    Diciamo allora che \(G\) è prodotto semidiretto (interno)
    di \(H\) e \(N\) e scriviamo
    \[
        G = H \ltimes N
    \]
}
