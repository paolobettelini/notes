...
\section{Eq diofantine}

Let \(ax + by = c\) with \(\gcd(a,b) \,|\, c\). Suppose \(a\) and \(b\)
are not zero.
Possiamo riscrivere l'equazione così:
\[
    \gcd(a,b)a'x + \gcd(a,b)b'y = \gcd(a,b)c'
\]
per opportuni \(a', b', c'\) interi.
Ma allora, poiché \(\gcd(a,b) \neq 0\) possiamo semplificare e ricavare l'equazione
\[
    a'x + b'y = c'
\]
dove \(a'\) e \(b'\) sono coprimi.
Sia ora \((\overline{x}, \overline{y})\) una soluzione particolare.
