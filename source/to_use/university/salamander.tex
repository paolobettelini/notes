...
\begin{proof}
    ...
    We will now compute the donor of \(A\), \(D(0)\) and the receiver of \(B\), \(D(e_3)\).
    \begin{enumerate}
...
Allora, l'oggetto (13) si immerge naturalmente nell'oggetto (14), non
il (15) come dimostratosi, mentre l'oggetto (14) si immerge naturalmente
...
Calcolo di quante ce ne sono con fattoriale XXX.
...
\[
    (n,1) \to (n-1, 2) \to \cdots \to (2, n-1) \to (1,n)
\]
...
Dobbiamo assicurarci che questi siano oggetti della forma di Bergman (quindi oltre che siano ben definiti). TODO.

\begin{lemma}
    Let \(i_1, i_2, \cdots, i_{n-1} \in \{1, \cdots, n\}\)
    such that \(i_1 \neq \cdots \neq i_{n-1}\). Then, any sequence of morphisms
    \[
        A_{x} \to
        A_{x + e_{i_1}} \to
    ...
    \[
        ...
        \cdots \to
        M_{n-1}\left(A_{x + \sum_{j=1}^{n-1} e_{i,j}}\right)
    \]
\end{lemma}

\begin{proof}
    Fissiamo \(r\) tale che \(1 \leq r < n - 1\) e consideriamo il morfismo \(f \colon A \to B\)
    con \[
        A = A_{x + \sum_{j=1}^r e_{i_j}}, \quad
        B = A_{x + \sum_{j=1}^{r+1} e_{i_j}}
    \]
    e mostriamo che questo morfismo induce un morfismo \(M_r(A) \to M_{r+1}(B)\).
    Mostriamo quindi separamente che la mappa manda...
\end{proof}

Da questa dimostrazione possiamo dedurre un criterio generale per quando
un oggetto si immerge naturalmente in un altro oggetto comologico.
Fissata una direzione \(e_i\) per ogni \(i \in \{1, \cdots, n\}\)
consideriamo due oggetti comologici arbitrari adiacenti nella direzione \(e_i\):
\[
    A = \frac{
        X
    }{
