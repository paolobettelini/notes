...
@article{bergman2012diagramchasing,
  ...
  eprinttype   = {arXiv},
  eprintclass  = {math.CT},
  url          = {https://math.berkeley.edu/~gbergman/papers/salamander.pdf},
  urldate      = {2026-05-18}
}
\end{filecontents*}
...
\begin{abstract}
    This paper explores the generalization of George M. Bergman's results on diagram-chasing in double complexes \cite{bergman2012diagramchasing} to n-dimensional cocomplexes.
\end{abstract}
...
Nel paper di Bergman \cite{bergman2012diagramchasing} viene notato che il reticolo di oggetti comologici viene generato
...
\begin{lemma}\label{lem:repeated-direction}
...
For the cochain bicomplexes we consider the local commutative diagram in Figure~\ref{fig:bicomplex-local}.
\def\myopacity{0.5}
...
\begin{tikzcd}[row sep=2cm, column sep=2.5cm]
    ...
    {A(+)}                                                 
\end{tikzcd}
\captionof{figure}{Local commutative diagram for a cochain bicomplex.}
\label{fig:bicomplex-local}
\end{center}
We define the directional homological objects, which are Bergman's objects \cite{bergman2012diagramchasing}, as shown in Figure~\ref{fig:bicomplex-objects}.
\begin{center}
...
\end{tabular}
\captionof{figure}{The four directional cohomological objects in a double complex.}
\label{fig:bicomplex-objects}
\end{center}

In three dimensions we have the commutative diagram in Figure~\ref{fig:three-local}.
\begin{center}
    ...
    \end{tikzcd}
\captionof{figure}{Local commutative diagram around the central object in dimension three.}
\label{fig:three-local}
\end{center}

We have the \(18\) cohomological objects shown in Figure~\ref{fig:three-objects}.
\begin{center}
...
\begin{tabular}{ccccc}
    ...
    \homologyDiag{1}
        { \ldot{-1}{0}{-1} }
        { \ldot{1}{0}{0} \ldot{0}{0}{1} }
    &
    \homologyDiag{2}
        { \ldot{-1}{-1}{0} }
        { \ldot{1}{0}{0} \ldot{0}{1}{0} }
    &
    \homologyDiag{3}
        { \ldot{0}{-1}{-1} }
        { \ldot{0}{1}{0} \ldot{0}{0}{1} }
    &
    \homologyDiag{4}
        { \ldot{-1}{0}{-1} \ldot{-1}{-1}{0} }
        { \ldot{1}{0}{0} \ldot{0}{1}{1} }
    &
    \homologyDiag{5}
        { \ldot{0}{-1}{-1} \ldot{-1}{-1}{0} }
        { \ldot{0}{1}{0} \ldot{1}{0}{1} }
    \\[1.5cm]

    \homologyDiag{6}
        { \ldot{0}{-1}{-1} \ldot{-1}{0}{-1} }
        { \ldot{0}{0}{1} \ldot{1}{1}{0} }
    &
    \homologyDiag{7}
        { \ldot{0}{-1}{-1} \ldot{-1}{0}{0} }
        { \ldot{1}{1}{0} \ldot{1}{0}{1} }
    &
    \homologyDiag{8}
        { \ldot{-1}{0}{-1} \ldot{0}{-1}{0} }
        { \ldot{1}{1}{0} \ldot{0}{1}{1} }
    &
    \homologyDiag{9}
        { \ldot{-1}{-1}{0} \ldot{0}{0}{-1} }
        { \ldot{1}{0}{1} \ldot{0}{1}{1} }
    &
    \homologyDiag{10}
        { \ldot{0}{0}{-1} \ldot{-1}{0}{0} }
        { \ldot{1}{0}{1} }
    \\[1.5cm]

    \homologyDiag{11}
        { \ldot{0}{0}{-1} \ldot{0}{-1}{0} }
        { \ldot{0}{1}{1} }
    &
    \homologyDiag{12}
        { \ldot{0}{-1}{0} \ldot{-1}{0}{0} }
        { \ldot{1}{1}{0} }
    &
    \homologyDiag{13}
        { \ldot{0}{0}{-1} \ldot{0}{-1}{0} \ldot{-1}{0}{0} }
        { \ldot{1}{1}{1} }
    &
    \homologyDiag{14}
        { \ldot{0}{-1}{-1} \ldot{-1}{0}{-1} \ldot{-1}{-1}{0} }
        { \ldot{1}{1}{0} \ldot{1}{0}{1} \ldot{0}{1}{1} }
    &
    \homologyDiag{15}
        { \ldot{-1}{-1}{-1} }
        { \ldot{1}{0}{0} \ldot{0}{1}{0} \ldot{0}{0}{1} }
    \\[1.5cm]
    &
    \homologyDiag{16}
        { \ldot{-1}{0}{0} }
...
\end{tabular}
\captionof{figure}{The \(18\) non-trivial local cohomological objects in dimension three.}
\label{fig:three-objects}
\end{center}
...
:=
\frac{
\ker\partial[0,(+,0,0)] \cap \ker\partial[0,(0,0,+)]
}{
\im\partial[(-,0,-),0]
}
\\[1ex]
...
:=
\frac{
\ker\partial[0,(+,0,0)] \cap \ker\partial[0,(0,+,+)]
}{
\im\partial[(-,0,-),0] + \im\partial[(-,-,0),0]
}
\\[1ex]
(5)\quad
&H_{(0,-,-),(-,-,0)}^{(0,+,0),(+,0,+)}
...
:=
\frac{
\ker\partial[0,(+,+,0)] \cap \ker\partial[0,(+,0,+)]
}{
\im\partial[(0,-,-),0] + \im\partial[(-,0,0),0]
}
\\[1ex]
(8)\quad
&H_{(-,0,-),(0,-,0)}^{(+,+,0),(0,+,+)}
...
\im\partial[(0,0,-),0] + \im\partial[(-,0,0),0]
}
\\[1ex]
(11)\quad
&H_{(0,0,-),(0,-,0)}^{(0,+,+)}
...
Cominciamo dimostrando esplicitamente il Lemma~1.4 del paper originale \cite{bergman2012diagramchasing}
...
\begin{figure}[htbp]
        \centering
        % Usiamo [b] per allineare i due blocchi alla base (sulle didascalie)
        \begin{minipage}[b]{0.47\textwidth}
            ...
            \vspace{0.3cm} % Spazio fisso, piccolo e identico per entrambi
            \captionof{figure}{Horizontal case}
        \end{minipage}%
        \hfill\vrule\hfill% separation line
        \begin{minipage}[b]{0.47\textwidth}
            \centering
            
            \begin{tikzcd}
                                                                    & \bullet \arrow[d, "b"']                            &                   \\
            \bullet \arrow[r, "a"] \arrow[d] \arrow[rd, "p", dashed] & A \arrow[r] \arrow[d, "f"] \arrow[rd, "q", dashed] & \bullet \arrow[d] \\
            \bullet \arrow[r]                                        & B \arrow[d, "c"] \arrow[r, "d"']                   & \bullet           \\
                                                                    & \bullet                                            &                  
            \end{tikzcd}
            
            \vspace{0.3cm} % Spazio fisso, piccolo e identico per entrambi
            \captionof{figure}{Vertical case}
        \end{minipage}
    \end{figure}
    \[
        \begin{aligned}
            A_\bx &= \frac{\ker q}{\im a + \im b}, \quad {}^\bx B &= \frac{\ker c \cap \ker d}{\im p}
        \end{aligned}
    \]
    Let \[
...
\begin{proof}
    Consider the counterexample in Figure~\ref{fig:triple-counterexample}: we consider \(\mathbf{Vect}_{\mathbb K}\) for some field \(\mathbb K\).
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \end{tikzcd}
    \end{minipage}
    \captionof{figure}{Shape of the attempted triple-complex map and the explicit vector-space counterexample.}
    \label{fig:triple-counterexample}
    \end{center}
    The diagram in Figure~\ref{fig:triple-counterexample} commutes as the compositions of the non-trivial quare are still the identity.
...
Notiamo che un tale oggetto è precisamente l'oggetto numerato (14) della Figure~\ref{fig:three-objects},
...
\begin{proof}
    ...
    e quindi la direzione \(e_i\) viene traversata \(\geq 2\) volte e quindi per il Lemma~\ref{lem:repeated-direction}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            due volte la direzione \(e_{i_r}\). Per il Lemma~\ref{lem:repeated-direction}
            ...
            \]
        \end{itemize}
        \item \(\tilde f\) manda \(I_r(y)\) in \(I_{r+1}(y+e_{i_r})\): consideriamo un morfismo \(\partial^{J,r}\)
        arbitrario della definizione di \(I_r(y)\) e mostriamo che esiste sempre
        un morfismo della definizione di \(I_{r+1}(y + e_{i_r})\) tale per cui \(f\) manda
        l'immagine del primo nell'immagine del secondo.
        Per costruzione, \(\#f = \{e_{i_r}\}\) che ha lunghezza \(1\) e \(\#\partial^{J,r} = \{e_j\}_{j \in J}\)
        con lunghezza \(r\) per costruzione di \(J\).
        Consideriamo il morfismo \(f \circ \partial^{J,r}\), che connette il dominio di
        \(\partial^{J,r}\) a \(B\), e distinguiamo due casi:
        \begin{itemize}
            \item se \(e_{i_r} \in \#\partial^{J,r}\), allora \((f \circ \partial^{J,r})\) traversa
            due volte la direzione \(e_{i_r}\). Per il Lemma~\ref{lem:repeated-direction}
            vediamo immediatamente che vale \(f \circ \partial^{J,r} = 0\) sempre.
            Dunque,
            \[
                f\left(\im \partial^{J,r}\right) = 0 \subseteq I_{r+1}(y+e_{i_r})
            \]
            \item se \(e_{i_r} \notin \#\partial^{J,r}\), allora \((f \circ \partial^{J,r})\) traversa
            solo una volta la direzione \(e_{i_r}\).
            Nella definizione di \(I_{r+1}(y+e_{i_r})\) vengono usati tutti i morfismi entranti in \(B\) di lunghezza
            \(r+1\). Fra questi, consideriamo solo quelli che includono un morfismo che traversa la direzione
            \(e_{i_r}\), cioè \[
                E = \left\{
                    \partial^{J, r+1} \mid J \subseteq \{1, \cdots, n\} \land |J| = r+1 \land i_r \in J
                \right\}
            \]
            Per ogni \(\partial \in E\), possiamo rappresentare \(\partial\) come
            \(\partial = \alpha_1 \circ \cdots \circ \alpha_{r+1}\) dove \(|\#\alpha_i| = 1\),
            cioè come composizione di morfismi di lunghezza uno. Per commutatività del diagramma,
            possiamo scegliere tale rappresentazione tale che \(\#\alpha_{r+1} = \{e_{i_r}\}\).
            Ma quindi, data una tale rappresentazione, deve essere per forza \(\alpha_{r+1} = f\) e possiamo scrivere
            \(\partial = f \circ (\alpha_1 \circ \cdots \circ \alpha_{r})\).
            Per definizione, \(E\) contiene tutti i possibili morfismi di lunghezza \(r+1\).
            In particolare, il termine \((\alpha_1 \circ \cdots \circ \alpha_{r})\) può assumere
            qualsiasi cammino di lunghezza \(r\) che termina in \(A\), e quindi deve esistere
            \(\partial \in E\) tale che \(\partial = f \circ \partial^{J,r}\).
            Dunque,
            \[
                f\left(\im \partial^{J,r}\right) \subseteq I_{r+1}(y+e_{i_r})
            \]
        \end{itemize}
        Per arbitrarietà di \(J\), troviamo che ogni addendo della somma che definisce
        \(I_r(y)\) viene mandato dentro \(I_{r+1}(y+e_{i_r})\).
        Dunque, per la proprietà universale della somma di sottoggetti, il morfismo
        \[
            \tilde f \colon K_r(y) \to K_{r+1}(y + e_{i_r})
        \]
        si restringe a un morfismo
        \[
            I_r(y) \to I_{r+1}(y + e_{i_r})
        \]
    \end{itemize}
...
Come notato nel paper originale \cite{bergman2012diagramchasing}, possiamo completare i pallini nei diagramma,
nell'ipercubo superiore e inferiore completando verso l'alto e verso il basso rispettivamente.
Cosifacendo, notiamo che le nostre strutture sono univocamente determinate
dai down-sets non banali, per l'ipercubo dei nuclei, e dai up-sets non banali complementari per l'ipercubo delle immagini.
Con questa visione possiamo quindi considerare il poset \((\mathcal P(\{1, \cdots, n\}, \leq))\) dove
\(\leq\) è la relazione di inclusione. Ad ogni struttura può essere associato un
up/down-set di un sottoinsieme \(\varnothing \neq G \subset \mathcal P(\{1, \cdots, n\})\).
Scegliamo \(G\) come rappresentante degli elementi i cui nuclei sono intersezionati (potremmo scegliere
pure gli elementi le cui immagini vengono sommate), definiamo gli up-set e down-set di \(G\) e
usiamo il minimo e il massimo rispettivo di questi insiemi per costruire l'oggetto comologico.
Stiamo quindi costruendo gli oggetti considerando i pallini dei diagrammi come elementi
di classi di insiemi, come in Figure~\ref{fig:downset-classes}.
\begin{center}
% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBoBGAXVJADcBDAGwFcYkQAdD4LhgJ0g4AFljABzUgAJyUgExSAzFwC+IZaXSZc+QijILqdJq3ZceHfoJHiKpJR1XrN2PASLyDNBizaJO3XnoBCGFRCUl7Rw0QDBcddwpDbxM-M0Dg0JsPFTVo2O03FBliJONff3NLEOsJGVkcpxitV11kBVISrzLTAIsgqzC5Brzm+JR22VKfHsr+6rDh5wLWuqmUivSBm2kGwxgoMXgiUAAzPggAWyQyEBwIJAAWRrPLpHbb+8QAVmfzq8QZB8kAB2X6vRDyIGIABsYP+NzubzhSEBiIhyMQDxoaJ+0Re-y+2M+sLxf0eRJBGIRn3IGOBFJhGPeaNkykoyiAA
\begin{tikzcd}
                                                            & {\{\varnothing,1,2\}} \arrow[rr] \arrow[dd] &                                             & {\{\varnothing, 2\}} \arrow[dd] \\
{\{\varnothing, 1, 2, 3\}} \arrow[ru] \arrow[rr] \arrow[dd] &                                             & {\{\varnothing,2,3\}} \arrow[ru] \arrow[dd] &                                 \\
                                                            & {\{\varnothing, 1\}} \arrow[rr]             &                                             & \{\varnothing\}                 \\
{\{\varnothing,1,3\}} \arrow[ru] \arrow[rr]                 &                                             & {\{\varnothing, 3\}} \arrow[ru]             &                                
\end{tikzcd}
\captionof{figure}{Classes of subsets used to encode local cohomological objects.}
\label{fig:downset-classes}
\end{center}

\begin{definition}
    Sia \(G \subseteq \mathcal P(\{1, \cdots, n\}) \setminus \{\varnothing\}\) una famiglia non vuota
    di sottoinsiemi. Definiamo l'\emph{up-set generato da \(G\)} come \[
        \uparrow G := \{
            T \subseteq \{1, \cdots, n\} \mid \exists S \in G \text{ such that } S \subseteq T
        \}
    \]
    Analogamente, definiamo il \emph{down-set complementare all'up-set generato da \(G\)} come \[
        \downarrow G := \mathcal P(\{1, \cdots, n\}) \setminus \uparrow G
    \]
\end{definition}
...
\begin{definition}
    Sia \(G \subseteq \mathcal P(\{1, \cdots, n\}) \setminus \{\varnothing\}\) una famiglia non vuota
    di sottoinsiemi.
    Definiamo l'oggetto comologico associato a \(G\) come
    \[
        \mathcal H_P(x) := \frac{
            \displaystyle\bigcap_{S \in \min(\uparrow G)} \ker \partial\left[x, x+e_S\right]
        }{
            \displaystyle\sum_{T \in \max(\downarrow G)} \im \partial\left[x - e_{\{1, \cdots, n\} \setminus T}, x\right]
        }
    \]
    dove \(\min\) e \(\max\) sono relativa al poset \((\mathcal P(\{1, \cdots, n\}, \leq))\) dove
    \(\leq\) è la relazione di inclusione.
\end{definition}

In generale, diverse famiglie possono essere associate al medesimo oggetto.
Il rappresentante unico è solo l'elemento minimale \(\min(\uparrow G)\) (analogamente quello massimale).

\begin{proposition}
    L'oggetto \(\mathcal H_G(x)\) è ben definito.
\end{proposition}
...
\begin{proof}
    ...
    \[
        I_G(x)
        :=
        \sum_{T\in \max(\downarrow G)}
        \im \partial[x-e_{N\setminus T},x].
    \]
    Indichiamo con
    \[
        \kappa_G \colon K_G(x) \hookrightarrow A_x
    \]
    il monomorfismo canonico dell'intersezione dei nuclei, e con
    \[
        \lambda_G \colon I_G(x) \hookrightarrow A_x
    \]
    il monomorfismo canonico della somma delle immagini.
    ...
    \[
        ...
        K_G(x)/I_G(x)
    \]
    è ben definito, dobbiamo mostrare che \(\lambda_G\) fattorizza attraverso
    \(\kappa_G\), cioè che esiste un morfismo
    \[
        \overline{\lambda}_G \colon I_G(x)\to K_G(x)
    \]
    tale che il diagramma
    \[
        \begin{tikzcd}
            I_G(x) \arrow[rr, "\lambda_G"] \arrow[rd, "\overline{\lambda}_G"', dashed] & & A_x \\
            & K_G(x) \arrow[ru, "\kappa_G"'] &
        \end{tikzcd}
    \]
    commuti.

    Per la proprietà universale dell'intersezione dei nuclei, è sufficiente
    ...
    \[
        I_G(x)
        \xrightarrow{\lambda_G}
        A_x
        \xrightarrow{\partial[x,x+e_S]}
        A_{x+e_S}
    \]
    è nullo.

    A sua volta, per la proprietà universale della somma delle immagini, è
    sufficiente verificarlo su ciascun termine della somma. Fissiamo quindi
    \[
        S\in\min(\uparrow G),
        \qquad
        T\in\max(\downarrow G),
    \]
    e consideriamo il composto
    \[
        \begin{tikzcd}
            A_{x-e_{N\setminus T}}
            \arrow[r, "{\partial[x-e_{N\setminus T},x]}"]
            \arrow[rr, "{\partial[x-e_{N\setminus T},x+e_S]}"', bend right]
            &
            A_x
            \arrow[r, "{\partial[x,x+e_S]}"]
            &
            A_{x+e_S}.
        \end{tikzcd}
    \]
    Poiché \(S\in\uparrow G\) mentre \(T\in\downarrow G\), per definizione di
    \(\downarrow G\) non può valere \(S\subseteq T\). Dunque, esiste
    \(i\in S\setminus T\) o equivalentemente \(i\in S\cap (N\setminus T)\)
    Quindi la direzione \(e_i\) compare sia nella mappa
    \[
        \partial[x-e_{N\setminus T},x],
    \]
    sia nella mappa \(\partial[x,x+e_S]\).
    Il composto
    \[
        \partial[x,x+e_S]
        \circ
        \partial[x-e_{N\setminus T},x]
        =
        \partial[x-e_{N\setminus T},x+e_S]
    \]
    percorre dunque la direzione \(e_i\) almeno due volte. Per il Lemma~\ref{lem:repeated-direction}, otteniamo
    \[
        \partial[x-e_{N\setminus T},x+e_S]=0.
    \]
    Quindi, per ogni \(S\in\min(\uparrow G)\), il composto
    \[
        I_G(x)
        \xrightarrow{\lambda_G}
        A_x
        ...
        A_{x+e_S}
    \]
    è nullo. Per la proprietà universale dell'intersezione dei nuclei,
    \(\lambda_G\) fattorizza in modo unico attraverso \(\kappa_G\). Esiste cioè
    un unico morfismo
    \[
        \overline{\lambda}_G\colon I_G(x)\to K_G(x)
    \]
    tale che
    \[
        \kappa_G\circ \overline{\lambda}_G = \lambda_G
    \]
    Possiamo quindi formare il quoziente
    \[
        \mathcal H_G(x)
        :=
        \operatorname{coker}
        \left(
            I_G(x)
            \xrightarrow{\overline{\lambda}_G}
            K_G(x)
        \right)
    \]
    che nella notazione usuale scriviamo come
    \[
        \mathcal H_G(x)
        =
        \frac{
...
Nel caso tridimensionale troviamo quindi il diagramma in Figure~\ref{fig:three-hasse}.
\begin{center}
\begin{tikzpicture}[
    >=Latex,
    x=1.1cm, y=0.9cm,
...
\end{tikzpicture}
\captionof{figure}{Hasse diagram of the intramural maps between the \(18\) three-dimensional objects.}
\label{fig:three-hasse}
\end{center}

\begin{proposition}
    Siano \(G,Q \subseteq \mathcal P(\{1, \cdots, n\}) \setminus \{\varnothing\}\) famiglie non vuote
    di sottoinsiemi tali che \(\uparrow G \supseteq \uparrow Q\). Allora, l'identità di \(A_x\) induce canonicamente una mappa
    \[
        \iota_{P,Q}(x) \colon \mathcal H_G(x) \to \mathcal H_Q(x)
    \]
    Queste mappe sono funtoriali: se \(R \supseteq G \supseteq Q\) con \(R\) come sopra,
    \[
        \iota_{Q,R} \circ \iota_{G,Q} = \iota_{G,R}
    \]
\end{proposition}

In generale le mappe vanno dal receiver verso il donor, al contrario delle extramural maps.
Per \(n=2\) ritroviamo banalmente il rombo di Bergman \cite{bergman2012diagramchasing}, come in Figure~\ref{fig:bergman-diamond}.
\begin{center}
    % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZARgBoAGAXVJADcBDAGwFcYkQAlACgA8BKEAF9S6TLnyEUZAEzU6TVuwAivAcNHY8BIuVLE5DFm0QgAEgH1iqoSJAZNEotL0GFxs+enXBcmFADm8ESgAGYAThAAtki6IDgQSNLqIOFRMTTxSADMyanRiM5xCYjEuRH5WRnFpZSCQA
    \begin{tikzcd}
                    & R(x) \arrow[ld] \arrow[rd] &                   \\
    H_1(x) \arrow[rd] &                            & H_2(x) \arrow[ld] \\
                    & D(x)                       &                  
    \end{tikzcd}
\captionof{figure}{The two-dimensional Bergman diamond.}
\label{fig:bergman-diamond}
\end{center}
...
% TODO
La lista dei 18 oggetti è numerata arbitrariamente. Si potrebbe enumerate secondo i livelli del diagramma
di Hasse.