...
\begin{plainhtml}
    Translations, isometries, rotations with fixed center are all examples of group actions on a plane.
\end{plainhtml}
\begin{plainhtml}
    The action that sends the tuple to the first element of the tuple, discarding the other, is a right action.
\end{plainhtml}
...
\begin{plainhtml}
    This is an automorphism if and only if the fixed element is the identity.
\end{plainhtml}
...
\begin{plainhtml}
    This action is always an automorphism.
\end{plainhtml} %TODOURGENT proofs, 28 nov

\begin{plainhtml}
    In such case, we have a function thar associaites an element of the group
...
\begin{plainhtml}
    Thus, every element cn be send into any other element.
\end{plainhtml}
...
\begin{snippetproof}{stabilizer-right-coset-equivalence-proof}{stabilizer-right-coset-equivalence}{}
    \begin{iffproof}
        \item From \(xg=xh\) we get \(xgh^{-1} = x\), meaning \(gh^{-1} \in \stabilizer_G(x)\)
        and thus \[
            g = {(gh^{-1})}h \in \stabilizer_G(x)h
            \implies \stabilizer_G(x)g = \stabilizer_G(x)h
        \]
        \item Let \(g = sh\) with \(s\in\text{Stable}_G(x)\).
...
\begin{plainhtml}
    The bigger the index, the bigger the orbit.
\end{plainhtml}
...
\begin{plainhtml}
    The more elements commute, the more classes we have, and the closer the group is to being abelian.
\end{plainhtml}
...
\begin{plainhtml}
    We are now going to characterize normal groups
\end{plainhtml}
...
\begin{plainhtml}
    Indeed, we proved that the right and left cosets of the kernel coincide.
\end{plainhtml}
