...
\begin{plainhtml}
    The set of translations form a subgroup of the isometries, but the rotations
    do not. Only the rottions with the same center do.
\end{plainhtml}

\begin{plainhtml}
    Given two lines with a common point, any isometry will keep the point still,
...
\begin{plainhtml}
    Note: this does not mean that every point is preserved. Consider, for instance,
...
\begin{plainhtml}
    Such isometries send sides to side and vertices to vertices.
\end{plainhtml}
...
\begin{plainhtml}
    Note that three vertices suffice to reconstruct the entire isometry.
\end{plainhtml}
...
\begin{plainhtml}
    The Cayley table of the Klein group is the table of a group of order four.
\end{plainhtml}
