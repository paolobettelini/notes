...
\begin{plainhtml}
    Every function has an arrow for each element in the domain.
\end{plainhtml}
...
\begin{snippetdefinition}{inclusion-map-definition}{Inclusion map}
    Let \(A\) and \(B\) be \set[sets] such that \(A \subseteq B\).
    Then, the \emph{inclusion map} is defined as
    \(\iota \colon A \inclusion B\) defined as
    \(\iota(a) \triangleq a\).
\end{snippetdefinition}
