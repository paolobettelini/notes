...
\begin{plainhtml}
    The theorem is false for vector-valued functions.
\end{plainhtml}
...
\begin{snippetproof}{constant-function-criterion-corollary-proof}{constant-function-criterion-corollary}{}
    \begin{iffproof}
        \item The forward direction is obvious.
        \item For the converse, let \(x_0 \in \Omega\) and set \(E_{x_0} = \{x \in \Omega \suchthat f(x) = f(x_0)\}\).
...
\begin{plainhtml}
    For single-variable functions, bounded derivative is a criterion for Lipschitz continuity.
\end{plainhtml}
