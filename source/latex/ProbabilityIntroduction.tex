...
\begin{snippetexample}{classical-probability-example}{Classical approach to probability}
    ...
    We thus want to place an ordering on the \set of \outcome[outcomes]
    of this experiment.
    Let \(\Omega\) be the \set of all possible \outcome[outcomes] of the experiment.
    ...
    This is called the additivity of probabilities for disjoint \event[events].
...
\begin{snippetexample}{frequentist-probability-example}{Frequentist approach to probability}
    ...
    In this case, the \event[events] or elementary cases are not necessarily equiprobable.
...
\begin{plainhtml}
    In both cases we have the same properties,
...
\begin{plainhtml}
    Each of these approaches covers the previous one.
...
\begin{plainhtml}
    If we have an urn, we can represent
...
\begin{snippetdefinition}{event-definition}{Event}
    An \emph{event} is a \set of \outcome[outcomes].
\end{snippetdefinition}
