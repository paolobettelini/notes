...
\begin{snippetdefinition}{greatest-element-definition}{Greatest element}
    Given a \partialorder on a set \(A\) and \(P\subseteq A\), an element \(g\in A\) is a \textit{greatest element}
    of \(A\) if \[\forall a\in A, a \leq g\].
\end{snippetdefinition}

\begin{snippetdefinition}{least-element-definition}{Least element}
    Given a \partialorder on a set \(A\) and \(P\subseteq A\), an element \(g\in A\) is a \textit{least element}
    of \(A\) if \[\forall a\in A, g \leq a\].
\end{snippetdefinition}
