...
\begin{snippetexercise}{sylow-ex5}{Group is not simple}
    Show that a \group of order \(36\) is not simple. % TODOURGENT \simplegrp
\end{snippetexercise}

\begin{snippetsolution}{sylow-ex5-sol}{Group is not simple}
    Given a (right) group action of a \group on a \set \(\Omega\),
    we can associate to every \(g\in G\) a \bijective[bijective function]
    ...
    Thus, the kernel is a non-trivial \normalsubgrptext[normal subgroup] that is not \(G\).
\end{snippetsolution}
...
\begin{snippetsolution}{sylow-ex6-sol}{}
    ...
    a \normalsubgrptext \sylowpsubgroup[Sylow \(p\)-subgroup], meaning that \(G\)
    ...
    By this lemma XXX % TODOURGENT cite "Consideriamo il cosiddetto commutatore d" lemma
...
\begin{snippetsolution}{sylow-ex7-sol}{}
    ...
    since it is a product of \abeliangroup[abelian groups].
    For the non trivial \sylowpsubgroup[Sylows] we have:
