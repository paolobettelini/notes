...
\begin{snippetdefinition}{elementary-lambda-matrix-definition}{Elementary \(\lambda\) matrix}
    Let \(n\in\naturalnumbers\) and \(1 \leq i,j \leq n\) such that \(i \neq j\).
    The \emph{elementary matrix} \(E_{i,j}(\lambda)\) is the \(n\times n\) \matrix defined as
    \[
        \left(E_{i,j}(\lambda)\right)_{a,b} \triangleq \kronecker_{a,b} + \lambda \kronecker_{i,a} \kronecker_{b,j}
    \]
...
\begin{snippetproof}{nonsingular-matrix-invertible-theorem-proof}{nonsingular-matrix-invertible-theorem}{Nonsingular matrix invertibility}
    \begin{iffproof}
        \item If \(A\) is invertible, then \(\exists_{=1} A^{-1} \suchthat A^{-1}A = I_n\).
        Thus, for every \(b\), we can find a unique solution to \(Ax = b\) for every \(b\), which is \(x = A^{-1}b\).
        \item If the system has only one solution for every \(b\), then we can find an inverse
...
\begin{snippetproof}{two-by-two-syste-unique-solution-proof}{two-by-two-syste-unique-solution}{}
    \begin{iffproof}
        \item By substituting \(x\) and \(y\) we can see that it is a solution.
        \item If there were multiple solutions \((x_0, y_0) \neq (x_1, y_1)\)
        we would have
        \[
...
\begin{snippetproof}{rouche-capelli-theorem-proof}{rouche-capelli-theorem}{Rouché-Capelli theorem}
    \begin{iffproof}
        \item Suppose that \(Ax = b\) has a solution and let us show that the rank
        of the augmented matrix is equal to the rank of the matrix \(\text{rank} A|b = \text{rank} A\).
        Let \(\overline{x} \in \mathbb{F}^n\) be a solution of \(Ax = b\).
        Then \(A\overline{x} = b\) is equivalent to
        \[
            \sum_i \overline{x}_i A^i = b \iff b \in \text{span}(A^1, \cdots, A^n)
        \]
        but then
        \[
            \mrank(A|b) = \text{dim span}(A^1, \cdots, A^n, b) = \text{dim span}(A^1, \cdots, A^n)
            = \mrank A
        \]
        \item Suppose that the two ranks are equal. Then the subspace generated by the columns of \(A\)
        coincides with the subspace generated by the columns of \(A|b\). In particular, there exist scalars
        \(\overline{x}_i\) such that
        \[
            b = \sum_i \overline{x}_i A^i \iff A\overline{x} = b
        \]
        Therefore \(Ax = b\) admits solutions.
        We already know that the set of solutions is an affine subspace of \(\mathbb{A}^n(\mathbb{F})\)
        directed by \(\text{ker} A\). Observe that
        \[
            \text{ker} A = \{x \in \mathbb{F}^n \suchthat Ax = 0\} = \text{ker} L_A
        \]
        where \(L_A \colon \mathbb{F}^n \to \mathbb{F}^n\).
        \begin{align*}
