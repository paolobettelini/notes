...
\begin{snippetdefinition}{linear-combination-definition}{Linear Combination}
    ...
    \(a_1, a_2, \cdots, a_n\) be \vsscalar[scalars] in \(\mathbb{F}\).
...
\begin{snippetdefinition}{lower-triangular-matrix-definition}{Lower triangular matrix}
    Let \(M\) be a square \matrix with entries \(a_{i,j}\).
    Then, \(M\) is \textit{lower triangular} if \(i < j \implies a_{i,j} = 0\).
\end{snippetdefinition}
...
\begin{snippetdefinition}{linearalgebra-matrix-left-and-write-inverses}{Matrix left and right inverses}
    Let \(A\) be an \(m \times n\) matrix. A \(n\times m\) \matrix \(B\)
    is called a \textit{right inverse} of \(A\)
    if \(AB=I_m\).
    A \(n\times m\) \matrix \(B\) is called a \textit{left inverse} of \(A\)
    if \(BA=I_n\).
\end{snippetdefinition}
