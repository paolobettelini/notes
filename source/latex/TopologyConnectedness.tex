...
\begin{snippetproof}{closure-of-connected-is-connected-proof}{closure-of-connected-is-connected}{Closure of connected}
    ...
    Taking closures in \(W\): \(\closure[]{Y} \subseteq \closure[]{Z} = Z\)
    (since \(Z\) is closed in \(W\)).
    Since \(W \subseteq \closure[]{Y}\), we get \(W \subseteq Z\), hence \(Z = W\).
\end{snippetproof}
...
\begin{snippetproof}{space-is-union-of-components-proof}{space-is-union-of-components}{Decomposition into components}
    ...
    By the closure lemma, \(\closure[]{C(x)}\) is connected.
    Since \(C(x)\) is maximal connected, \(C(x) = \closure[]{C(x)}\), so \(C(x)\) is closed.
