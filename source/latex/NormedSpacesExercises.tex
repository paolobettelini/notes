...
\begin{snippetproposition}{metric-space-diameter-closure}{}
...
\begin{snippetproof}{metric-space-diameter-closure-proof}{metric-space-diameter-closure}{}
    \begin{doubleleq}
        \item Since \(A \subseteq \overline{A}\), 
        \[
            \msdiameter A = \sup_{x,y \in A} d(x,y)
            \leq \sup_{x,y \in \overline{A}} d(x,y)
            = \msdiameter \overline{A}
        \]
        \item Let \(x,y \in \overline{A}\).
...
\begin{snippetexercise}{differentiability-of-norm-ex1}{}
    Determine in which points the \function
    \[
        ||\cdot||_1 \colon \realnumbers^n \fromto \realnumbers
    \]
    is differentiable.
\end{snippetexercise}

\begin{snippetsolution}{differentiability-of-norm-ex1-sol}{}
    If
    \[
        \prod_{i=1}^n x_i \neq 0
    \]
    then \(||\cdot||_1\) is differentiable as sum of \polynomial[polynomials].
    Suppose that \(x_k = 0\).
    The partial derivative at \((x_1, \cdots, x_{k-1}, 0, x_{k+1}, \cdots, x_n)\) is given by
    \begin{align*}
        \lim_{h\to 0} \frac{
            ||(x_1, \cdots, x_{k-1}, h, x_{k+1}, \cdots, x_n)||_1 -
            ||(x_1, \cdots, x_{k-1}, 0, x_{k+1}, \cdots, x_n)||_1
        }h
        &= \lim_{h\to 0} \frac{|h|}{h}
    \end{align*}
    which does not exist as the left anf right limits are not the same.
    The \function is differentiable at the point with no null-coordinates.
\end{snippetsolution}

\begin{snippetexercise}{differentiability-of-norm-ex2}{}
    Determine in which points the \function
    \[
        ||\cdot||_\infty \colon \realnumbers^n \fromto \realnumbers
    \]
    is differentiable.
\end{snippetexercise}

\begin{snippetexercise}{differentiability-of-norm-e3}{}
    Determine in which points the \function
    \[
        ||\cdot||_2 \colon \realnumbers^n \fromto \realnumbers
    \]
    is differentiable.
\end{snippetexercise}

\begin{snippetexercise}{differentiability-of-norm-ex4}{}
    Determine in which points the \function
    \[
        ||\cdot||_2^2 \colon \realnumbers^n \fromto \realnumbers
    \]
    is differentiable.
\end{snippetexercise}

\begin{snippetexercise}{differentiability-of-norm-ex5}{}
    Determine in which points the \function
    \[
        ||\cdot||_1^2 \colon \realnumbers^n \fromto \realnumbers
    \]
    is differentiable.
\end{snippetexercise}
