...
\begin{plainhtml}
    So the sequence converges at every point, but the speed of convergence may depend on the point.
...
\begin{plainhtml}
    We want the sequence of functions to uniformly approximate
...
\begin{plainhtml}
    Starting from a certain index, all functions in the sequence are very close to each other uniformly over the entire domain,
    regardless of the limit function.
\end{plainhtml}

\begin{snippettheorem}{function-sequence-cauchy-criterion}{Cauchy criterion for sequences of functions}
    A \sequence of \function[functions] \(\{f_n\}_{n\in\naturalnumbers}\) converges uniformly to \(f \colon S \fromto \realnumbers\)
    in \(S\) \ifandonlyif for each \(\varepsilon > 0\) there exists \(N(\varepsilon)\) natural such that
    \[
        \forall m,n > N,
...
\begin{snippetproof}{function-sequence-cauchy-criterion-proof}{function-sequence-cauchy-criterion}{Cauchy criterion for sequences of functions}
    \begin{iffproof}
        \item Let \(\varepsilon > 0\).
        From the definition of uniform convergence,
        there exists \(N\left(\frac{\varepsilon }{2}\right)\) such that
        \[
            \left|f_n(x) - f(x)\right| < \frac{\varepsilon}{2}
        \]
        for each \(n > N\) and \(x \in S\).
        Therefore, \begin{align*}
            \left|f_n(x) - f(x)\right| &\leq \left|f_n(x) - f(x)\right| + \left|f_n(x) - f(x)\right| \\
            &\leq \frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon
        \end{align*}
        \item Let us show the uniform convergence. Let \(x \in S\).
        Then, by the satisfied condition, \(\{f_n(x)\}\) is a numerical sequence
        that verifies the Cauchy criterion. Consequently, there exists
        \(f_x\) such that
        \[
            \lim_{n \to \infty} f_n(x) = f_x
        \]
        for every \(x \in S\) since the hypothesis holds for all \(x\).
        We define \(f \colon S \fromto \realnumbers\) by
        \[
            f(x) \triangleq f_x
        \]
        By hypothesis, for every \(\varepsilon > 0\) there exists a natural number \(N(\varepsilon)\) such that
        \begin{align*}
            |f_n(x) - f_m(x)| < \varepsilon
        \end{align*}
        for all \(m,n > N\) and \(x \in S\).
        Taking the limit as \(m \to \infty\), by the definition of \(f\), we obtain
        \[
            |f_n(x) - f(x)| < \varepsilon
        \]
        for all \(n > N\) and \(x \in S\).
    \end{iffproof}
\end{snippetproof}
