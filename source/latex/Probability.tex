...
\begin{snippetdefinition}{probability-space-definition}{Probability Space}
    ...
    where
    \begin{itemize}
        \item \(\Omega \neq \emptyset\) is the \set of all possible outcomes, called \textit{sample space};
        \item \(\mathcal F \subseteq \powerset \Omega\) is the \set of events, called \textit{event space};
        \item \(\mathbb P\) is a \function \(\mathbb{P}\colon \mathcal{F} \fromto [0;1]\), called a \textit{probability measure}. 
    \end{itemize}
