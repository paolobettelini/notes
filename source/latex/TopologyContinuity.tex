...
\begin{plainhtml}
    The definition of continuity needs to preserve the structure of the topology.
...
\begin{plainhtml}
    This is also equivalent to saying that the map is continuous at every point.
\end{plainhtml}
...
\begin{snippetproof}{topology-continuous-map-characterization-proof}{topology-continuous-map-characterization}{}
    \begin{iffproof}
        \item By the definition of continuity (with the complement)
        \[
        ...
        \item We want to show that for each closed set \(C\), its preimage is also closed.
        Let \(A = f^{-1}(C)\). We have
        \begin{align*}
...
\begin{plainhtml}
    This means that the composition of continuous functions are also continuous.
\end{plainhtml}
...
\begin{snippetproof}{topological-continuous-at-every-point-theorem-proof}{topological-continuous-at-every-point-theorem}{}
    \begin{iffproof}
        \item Let \(x \in X\) and \(V\) a \neighborhood of \(f(x_0)\).
        By definition
        \[
            V \supseteq A \ni f(x)
        \]
        where \(A\) is \openset[open][Open].
        By continuity at a point \(x \in f^{-1}(A)\)
        is \openset[open][Open] as \(f(x) \in A\).
        We can take \(U = f^{-1}(A)\) and thus
        \[
            f(U) = f(f^{-1}(A)) \subseteq A \subseteq V
        \]
        \item We want to show that for each \openset[open][Open] \(A\) of \(Y\),
        \(f^{-1}(A)\) is \openset[open][Open] in \(X\).
        ...
        \]
        for some \openset[open][Open] \(U\) such that \(x \in U\).
        By taking \(V = A\) we know that such a \(U\) exists by definition of continuity of \(f\)
        in \(x\). Thus, there exists a \openset[open][Open] \neighborhood \(U\)
...
\begin{snippetdefinition}{topological-property-definition}{Topological property}
    A property of \topologicalspace[topological spaces]
    is a \emph{topological property} if it is preserved by homeomorphisms.
\end{snippetdefinition}

\begin{plainhtml}
    Homeomorphisms induce equivalence relations between topological spaces.
\end{plainhtml}
...
\begin{snippetdefinition}{open-closed-topological-map-definition}{Open and closed topological map}
    ...
    if for every \openset[Open] / \closedset \(S\),
    \(f(S)\) is \openset[open][Open] / \closedset[closed].
\end{snippetdefinition}
...
\begin{snippetproof}{homeomorphism-equivalent-conditions-proof}{homeomorphism-equivalent-conditions}{}
    \begin{itemize}
        ...
        and \(g\) is continuous \ifandonlyif \(\forall A\) \openset[open][Open] / \closedset[closed]
        \(g^{-1}(A) = f(A)\) is \openset[open][Open] / \closedset[closed].
    \end{itemize}
\end{snippetproof}
...
\begin{snippet}{subspace-topology-expl}
    ...
    We thus want \(i \colon Y \xhookrightarrow{} X\) to be continuous.
    For each \openset[Open] \(A\),
...
\begin{snippetproof}{subspace-topology-universal-property-proof}{subspace-topology-universal-property}{Subspace topology universal property}
    \begin{ffiproof}
        \item This is obvious as the composition of continuous functions are continuous.
        \item Suppose that \(i \circ f\) is continuous.
        We want to show that \(f\) is continuous, meaning that \(\forall B\)
        \openset[open][Open] in \(Y\), \(f^{-1}(B)\) is \openset[open][Open]
        in \(Z\).
        By definition of subspace topology, \(B = i^{-1}(A)\) for some \openset[open][Open]
        \(A\) of \(X\).
        \[
            f^{-1}(B) = f^{-1}(i^{-1}(A)) = {(i \circ f)}^{-1}(A)
        \]
        \openset[open][Open] in \(Z\), since \(i \circ f\) is continuous.
    \end{ffiproof}
\end{snippetproof}
...
\begin{snippetlemma}{closure-and-opennes-in-topological-subsets}{}
    ...
    \begin{enumerate}
        \item if \(Y\) is \openset[open][Open] in \(X\),
        then \(Z\) is \openset[open][Open] in \(Y\)
        \ifandonlyif \(Z\) is \openset[open][Open] in \(X\);
...
\begin{snippetproof}{closure-and-opennes-in-topological-subsets-proof}{closure-and-opennes-in-topological-subsets}{}
    \begin{enumerate}
        \item By definition of subspace, \(Z\) is \openset[open][Open] in \(Y\) \ifandonlyif
        \(Z = A \intersection Y\) for some \(A\) \openset[open][Open] in \(X\).
        If \(Y\) is \openset[open][Open] in \(X\), then \(Z\)
        is \openset[open][Open] in \(X\) as intersection of two \openset[open sets][Open]
        in \(X\). The other direction is obvious by definition of subspace topology.
        \item Analogous.
        \item There exist an \openset[Open] \(U\)
        of \(X\) such that \(y \in U \subseteq Y\).
        SUppose that \(Z\) is a \neighborhood of \(y\) in \(X\).
        We have \(Z \supseteq V \ni y\) where \(V\) is \openset[open][Open]
        in \(X\). This means that \(Z \intersection Y \supseteq V \intersection Y \ni y\)
        which is also \openset[open][Open] because it is the intersection of two
        \openset[open sets][Open]. \\
        Viceversa, suppose that \(Z\) is a \neighborhood
        of \(y\) in \(Y\).
        Then, there is a \(V\) \openset[open][Open] in \(Y\)
        such that \(y \in V \subseteq Z\).
        Consider \(U\) \openset[open][Open] in \(X\) such that
        ...
        This is because the \openset[open sets][Open] of such topology
        are the subsets of the form \((A \intersection Y) \intersection U\)
        where \(A\) is \openset[open][Open] in \(X\).
        ...
        The subset \(U \intersection V\) is \openset[open][Open]
        of \(U\) (viewed as a subspace of \(Y\)) as \(V\)
        is \openset[open][Open] in \(Y\).
        However, \(U\) is \openset[open][Open] in \(X\)
        and thus by \((1)\), \(U \intersection V\) is \openset[open][Open] of \(X\)
...
\begin{snippetdefinition}{topological-immersion-definition}{Topological immersion}
    ...
    if the \openset[open sets][Open]
    of \(X\) are precisely the subsets of the form \(f^{-1}(A)\)
    where \(A\) is \openset[open][Open] in \(Y\).
\end{snippetdefinition}

\begin{snippetproposition}{topological-immersion-subspace-homeomorphism}{}
    Let \(f \colon X \fromto Y\)
    be a map between \topologicalspace[topological spaces] that is continuous and \injective.
    Then, \(f\) is a \topologicalimmersion \ifandonlyif if it induces a \homeomorphism
    between \(X\) and the subspace \(f(X)\) of \(Y\).
\end{snippetproposition}

\begin{snippetproof}{topological-immersion-subspace-homeomorphism-proof}{topological-immersion-subspace-homeomorphism}{}
    \begin{iffproof}
        \item \begin{enumerate}
            \item \(f\) is \injective since by hypothesis and
                it is \surjective as the codomain is \(f(X)\).
            \item \(f\) is continuous by the universal property
                of subspace on \(f(X) \xhookrightarrow Y\).
            \item show that \(f\) is open.
                Let \(B\) be an \openset[Open] in \(X\).
                We have that \(B = f^{-1}(A)\) for some \openset[Open]
                \(A\) in \(Y\). Since \(f\) is a \topologicalimmersion
                \[
                    f(B) = f(f^{-1}(A)) = A \intersection f(X)
                \]
                which is thus \openset[open][Open].
        \end{enumerate}
        \item Let \(B\) be an \openset[Open] in \(X\).
        We know that \(f(B)\) is \openset[open][Open] in \(f(X)\)
        and since \(f(B) = V \intersection f(X)\) for some \openset[Open]
        \(V\) of \(Y\), it is also \openset[open][Open] in \(Y\).
        Since \(f\) is \injective we have \(B = f^{-1}(f(B))\)
        and thus \(B\) is the preimage of some \openset[open][Open] in \(Y\).
    \end{iffproof}
\end{snippetproof}

\begin{snippetlemma}{continuous-injective-map-open-closed-immersion}{}
    Let \(f \colon X \fromto Y\)
    be a map between \topologicalspace[topological spaces] that is continuous and \injective. Then,
    \begin{enumerate}
