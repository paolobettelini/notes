...
\begin{plainhtml}
    Note that the intersection of infinitely many open sets need not be open.
\end{plainhtml}

\begin{plainhtml}
    The elements of the topology are precisely what defines which sets are open or not.
...
\begin{snippetdefinition}{interior-point-definition}{Interior point}
    Let \(\ts\) be a \topologicalspace. A point \(p \in X\) is an
    \textit{interior point} if there exists an \openset \(S \subseteq X\) such that
    \[ x \in S \subseteq X \]
\end{snippetdefinition}
...
\begin{plainhtml}
    Interior and closure and dual via the complement (by De Morgan's laws).
\end{plainhtml}
...
\begin{plainhtml}
    This means that every neighbourhood of the point contains points of in the subset other than the central point itself.
\end{plainhtml}
