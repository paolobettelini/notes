...
\begin{plainhtml}
    Note that every two integers are congruent modulo 1.
\end{plainhtml}
...
\begin{snippetproof}{inverse-congruent-class-invertibility-theorem-proof}{inverse-congruent-class-invertibility-theorem}{Invertibility of congruence classes}
    \begin{iffproof}
        \item If \(a\) and \(n\) are \coprime, there exist \(u\) and \(v\) such that
        \[
            au+nv = 1
        \]
        ...
        \item Let \({[b]}_n\) be such that \({[a]}_n{[b]}_n = {[1]}_n\). Now,
