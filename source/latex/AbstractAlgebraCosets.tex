...
\begin{plainhtml}
    This does not work in a monoid.
\end{plainhtml}
...
%\begin{snippet}{properties-of-cosets}
...
%    where \[H = \gengrp{\kcycle{1,2}} = \{\identityfunc, \kcycle{1,2}\}\]
...
%    \end{align*}
%    Notiamo che \(H\identityfunc = H\kcycle{1,2}\),
...
\begin{snippetproposition}{cosets-partition}{Cosets form a partition}
    Let \((H, \circ) \subgroupleq (G, \circ)\) be a \group[groups].
    \begin{enumerate}
        \item \(\forall g\in G, g\in Hg\);
        \item if \(l\in Hg\), then \(Hl = Hg\);
        \item if \(Hg \intersection Hk \neq \emptyset\) for some \(g,k\in G\); then \(Hg = Hk\);
    \end{enumerate}
    In particular, the cosets of \(H\) form a \partition
    of \(G\) with subsets of cardinality \(\cardinality{H}\).
    The same properties are true for the left cosets.
\end{snippetproposition}

\begin{plainhtml}
    If we had defined the cosets on a monoid, a partition would not necessarily
form a partition and maintain the same cardinality within the cosets.
\end{plainhtml}
...
\begin{plainhtml}
    The only coset containing the identity is the subgroup itself, meaning that it is also the only coset that is a subgroup.
\end{plainhtml}
...
\begin{snippetproof}{cosets-two-elements-equivalences-proof}{cosets-two-elements-equivalences}{}
    \begin{enumerate}
        ...
        This means that \(g_1g_2^{-1} = h \in H\).
...
\begin{snippetdefinition}{subgroup-index-definition}{Subgroup index}
    Let \(G \subgroupleq H\) be \group[groups].
    The \emph{index} of \(H\) in \(G\) is the amount of right (or left) cosets of \(H\) in \(G\)
    \[
        |G\,:\, H| = |\{Hg \suchthat g\in G\}|
    \]
\end{snippetdefinition}
...
\begin{plainhtml}
    In the infinite case we can still say tha that the cardinality of the right cosets
...
\begin{snippetproof}{lagrange-theorem-proof}{lagrange-theorem}{Lagrange theorem}
    Obvious since \snippetref[cosets-partition][cosets form a partition] with same cardinality.
\end{snippetproof}
...
\begin{plainhtml}
    This also means that an infinite group can have finite cosets.
\end{plainhtml}
...
\begin{snippetproof}{prime-order-group-is-cyclic-proof}{prime-order-group-is-cyclic}{Group with prime order is cyclic}
    Let \(g \neq 1_G \in G\). Its \elementperiodtext divides \(\cardinality{G}\),
    but \(\elementperiod{g}\) is \primen, and thus \(\elementperiod{g} = 1\) or \(\elementperiod{g} = |G|\), but
...
\begin{snippettheorem}{index-multiplication-theorem}{Index multiplication theorem}
    ...
    \(|G\,:\,H|\) and \(|H\,:\,K|\) are finite. In such case,
    \[
        |G\,:\,K| = |G\,:\,H| \cdot |H\,:\,K|
    \]
\end{snippettheorem}
...
\begin{snippetproof}{index-multiplication-proof}{index-multiplication}{Index multiplication theorem}
    ...
    Likewise, let \(|G\,:\,H| = r\) and \(|H\,:\,K| = s\) be finite.
...
\begin{snippetproposition}{union-of-subgroups-is-in-product}{}
    Let \(H_1, H_2, \cdots, H_n \subgroupleq G\) be \group[groups]. Then,
    \[
        \bigcup_{i=1}^n H_i \subseteq \prod_{i=1}^n H_i \subseteq \gengrp{\bigcup_{i=1}^n H_i}
    \]
\end{snippetproposition}

\begin{snippetproof}{union-of-subgroups-is-in-product-proof}{union-of-subgroups-is-in-product}{}
    Consider the case with two \subgroup[subgroups] \(H, K\).
    Since \(h\in H\), we have \(h=h \circ 1 \in HK\)
    and the same goes ofr \(K\).
    For the second proposition if \(h\in H\)
    and \(k \in K\), we have \(k\in \gengrp{H \union K}\)
...
\begin{snippetproof}{subgroup-product-cardinality-theorem-proof}{subgroup-product-cardinality-theorem}{}
    ...
    and \(\overline{k} = x^{-1}k\).
    \begin{iffproof}
        \item This means that \(\overline{h} \circ \overline{k} = hxx^{-1}k = hk\).
        \item Suppose that \(hk = \overline{h} \circ \overline{k}\). Thus,
