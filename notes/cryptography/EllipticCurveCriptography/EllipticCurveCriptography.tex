...
\begin{enumerate}
	...
	\item The \textit{client} computes a public key \(P_c=k_cG\).
	\item The \textit{server} computes a public key \(P_s=k_sG\).
	\item The two parts publicly exchange the public keys.
	\item The \textit{client} computes \(R=k_cP_s\).
	\item The \textit{server} computes \(R=k_cP_s\).
\end{enumerate}
